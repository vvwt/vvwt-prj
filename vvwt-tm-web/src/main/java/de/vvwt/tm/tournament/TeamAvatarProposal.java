 ...
 * (E48S07 + E48S20, AC-IMPL-DTO-PLACEMENT, AC-IMPL-DTO-EXTENDED).
 *
 ...
 * the DOM; it renders {@code teamNumber} and {@code teamDescription} instead. The web controller's
 * commit path constructs {@link TeamAvatarProposal} via {@link #forCommit} with null display fields
 * (only teamId, groupNumber, groupPosition are needed for commit — display fields are ignored by
 * {@link de.vvwt.tm.tournament.internal.DefaultPhaseTransitionService#commitTransition}).
 *
...
public record TeamAvatarProposal(
     ...
     * teamDescription}, {@code sourceGroupNumber}, {@code sourceGroupPosition}) are not used by
     * {@code commitTransition} — they are meaningful only for the proposal/review phase.
     *
     ...
     * @return a proposal with zero teamNumber, null teamDescription, null source fields
     */
    public static TeamAvatarProposal forCommit(UUID teamId, int groupNumber, int groupPosition) {
        return new TeamAvatarProposal(teamId, 0, null, groupNumber, groupPosition, null, null);
    }
}

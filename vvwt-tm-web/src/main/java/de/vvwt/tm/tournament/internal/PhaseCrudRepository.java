 ...
 * into {@link de.vvwt.tm.tournament.PhaseRepository} as the low-level CRUD provider.
 *
 * <p><b>Activation note:</b> This interface is intentionally NOT registered as an active Spring
 * Data JDBC repository during the E21 reconstruction-in-place phase. Spring Data JDBC would attempt
 * to scan the {@code Phase} entity at {@code de.vvwt.tm.tournament.Phase} and the legacy {@code
 * de.vvwt.tm.domain.Phase} simultaneously — same {@code @Table("phase")} — causing an {@code
 * IllegalStateException} in the MappingContext (same pattern as E21S02 TournamentCrudRepository
 * §Deviation). {@link de.vvwt.tm.tournament.PhaseRepository} uses JdbcTemplate + manual RowMapper
 * until the E21S13 atomic cutover deletes the legacy entity.
 *

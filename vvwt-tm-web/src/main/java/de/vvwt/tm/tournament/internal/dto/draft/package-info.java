 ...
 * tournament} module. Spring Modulith 2.x does NOT automatically extend a parent package's
 * {@code @NamedInterface} to its sub-packages: {@code tournament::dto} (on {@code
 * tournament.internal.dto}) covers only types directly in that package. The {@code
 * tournament.internal.dto.draft} sub-package requires its own {@code @NamedInterface} so that the
 * {@code web} module can import {@link de.vvwt.tm.tournament.internal.dto.draft.DraftRequest},
 * {@link de.vvwt.tm.tournament.internal.dto.draft.DraftResponse}, and related records cross-module
 * without violating Spring Modulith boundary rules.
 *
 * <p>Consumers declare {@code allowedDependencies = {"tournament::draft-dto", ...}} in their
 * {@code @ApplicationModule} annotation.
 *

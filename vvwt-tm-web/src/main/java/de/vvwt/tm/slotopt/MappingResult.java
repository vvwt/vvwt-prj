...
/**
 * Value object combining the forward-mapping outputs from
 * {@link PhaseToRawPhaseDefMapper}.
 *
 * <p>Carries everything needed by the result applicator ({@link SlotResultApplicator})
 * to apply an optimizer result back to the Tournament Manager domain:
 * <ul>
 *   <li>The {@link RawPhaseDef} submitted to the optimizer.</li>
 *   <li>The {@link CanonicalPhaseDef} produced by
 *       {@link de.vvwt.worker.types.StructuralFingerprint#canonicalize(RawPhaseDef)}.</li>
 *   <li>{@code N}: the avatar count, as specified in AC2 of story E04S02.</li>
 ...
 *       match (AC3).</li>
 *   <li>{@code denseIdsByRawRow}: for each raw row index, the two dense avatar IDs occupying
 *       that row. Needed by the applicator to look up avatar positions under the permutation
 *       without re-running canonicalization.</li>
 * </ul>
 *
 ...
 * @param denseIdsByRawRow for each raw row index {@code i}, a 2-element array
 *                         {@code [denseId0, denseId1]} corresponding to the two PositionTuples
 *                         in {@code raw.rows().get(i)}
    ...
    public MappingResult {
        ...
        if (avatarCount < 0) {
            throw new IllegalArgumentException(
                    "avatarCount must be >= 0 but was: " + avatarCount);
        }
        if (matchOrder == null) {
        ...
        if (matchOrder.size() != raw.rowCount()) {
            throw new IllegalArgumentException(
                    "matchOrder.size()=" + matchOrder.size()
                    + " does not match raw.rowCount()=" + raw.rowCount());
        }
        ...
        if (denseIdsByRawRow.length != raw.rowCount()) {
            throw new IllegalArgumentException(
                    "denseIdsByRawRow.length=" + denseIdsByRawRow.length
                    + " does not match raw.rowCount()=" + raw.rowCount());
        }

...
/**
 * Spring Application Event published by {@link de.vvwt.tm.domain.CascadeRecomputeService} at
 * cascade step 12 (D-21, E03S11, AC14).
 *
 * <p>The event is published via {@link org.springframework.context.ApplicationEventPublisher}
 * inside the {@code @Transactional} method. Spring defers actual listener invocation until after
 * the transaction commits when listeners are annotated with
 * {@code @TransactionalEventListener(phase = AFTER_COMMIT)}. This guarantees that subscribers
 * (e.g., E03S13 RoundSnapshotService, E07 Gesamtübersicht) always see committed state when they
 * query.
 *

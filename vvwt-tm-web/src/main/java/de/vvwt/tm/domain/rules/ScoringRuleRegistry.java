...
/**
 * Spring bean registry that auto-injects all {@link ScoringRule} beans and exposes
...
public class ScoringRuleRegistry {
    ...
    public ScoringRuleRegistry(List<ScoringRule> rules) {
        this.rulesByBeanId = rules.stream()
                .collect(Collectors.toMap(
                        ScoringRule::getBeanId,
                        Function.identity(),
                        (a, b) -> {
                            throw new IllegalStateException(
                                    "Duplicate ScoringRule bean ID: '" + a.getBeanId()
     ...
     * @throws IllegalArgumentException if no rule is registered under {@code beanId},
    ...
    public ScoringRule get(String beanId) {
        ...
        if (rule == null) {
            throw new IllegalArgumentException(
                    "No ScoringRule bean with id '" + beanId
                    + "' — known ids: " + String.join(", ", rulesByBeanId.keySet()));
        }
        return rule;
    }
     ...
     * Useful for diagnostics and the error message in {@link #get(String)}.
     *

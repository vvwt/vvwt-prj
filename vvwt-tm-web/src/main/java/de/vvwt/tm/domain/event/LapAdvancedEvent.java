 ...
 * when the auto-advance logic at cascade step 10 increments the current lap number (E07S06, AC4).
 *
 * <p>This event is published only when a real lap advance occurs — i.e., when
 * {@code previousLapNumber != newLapNumber} after the cascade. When no advance
 * occurs, this event is NOT published.
 *
 * <p>Consumers (e.g., {@link de.vvwt.tm.infrastructure.web.DomainEventBridge}) use this event
 * to notify display devices that they should refresh their match grid to show the new lap.
 *
...
public class LapAdvancedEvent extends ApplicationEvent {
     ...
     */
    public LapAdvancedEvent(Object source,
                            UUID tenantId,
                            UUID tournamentId,
                            UUID phaseId,

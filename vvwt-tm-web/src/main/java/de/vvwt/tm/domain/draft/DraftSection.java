 ...
 * and in {@link #validate()} / {@link #validateBreaks(int)}, which are called by the
 * {@code DraftService} before persisting.
 *
 * <p>E08S05: the {@code breaks} field is added to support intra-phase breaks (AC2). Each break
 * occupies a position after a given lap number. On apply, breaks become
 * {@link de.vvwt.tm.domain.PhaseBreak} entities (AC7).
 *
...
public final class DraftSection {
    ...
    /**
     * Optional list of intra-phase breaks for this section (AC2 — E08S05).
     * Each break occurs after a specific lap number. May be empty; never {@code null}.
     * Semantic validation (valid position, no duplicates) is performed by
     * {@link #validateBreaks(int)} in the service layer before persisting.
     */
     ...
     * <p>Rules:
     * <ul>
     *   <li>Each break's {@code afterLapNumber} must be ≥ 1 and strictly less than {@code totalLaps}</li>
     *   <li>No two breaks may share the same {@code afterLapNumber} (AC9)</li>
     *   <li>Each break's {@code durationMinutes} must be {@literal > 0} (AC5 — validated at REST layer too)</li>
     * </ul>
     *
    ...
    public void validateBreaks(int totalLaps) {
        ...
        for (DraftBreak b : breaks) {
            if (b.getDurationMinutes() <= 0) {
                throw new IllegalArgumentException(
                        "Section " + sectionNumber + ": break durationMinutes must be > 0, got: "
                        + b.getDurationMinutes());
            }
            if (b.getAfterLapNumber() < 1) {
                throw new IllegalArgumentException(
                        "Section " + sectionNumber + ": break afterLapNumber must be ≥ 1, got: "
                        + b.getAfterLapNumber());
            }
            if (b.getAfterLapNumber() >= totalLaps) {
                throw new IllegalArgumentException(
                        "Section " + sectionNumber + ": break afterLapNumber " + b.getAfterLapNumber()
                        + " is out of range — must be < total laps (" + totalLaps + ")");
            }
            if (!seenPositions.add(b.getAfterLapNumber())) {
                throw new IllegalArgumentException(
                        "Section " + sectionNumber + ": duplicate break at afterLapNumber "
                        + b.getAfterLapNumber() + " (AC9 — breaks at the same lap position are not allowed)");
            }
        }
    }
    ...
    public List<DraftBreak> getBreaks() { return breaks; }
}

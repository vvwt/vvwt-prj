...
/**
 * Spring Data JDBC delegate for {@link PhaseBreak} persistence. Wired into {@link
 * PhaseBreakRepository} as the low-level CRUD provider.
 *
 * <p>All methods here are "raw" — they do not apply the tenant filter. {@link PhaseBreakRepository}
 * is responsible for all tenant-scoped operations.
 */
interface PhaseBreakCrudRepository extends CrudRepository<PhaseBreak, UUID> {
     ...
     * PhaseBreakRepository#findByPhaseId(UUID)}).
     *
     ...
     * tenant filtering applied by {@link PhaseBreakRepository#findByPhaseIdAndAfterLapNumber(UUID,
     * int)}).
     *
     * @param phaseId the phase to query
     ...
     */
    @Query(
            "SELECT * FROM phase_breaks WHERE phase_id = :phaseId AND after_lap_number ="
                    + " :afterLapNumber")

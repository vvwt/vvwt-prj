 ...
 *
 * <p>Holds the points awarded to each team slot for a single match. Points are always
 * non-negative integers (guard is enforced by the rules, not the record itself — see AC14).
 *
    ...
    public ScoringResult {
        if (team1Points < 0) {
            throw new IllegalArgumentException(
                    "team1Points must be >= 0, got: " + team1Points);
        }
        if (team2Points < 0) {
            throw new IllegalArgumentException(
                    "team2Points must be >= 0, got: " + team2Points);
        }
    }
}

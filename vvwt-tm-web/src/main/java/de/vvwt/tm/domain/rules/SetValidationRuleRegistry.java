 ...
 *
 * <p>Spring automatically injects all {@link SetValidationRule} implementations into the {@code
 * rules} map, keyed by Spring bean name (the value declared in each {@code @Component("...")}).
 *
 ...
 * }</pre>
 *
 * <p>Throws {@link IllegalArgumentException} with a descriptive message listing known IDs if the
 * requested bean name is not found.
 */
@Component
public class SetValidationRuleRegistry {
     ...
     * @throws IllegalArgumentException if no rule is registered for the given ID, including the
     *     list of known IDs in the message
    ...
    public SetValidationRule get(String beanId) {
        ...
        if (rule == null) {
            ...
            throw new IllegalArgumentException(
                    "No SetValidationRule bean with id '" + beanId + "' — known ids: " + knownIds);
        }
        return rule;
    }

    /**
     * Returns an unmodifiable view of all registered rules, keyed by bean ID. Primarily intended

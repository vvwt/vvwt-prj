 ...
 * {@link UnsupportedOperationException}. The only write method is {@link #save(AuditLogEntry)}.
 *
...
public class AuditLogRepository extends TenantScopedRepository<AuditLogEntry, UUID> {
    ...
    public AuditLogRepository(AuditLogCrudRepository delegate,
    ...
    @Override protected CrudRepository<AuditLogEntry, UUID> delegate() { return delegate; }
    ...
    @Override protected UUID extractTenantId(AuditLogEntry e) { return e.getTenantId(); }
    @Override protected void setTenantId(AuditLogEntry e, UUID id) { e.setTenantId(id); }
    @Override protected UUID extractId(AuditLogEntry e) { return e.getId(); }
     ...
     */
    public List<AuditLogEntry> findByMatchIdAndSetIndexOrderByChangedAt(UUID matchId, int setIndex) {
        UUID tenantId = activeTenantId();  // guard fires here — before any SQL
        List<AuditLogEntry> result = new ArrayList<>();
        for (AuditLogEntry entry : delegate.findByMatchIdAndSetIndexOrderByChangedAtRaw(matchId, setIndex)) {

 ...
 *
 * <ul>
 *   <li>{@link #isTieBreak(int, int)} — canonical tie-break detection used by {@code
 *       ThreePointMatchRule} in E03S08 (D-15)
 *   <li>{@link #deriveMatchState(int, int, int)} — derives a terminal/ongoing {@link MatchState}
 *       verdict from set scores
 * </ul>
 *
 * <p>Persistence: stored as {@code VARCHAR} enum name in the {@code tournament.match_format} column
 * (E03S01 AC2). The entity layer uses a Spring Data JDBC or JPA enum converter. If an unknown value
 * is read from the DB (e.g. after a post-V1 enum deletion or DB drift), the conversion fails fast
 * with an {@link IllegalArgumentException} naming the unknown value — no silent fallback.
 */
public enum MatchFormat {
     ...
     *
     * <p>Definition: a tie-break occurred when both teams were level at {@code requiredToWin - 1}
     * sets each and the match was decided in the final set. This is the canonical detection used by
     * {@code ThreePointMatchRule} (E03S08).
     *
     * <p>Return-false conditions (evaluated in order):
     *
     * <ol>
     *   <li>{@code allowsTies == true} — tie-break concept does not apply to {@code FIXED_2_SETS}
     *   <li>{@code requiredToWin <= 1} — single-set formats like {@code BEST_OF_1} have no
     *       tie-break concept; the only set IS the match
     *   <li>{@code setsPlayed != maxSets} — match did not go the full distance
     *   <li>{@code loserSetsWon != requiredToWin - 1} — loser did not reach the maximum losing
     *       score
     * </ol>
     *
     * <p>Semantic note for {@code BEST_OF_1}: a single-set match always produces a 1:0 result; the
     * {@code ThreePointMatchRule} falls through to the normal 3:0 branch, which is the correct
     * intent.
     *
     ...
     *
     * <ul>
     *   <li>{@link MatchState#FINISHED_WINNER1} — team 1 has reached {@link #getRequiredToWin()}
     *   <li>{@link MatchState#FINISHED_WINNER2} — team 2 has reached {@link #getRequiredToWin()}
     *   <li>{@link MatchState#FINISHED_STANDOFF} — {@code allowsTies == true} and both teams are
     *       level after all sets
     *   <li>{@link MatchState#ONCHECK} — match is still ongoing
     * </ul>
     *
     * <p>{@link MatchState#INPROGRESS} is never returned; that state is set externally when play
     * begins.
     *
     ...
     * constant — no silent fallback to a default. This guards against post-V1 enum deletions or DB
     * drift.
     *

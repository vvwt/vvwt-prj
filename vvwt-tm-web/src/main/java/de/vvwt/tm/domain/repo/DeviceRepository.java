 ...
 *
 * <p>Extends {@link TenantScopedRepository} to ensure all device queries are filtered by the active
 * tenant (DEC-5, DEC-17, AC9).
 *
 * <h2>Additional tenant-scoped query methods</h2>
 *
 * <ul>
 *   <li>{@link #findByDeviceToken(String)} — finds device by token, filtered to active tenant (AC2,
 *       AC3, AC8)
 *   <li>{@link #findByPin(String)} — finds device by PIN within active tenant (AC4)
 *   <li>{@link #findByLocationAndField(UUID, Integer)} — finds device assigned to a field within a
 *       location for the active tenant (AC5 conflict check)
 * </ul>
 *
...
public class DeviceRepository extends TenantScopedRepository<Device, UUID> {
    ...
    @Override
    protected CrudRepository<Device, UUID> delegate() {
        return delegate;
    }
    ...
    @Override
    protected UUID extractTenantId(Device e) {
        return e.getTenantId();
    }

    @Override
    protected void setTenantId(Device e, UUID id) {
        e.setTenantId(id);
    }

    @Override
    protected UUID extractId(Device e) {
        return e.getId();
    }
     ...
     *
     * <p>Used by the tablet to poll its own status and by any endpoint that validates device token
     * ownership.
     *
     * @param deviceToken the opaque device token
     * @return the device if found and owned by the active tenant; {@link Optional#empty()}
     *     otherwise
     */
    ...
    /**
     * Finds the device currently assigned to a specific field within a location, scoped to the
     * active tenant (AC5 — conflict check before assignment).
     *
    ...
    public Optional<Device> findByLocationAndField(UUID locationId, Integer fieldNumber) {
        UUID tenantId = activeTenantId();
        return delegate.findByTenantIdAndLocationIdAndAssignedField(
                tenantId, locationId, fieldNumber);
    }
    ...
    /**
     * Counts devices of the given type within the active tenant and specified location (E07S02
     * AC2).
     *
     * <p>Used to enforce the configurable device limit before registration. The count is scoped to
     * both tenant (DEC-5 isolation) and location (devices are per-location per DEC-5).
     *
     * @param locationId the location UUID
     ...
     *
     * <p>First looks up the device by ID within the active tenant scope (DEC-5 isolation). If
     * found, deletes it. Returns {@code false} if the device does not exist for the active tenant

 ...
 *
 * <ul>
 *   <li>AC4: Only DRAFT tournaments may be updated
 *   <li>AC5: Only DRAFT tournaments with no phases may be deleted
 *   <li>AC6: DEC-5 single-active constraint is enforced at the schema level via the {@code
 *       active_sentinel} generated column (V2 migration). The constraint is triggered when a
 *       tournament transitions to ACTIVE status (E05S06/E05S07), not at creation time. New
 *       tournaments are always created in DRAFT status. Multiple DRAFTs may coexist.
 * </ul>
 *
 * <h2>Tenant scoping</h2>
 *
 * <p>All repository calls are tenant-scoped via the {@link de.vvwt.tm.domain.repo.TenantContext}
 * ThreadLocal, resolved by {@link de.vvwt.tm.domain.repo.DefaultTenantContextResolver} for each
 * HTTP request (DEC-5, DEC-17).
 *
 * @see <a href="../../../../../../.gaai/project/contexts/artefacts/stories/E05S04.story.md">Story
 *     E05S04</a>
 */
@Service
public class TournamentService {
     ...
     *
     * <p>The new tournament is assigned a freshly generated UUID. All strategy bean IDs are
     * validated against the live Spring context before persisting.
     *
     ...
     *
     * <p>All provided fields replace the current values. A {@code null} value means "do not change
     * this field"; only non-null values are applied.
     *
     * @param id the tournament UUID
     * @param description new description (applied if not {@code null})
     * @param appointment new appointment (applied if non-null sentinel absent — use a dedicated
     *     clear mechanism if needed in future)
    ...
    /**
     * Validates that all strategy bean IDs refer to registered beans and that the match format is a
     * valid {@link MatchFormat} value. Throws {@link IllegalArgumentException} on the first invalid
     * value found.
     *

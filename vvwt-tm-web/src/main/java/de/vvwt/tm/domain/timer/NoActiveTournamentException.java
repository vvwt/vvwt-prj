 ...
 * Timer-accessible statuses are {@code PLANNED}, {@code ACTIVE}, and {@code COMPLETED}.
 * Tournaments in {@code DRAFT} or {@code CANCELLED} status are not accessible via the timer.
 *
...
public class NoActiveTournamentException extends RuntimeException {
    ...
    public NoActiveTournamentException(UUID tournamentId, String status) {
        super("Tournament " + tournamentId + " is in status '" + status
              + "' and has no active timer. Timer is available for PLANNED, ACTIVE, and COMPLETED tournaments.");

 ...
 *
 * <p>Calculated by {@link de.vvwt.tm.domain.DraftService#previewDraft} without creating any
 * database entities. Allows the organizer to review what the draft will produce before committing
 * via apply.
 *
 * @see DraftSection
 * @see <a
...
public final class DraftPreviewSection {
    ...
    /**
     * Estimated teams per group (based on participating team count ÷ groupCount). Uses integer
     * division; last group may receive fewer teams if not evenly divisible.
     */
    private final int teamsPerGroup;

    /**
     * Matches per group for round-robin game mode. Formula: {@code teamsPerGroup × (teamsPerGroup -
     * 1) / 2}.
     */
    private final int matchesPerGroup;

    /** Total laps (rounds) in this section for round-robin. Formula: {@code teamsPerGroup - 1}. */
    ...
    /**
     * Estimated duration of the section in minutes. Formula: {@code totalLaps × lapTimeMinutes +
     * max(0, totalLaps - 1) × lapBreakTimeMinutes + sectionBreakTimeMinutes}.
     */

...
/**
 * Spring Application Event published by {@link de.vvwt.tm.domain.DeviceService}
 * after a new device is persisted (E06S05, AC6).
 *
 * <p>The event is published via {@link org.springframework.context.ApplicationEventPublisher}
 * inside the {@code registerDevice()} method (non-transactional path — device registration is
 * not wrapped in a transaction at the service layer in V1). The listener in
 * {@link de.vvwt.tm.infrastructure.web.DomainEventBridge} uses
 * {@code @TransactionalEventListener(phase = AFTER_COMMIT)} — because device registration is
 * not transactional, the event fires immediately in the same thread.
 *
 * <p>The admin SPA subscribes to {@code /topic/events} and reacts to
 * {@code eventType == "DEVICE_REGISTERED"} by refreshing the device list.
 *
...
public class DeviceRegisteredEvent extends ApplicationEvent {
     ...
     *
     * @param source   the object on which the event initially occurred (the {@link de.vvwt.tm.domain.DeviceService})

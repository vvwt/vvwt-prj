 ...
 *
 * <p>Story E11S01 — AC1–AC7. All operations are scoped to a tournament that must belong
 * to the active tenant (DEC-5, DEC-17). Callers are responsible for ensuring that a valid
 * {@link de.vvwt.tm.domain.repo.TenantContext} is active before invoking any method.
 *
 * <p>Files are persisted on disk at
 * {@code {dataDir}/audio/{tournamentId}/{category}.mp3} per AC6 / DEC-15.
 * No database table is used — persistence is purely filesystem-based.
 *
...
public interface AudioStorageService {
     ...
     *
     * <p>AC1: Accepts a multipart {@code .mp3} upload. Uploading to a category that already
     * has a file replaces it. Returns metadata for the stored file.
     *
     ...
     * @throws java.util.NoSuchElementException if the tournament does not exist or is not in the active tenant (AC5 → 404)
     ...
     */
    AudioFileMetadata upload(UUID tournamentId, AudioCategory category, String filename,
                             InputStream inputStream, long sizeBytes);

    /**
     * Opens a stream for reading the audio file for the given tournament and category.
     *
     * <p>AC2: Returns the raw byte stream. Callers must close the stream after use.
     * Returns an empty {@link java.util.Optional} if no file exists for the category (→ 404).
     *
     * @param tournamentId the tournament UUID (must belong to the active tenant)
     * @param category     the audio category (START, END, PAUSE)
     ...
     *
     * <p>AC4: Returns {@code true} if the file was deleted, {@code false} if no file
     * existed for that category (→ 404 from the controller).
     *
     * @param tournamentId the tournament UUID (must belong to the active tenant)
     * @param category     the audio category (START, END, PAUSE)

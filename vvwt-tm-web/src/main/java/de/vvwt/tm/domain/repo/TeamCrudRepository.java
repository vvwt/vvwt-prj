...
interface TeamCrudRepository extends CrudRepository<Team, UUID> {
     ...
     * ordered by team_number ascending (AC1 — E05S05).
    ...
    /**
     * Counts how many team_avatar rows reference the given team (AC4 — E05S05).
     * Used to prevent deletion of teams that are already referenced in avatars.
     */
    @Query("SELECT COUNT(*) FROM team_avatar WHERE team_id = :teamId")
    int countAvatarsByTeamId(@Param("teamId") UUID teamId);

    /**
     * Counts teams in the tournament with the given team_number, excluding one team by ID.
     * Used for AC10 duplicate team_number detection on create and update.
     */
    @Query("SELECT COUNT(*) FROM team WHERE tournament_id = :tournamentId "
    ...
    /**
     * Returns the maximum team_number in use for a tournament, or 0 if no teams exist.
     * Used for auto-assignment of team_number when not provided (AC2 — E05S05).
     */

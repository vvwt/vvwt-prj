 ...
 *
 * <p>A {@code MatchOutcome} is a 1:1 aggregate of a completed match, storing the final
 * set scores and ball scores across all {@code SetResult} rows. It is populated by
 * cascade step 4 (D-21) and kept synchronized with {@code match.state} by cascade step 6.
 *
 * <h2>Match-state invariant (D-32)</h2>
 * <p>The {@code computedState} column must always equal {@code match.state}. This invariant
 * is maintained by the cascade service (E03S11), not by a database constraint (H2 does not
 * support cross-table CHECK constraints). The helper method
 * {@link #matchesComputedState(Match)} allows the cascade service to verify the invariant
 * before committing a result.
 *
 * <h2>Backward compatibility (E03S08)</h2>
 * <p>This class was introduced as a minimal value class in E03S08 with three fields
 * ({@code team1SetsWon}, {@code team2SetsWon}, {@code setCount}) for use by
 * {@code ScoringRule} implementations. E03S04 augments it with {@code @Table}, {@code @Id},
 * and the additional DB-mapped fields required by AC2. All existing code that constructs
 * {@code MatchOutcome} using the three-field constructor continues to compile and run
 * correctly — the new persistence-layer fields are set by the repository layer.
 *
...
public class MatchOutcome {
    ...
    /**
     * Primary key — also the FK to {@code match(id)} (1:1 relationship).
     * Set by the application layer before insert (same UUID as the corresponding Match).
     */
    @Id
    private UUID matchId;

    /**
     * Tenant scope — NOT NULL per DEC-17. Every match outcome belongs to exactly one tenant.
    ...
    /**
     * Total number of sets played (team1SetsWon + team2SetsWon for all V1 formats).
     * Default 0.
     */
    private int setCount;

    /**
     * The {@link MatchState} derived from this outcome, stored as a legacy integer code.
     * Must always equal {@code match.state} (D-32 invariant).
     * Use {@link #getComputedMatchState()} and {@link #setComputedMatchState(MatchState)}
     ...
     *  updated by the cascade service on each correction. */
    ...
    /**
     * Minimal constructor for {@link de.vvwt.tm.domain.rules.ScoringRule} usage (E03S08
     * backward compatibility). Creates an instance without persistence-layer fields set.
     *
     ...
     */
    public MatchOutcome(UUID matchId, UUID tenantId,
                        int team1SetsWon, int team1BallsWon,
                        int team2SetsWon, int team2BallsWon,
                        int setCount, int computedState,
     ...
     *
     * <p>The cascade service (E03S11) calls this helper after cascade step 4 and before
     * committing the outcome to confirm that the {@link MatchOutcome} reflects the same
     * lifecycle state as the {@link Match} it is attached to.
     *
    ...
    public int getTeam1BallsWon() { return team1BallsWon; }
    public void setTeam1BallsWon(int team1BallsWon) { this.team1BallsWon = team1BallsWon; }
    ...
    public int getTeam2BallsWon() { return team2BallsWon; }
    public void setTeam2BallsWon(int team2BallsWon) { this.team2BallsWon = team2BallsWon; }

    public int getSetCount() { return setCount; }
    public void setSetCount(int setCount) { this.setCount = setCount; }

    public int getComputedState() { return computedState; }
    public void setComputedState(int computedState) { this.computedState = computedState; }
    ...
    public String toString() {
        return "MatchOutcome{matchId=" + matchId

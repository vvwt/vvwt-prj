 ...
 * <ul>
 *   <li>In a non-deciding set: the winner must reach <strong>25 points</strong> with a
 *       lead of at least 2 points.</li>
 *   <li>In the deciding set (the last possible set of the match): the winner must reach
 *       <strong>15 points</strong> with a lead of at least 2 points.</li>
 * </ul>
 *
 * <p>The Spring bean name {@code "standardVolleyball"} is the identifier stored in
 * {@code Tournament.set_validation_rule_id} and used for registry lookup.
 *
 * <p>This rule is <em>not</em> compatible with {@link MatchFormat#FIXED_2_SETS} because
 * that format has no deciding-set concept ({@code decidingSet} is empty).
 * Attempting to use this rule with {@code FIXED_2_SETS} throws
 * {@link IllegalArgumentException} — the organizer must configure {@code timeBounded}
 * for {@code FIXED_2_SETS} tournaments.
 *
...
public class StandardVolleyballSet implements SetValidationRule {
     ...
     * <p>Logic:
     * <ol>
     *   <li>Validate inputs (null format, negative points)</li>
     *   <li>Reject {@code FIXED_2_SETS} — no deciding-set concept</li>
     *   <li>Determine the target: 15 pts if {@code setIndex} is the deciding-set index,
     *       25 pts otherwise</li>
     *   <li>Check whether either team leads with &ge; target AND a &ge; 2-point margin</li>
     * </ol>
     *
     * @throws IllegalArgumentException if {@code format} is {@code null}, if either
     *         point count is negative, or if {@code format} is {@code FIXED_2_SETS}
     */
    @Override
    public ValidationResult isSetClosed(int team1Pts, int team2Pts, int setIndex, MatchFormat format) {
        ...
        if (!format.getDecidingSet().isPresent()) {
            throw new IllegalArgumentException(
                    "StandardVolleyballSet is not compatible with format " + format
        ...
        log.debug("{} decision for ({},{}) set={} format={} target={}: closed={} reason='{}'",
                BEAN_ID, team1Pts, team2Pts, setIndex, format, target,

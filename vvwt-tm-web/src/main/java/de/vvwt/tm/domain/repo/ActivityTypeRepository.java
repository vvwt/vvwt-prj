 ...
 *
 * <p>All operations enforce tenant context via the {@link TenantScopedRepository} guard. A call
 * without an active {@link TenantContext} throws {@link IllegalStateException} before any SQL is
 * executed.
 *
...
public class ActivityTypeRepository extends TenantScopedRepository<ActivityType, UUID> {
    ...
    @Override
    protected CrudRepository<ActivityType, UUID> delegate() {
        return delegate;
    }
    ...
    @Override
    protected UUID extractTenantId(ActivityType e) {
        return e.getTenantId();
    }

    @Override
    protected void setTenantId(ActivityType e, UUID id) {
        e.setTenantId(id);
    }

    @Override
    protected UUID extractId(ActivityType e) {
        return e.getId();
    }

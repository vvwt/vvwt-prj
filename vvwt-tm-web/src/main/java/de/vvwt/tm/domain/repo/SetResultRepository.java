 ...
 * Data {@code @Id} field (as noted in the E03S03 impl-report). Therefore this repository does NOT
 * extend {@link TenantScopedRepository} — it uses {@link NamedParameterJdbcTemplate} directly and
 * applies the tenant guard manually.
 *
 * <p>All methods call {@link TenantContext#getTenantId()} before any SQL, satisfying the AC6
 * runtime guard requirement. The AC5 append-only constraint does not apply here — see {@link
 * AuditLogRepository}.
 *
 * @see TenantContext
 * @see <a
...
public class SetResultRepository {
    ...
    private static final String INSERT_SQL =
            "INSERT INTO set_result (match_id, set_index, tenant_id, phase_id, team1_points,"
                + " team2_points, set_state, change_time, created_at) VALUES (:matchId, :setIndex,"
                + " :tenantId, :phaseId, :team1Points, :team2Points, :setState, CURRENT_TIMESTAMP,"
                + " CURRENT_TIMESTAMP)";

    private static final String UPDATE_SQL =
            "UPDATE set_result SET team1_points = :team1Points, team2_points = :team2Points,"
                    + " set_state = :setState, change_time = CURRENT_TIMESTAMP WHERE match_id ="
                    + " :matchId AND set_index = :setIndex AND tenant_id = :tenantId";
    ...
    public SetResultRepository(
            NamedParameterJdbcTemplate namedParameterJdbcTemplate, TenantContext tenantContext) {
    ...
    private static final RowMapper<SetResult> SET_RESULT_ROW_MAPPER =
            new RowMapper<>() {
                @Override

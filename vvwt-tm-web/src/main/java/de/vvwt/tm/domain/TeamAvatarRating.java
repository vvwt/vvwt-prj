 ...
 *
 * <p>A {@code TeamAvatarRating} is the standings row for a team's avatar in its phase. It is 1:1
 * with {@link TeamAvatar}: one rating row per avatar, updated by cascade steps 7–8 (D-21) each time
 * a match result is registered.
 *
 * <h2>Sort order for group table derivation (D-33)</h2>
 *
 * <p>The {@link #compareTo(TeamAvatarRating)} method implements the canonical sort order used by
 * the group table view:
 *
 * <ol>
 ...
 *
 * <p>When a team has no losses, division by zero would produce infinity. The sentinel convention is
 * to use {@link Double#MAX_VALUE} as the quotient in that case (legacy parity). This ensures an
 * undefeated team always outranks a team with any losses when points and raw set/ball counts are
 * tied.
 *
 * <h2>is_without_assessment flag</h2>
 *
 * <p>Copied from {@code team.without_assessment} at avatar creation time. V1 does NOT auto-refresh
 * this flag if the team flag changes mid-tournament (D-26, story notes).
 *
...
public class TeamAvatarRating implements Comparable<TeamAvatarRating> {
    ...
    /**
     * Primary key — also the FK to {@code team_avatar(id)} (1:1 relationship). Set by the
     * application layer before insert (same UUID as the corresponding TeamAvatar).
     */
    @Id private UUID avatarId;

    /**
     * Tenant scope — NOT NULL per DEC-17. Every rating belongs to exactly one tenant. FK references
     * {@code tenants(id)}.
     */
    ...
    /**
     * Set quotient: {@code setsWon / setsLost}. {@link Double#MAX_VALUE} when {@code setsLost == 0}
     * (sentinel for undefeated in sets). Default 0.
     */
    private double setQuotient;

    /**
     * Ball quotient: {@code ballsWon / ballsLost}. {@link Double#MAX_VALUE} when {@code ballsLost
     * == 0} (sentinel for undefeated in balls). Default 0.
     */
    private double ballQuotient;

    /**
     * If {@code true}, this avatar is excluded from normal standings and always ranks last in group
     * tables (D-26). Copied from {@code team.without_assessment} at avatar creation. Default {@code
     * false}.
     */
     ...
     *
     * <ol>
     *   <li>{@code isWithoutAssessment = true} rows always sort last (force to end)
     *   <li>Higher {@code points} ranks first (DESC)
     *   <li>Tie: higher {@code setQuotient} ranks first (DESC)
     *   <li>Tie: higher {@code ballQuotient} ranks first (DESC)
     * </ol>
     *
    ...
    public boolean isWithoutAssessment() {
        return isWithoutAssessment;
    }

    public void setWithoutAssessment(boolean withoutAssessment) {
        isWithoutAssessment = withoutAssessment;
    }
    ...
    public String toString() {
        return "TeamAvatarRating{avatarId="
                + avatarId
                + ", points="
                + points
                + ", setsWon="

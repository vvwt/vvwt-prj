 ...
 * <ul>
 *   <li>AC5 — lap range validation: {@code afterLapNumber} must be ≥ 1 and < totalLaps in phase</li>
 *   <li>AC6 — duplicate detection: at most one break per lap boundary per phase</li>
 *   <li>AC7 — tenant scope: delegated to {@link PhaseBreakRepository}</li>
 *   <li>AC8 — i18n: validation error messages sourced from {@code messages.properties}</li>
 * </ul>
 *
 * <p>Total laps for a phase are derived from the match schedule: the highest {@code lapNumber}
 * across all {@link Match} rows for the phase. This follows from the story definition:
 * "total laps are derived from the phase's match schedule (the highest {@code lap_number}
 * across all matches in the phase)."
 *
 * @see PhaseBreak
...
public class PhaseBreakService {
    ...
    public PhaseBreakService(PhaseRepository phaseRepository,
     ...
     * <ol>
     *   <li>Load the phase (throws {@link NoSuchElementException} if absent or wrong tenant — AC7)</li>
     *   <li>Determine total laps from the match schedule (max {@code lapNumber} across matches)</li>
     *   <li>Validate {@code afterLapNumber} ≥ 1 and < totalLaps (AC5)</li>
     *   <li>Detect duplicate at the same lap boundary (AC6)</li>
     *   <li>Persist and return the new {@link PhaseBreak}</li>
     * </ol>
     *
     ...
     * @throws ConflictException if a break already exists at {@code afterLapNumber} in the phase (AC6)
     */
    @Transactional
    public PhaseBreak createPhaseBreak(UUID phaseId, int afterLapNumber,
                                        int durationMinutes, String label) {
        ...
        phaseBreakRepository.findByPhaseIdAndAfterLapNumber(phaseId, afterLapNumber)
                .ifPresent(existing -> {
                    String message = messageSource.getMessage(
        ...
        PhaseBreak phaseBreak = new PhaseBreak(
                UUID.randomUUID(),
                null,  // tenantId: TenantScopedRepository.save() sets this from TenantContext
                phaseId,
                afterLapNumber,
                durationMinutes,
                label);

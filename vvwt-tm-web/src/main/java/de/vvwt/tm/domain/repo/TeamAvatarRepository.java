 ...
 *
 * <p>Provides the domain-specific query {@link #findByPhaseId(UUID)} required by the match
 * generator (E03S09).
 *
...
public class TeamAvatarRepository extends TenantScopedRepository<TeamAvatar, UUID> {
    ...
    public TeamAvatarRepository(
            TeamAvatarCrudRepository delegate,
    ...
    @Override
    protected CrudRepository<TeamAvatar, UUID> delegate() {
        return delegate;
    }
    ...
    @Override
    protected UUID extractTenantId(TeamAvatar e) {
        return e.getTenantId();
    }

    @Override
    protected void setTenantId(TeamAvatar e, UUID id) {
        e.setTenantId(id);
    }

    @Override
    protected UUID extractId(TeamAvatar e) {
        return e.getId();
    }

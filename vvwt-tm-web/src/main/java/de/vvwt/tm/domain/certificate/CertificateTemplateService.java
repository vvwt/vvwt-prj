 ...
 * <p>All methods validate tournament existence and tenant membership before touching the
 * filesystem or H2. A missing or wrong-tenant tournament yields {@link java.util.NoSuchElementException}.
 *
 * <h2>One template per tournament (AC8)</h2>
 * <p>The system enforces exactly one certificate template per tournament. Uploading a new
 * template replaces the existing one (AC4) — both the file and the H2 metadata row.
 *
...
public interface CertificateTemplateService {
     ...
     *
     * <p>If a template already exists, the file is replaced on the filesystem and the
     * metadata row in H2 is upserted (AC4). Returns the metadata of the stored template.
     *
     ...
     * @throws CertificateTemplateFormatException    if format is not .html or .svg, or file is malformed (AC7)
     ...
     */
    CertificateTemplateMetadata upload(UUID tournamentId, String filename,
     ...
     * @return the file content and content-type, or {@link Optional#empty()} if no template uploaded
    ...
    /**
     * Returns the template metadata (filename, format, upload timestamp, file size) for the
     * given tournament (AC3).
     *
     * @param tournamentId tournament UUID (tenant-scoped)
     ...
     * system's template contract — the set of Mustache placeholders
     * ({@code {{name}}}) that the rendering engine (E12S06) will substitute during generation.
     *

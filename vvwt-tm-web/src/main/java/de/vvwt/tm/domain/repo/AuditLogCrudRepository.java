...
/**
 * Spring Data JDBC delegate for {@link AuditLogEntry} persistence. Wired into {@link
 * AuditLogRepository} as the low-level CRUD provider.
 */
interface AuditLogCrudRepository extends CrudRepository<AuditLogEntry, UUID> {
    @Query(
            "SELECT * FROM audit_log WHERE match_id = :matchId AND set_index = :setIndex "

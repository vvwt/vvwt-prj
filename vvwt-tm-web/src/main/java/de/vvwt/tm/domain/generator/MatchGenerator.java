 ...
 *
 * <p>Each V1 match-generator implementation is a stateless Spring bean registered with a unique
 * bean ID. The phase-preparation service (E03S12) resolves the applicable generator via {@code
 * Tournament.matchGeneratorId} through the {@link MatchGeneratorRegistry}, then calls this method
 * to produce the full set of matches for a phase.
 *
 * <p>V1 implementations:
 *
 * <ul>
 *   <li>{@code roundRobin} — every team plays every other team exactly once ({@link
 ...
 *   <li>The generator returns NEW {@link Match} instances (not yet persisted). The caller (E03S12
 *       phase-preparation service) persists them.
 *   <li>The generator MUST NOT mutate the {@code Phase} or the {@link TeamAvatar} entities.
 *   <li>The returned {@code List} has a deterministic ordering for the same input (AC5).
 *   <li>If the avatar list has fewer than 2 entries, the generator returns an empty list (AC12).
 *   <li>{@code null} phase, {@code null} avatars list, {@code null} entries in the list, or
 *       duplicate avatar IDs throw {@link IllegalArgumentException} (AC13).
 * </ul>
 *
...
public interface MatchGenerator {
     ...
     *
     * <p>The returned matches have {@code state = OPEN(0)}, {@code lapNumber = null}, and {@code
     * fieldNumber = null}. The {@code set_limit} is derived from the tournament's {@code
     * MatchFormat.maxSets}. The caller is responsible for persisting the returned list.
     *
     ...
     *     may be empty (returns empty list for &lt; 2)
     ...
     *     entry in {@code avatars} is null, or if {@code avatars} contains duplicate IDs
     */
    List<Match> generate(Phase phase, List<TeamAvatar> avatars);

    /**
     * Returns the Spring bean ID of this generator, used as the key in {@link
     * MatchGeneratorRegistry}.
     *
     * <p>The returned value must match the {@code @Component} name declared on the implementation
     * class and the value stored in {@code Tournament.matchGeneratorId}.
     *

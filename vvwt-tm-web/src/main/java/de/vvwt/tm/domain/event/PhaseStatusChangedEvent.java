 ...
 * <ul>
 *   <li>A new phase is started: PENDING → ACTIVE</li>
 *   <li>A phase is completed: ACTIVE → COMPLETED</li>
 * </ul>
 *
 * <p>Display devices subscribed to the tenant-scoped WebSocket topic receive this event
 * and re-fetch the full phase overview from the display REST endpoints to re-render
 * the Gesamtübersicht layout for the new phase.
 *
 * <p>Published via {@link org.springframework.context.ApplicationEventPublisher} inside a
 * {@code @Transactional} method. Listeners with
 * {@code @TransactionalEventListener(phase = AFTER_COMMIT)} fire after the transition is
 * committed, ensuring display devices always see the final committed state.
 *
...
public class PhaseStatusChangedEvent extends ApplicationEvent {
     ...
     */
    public PhaseStatusChangedEvent(Object source,
                                   UUID tenantId,
                                   UUID tournamentId,
                                   UUID phaseId,

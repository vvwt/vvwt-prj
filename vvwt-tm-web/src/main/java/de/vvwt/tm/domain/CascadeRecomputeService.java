...
import de.vvwt.tm.tournament.internal.AuditLogRepository;
...
/**
 * Core cascade recompute service — implements the 13-step {@code registerMatchResult} flow per D-21
 * (E03S11).
 *
 * <h2>Transaction boundary (AC2)</h2>
 *
 * <p>The {@code registerMatchResult} method is annotated {@code @Transactional} with {@code
 * REPEATABLE_READ} isolation. All 13 steps run inside a single transaction. A failure at any step
 * rolls back the entire operation. Step 12 (event publication) fires after commit via Spring's
 * {@code @TransactionalEventListener(phase = AFTER_COMMIT)} on the listener side.
 *
 ...
 *
 * <p>Step 9 is intentionally absent from the runtime — per D-24 and S-22, the GroupTable is
 * computed on read. No {@code group_table} table is written during the cascade.
 *
 ...
 * structurally impossible through the repository API.
 *
 * @see SetResultInput
...
public class CascadeRecomputeService {
    ...
    public CascadeRecomputeService(
     ...
     *
     * <p>Steps 1–11 execute within a single {@code REPEATABLE_READ} transaction. Step 12 (event
     * publication) fires after commit on the listener side. Step 13 returns void.
     *
     * <p>All repository calls carry the active {@link de.vvwt.tm.domain.repo.TenantContext} (AC27,
     * DEC-17).
     *
     * @param input the set result to register (must not be {@code null})
     * @throws ValidationException if the set score fails the {@link
     *     de.vvwt.tm.domain.rules.SetValidationRule} (AC3)
    ...
    public void registerMatchResult(SetResultInput input) {
        ...
        log.info(
                "[cascade] START matchId={} setIndex={} team1={} team2={} actor={}"
                        + " correlationId={}",
                input.matchId(),
                input.setIndex(),
                input.team1Points(),
        ...
        Tournament tournament =
                tournamentRepository
                        .findById(match.getTournamentId())
                        .orElseThrow(
                                () ->
                                        new IllegalArgumentException(
        ...
        if (!validationResult.isClosed()) {
            log.warn(
                    "[cascade] FAIL Step1: validation rejected — {} correlationId={}",
        ...
        if (isUpdate) {
            ...
            log.info(
                    "[cascade] Step2 UPDATE set_result ({},{}) old=({},{}) new=({},{})"
                            + " correlationId={}",
                    input.matchId(),
                    input.setIndex(),
                    team1PointsOld,
            ...
            log.info(
                    "[cascade] Step2 INSERT set_result ({},{}) new=({},{}) correlationId={}",
                    input.matchId(),
                    input.setIndex(),
                    input.team1Points(),
                    input.team2Points(),
        ...
        log.debug(
                "[cascade] Step2 audit_log written id={} correlationId={}",
                auditEntry.getId(),
                correlationId);
        ...
        log.debug(
                "[cascade] Step3 aggregate sets={} t1setsWon={} t2setsWon={} t1balls={} t2balls={}"
                        + " correlationId={}",
                setCount,
                team1SetsWon,
                team2SetsWon,
        ...
        log.info(
                "[cascade] Step4 MatchOutcome upserted matchId={} derivedState={} correlationId={}",
                input.matchId(),
                derivedState,
                correlationId);
        ...
        if (previousState != derivedState) {
            ...
            log.info(
                    "[cascade] Step6 Match.state updated {} -> {} matchId={} correlationId={}",
                    previousState,
                    derivedState,
                    input.matchId(),
                    correlationId);
            ...
            log.debug(
                    "[cascade] Step6 Match.state unchanged ({}) — no-op UPDATE matchId={}"
                            + " correlationId={}",
                    previousState,
                    input.matchId(),
                    correlationId);
        ...
        log.debug(
                "[cascade] Step7 refreshAvatarRating avatar1Id={} correlationId={}",
        ...
        log.debug(
                "[cascade] Step8 refreshAvatarRating avatar2Id={} correlationId={}",
        ...
        if (currentMatchLap != null) {
            ...
            if (allTerminalInLap) {
                ...
                log.info(
                        "[cascade] Step10 lap auto-advanced {} -> {} phaseId={} correlationId={}",
                        previousLapNumber,
                        newLapNumber,
                        phase.getId(),
                        correlationId);
                ...
                log.debug(
                        "[cascade] Step10 lap NOT advanced (not all matches terminal in lap {})"
            ...
            log.debug(
                    "[cascade] Step10 skipped — match has no lapNumber assigned yet matchId={}"
                            + " correlationId={}",
                    input.matchId(),
                    correlationId);
        }
        ...
        if (previousLapNumber != newLapNumber) {
            ...
            log.info(
                    "[cascade] Step12a LapAdvancedEvent published {} -> {} phaseId={}"
                            + " correlationId={}",
                    previousLapNumber,
                    newLapNumber,
                    match.getPhaseId(),
        ...
        // -----------------------------------------------------------------------
        log.info(
                "[cascade] DONE matchId={} newState={} correlationId={}",
                input.matchId(),
                derivedState,
                correlationId);
    }
     ...
     *
     * <p>Reads ALL terminal matches for the avatar in the phase, recomputes aggregates from
     * scratch, and upserts the rating row. This "full-recompute" approach avoids incremental-delta
     * bugs on corrections (per Brief T-11 rationale).
     *
    ...
    private void refreshAvatarRating(
        ...
        for (Match m : terminalMatches) {
            ...
            if (outcomeOpt.isEmpty()) {
                log.warn(
                        "[cascade] refreshAvatarRating: MatchOutcome not found for terminal match"
        ...
        TeamAvatarRating rating =
                new TeamAvatarRating(
                        avatarId,
                        null, // tenantId will be set by TenantScopedRepository.save()
                        matchCount,
                        totalSetCount,
                        totalPoints,
                        totalSetsWon,
        ...
        log.debug(
                "[cascade] refreshAvatarRating avatarId={} matchCount={} points={} setsWon={}"
                        + " setsLost={} correlationId={}",
                avatarId,
                matchCount,
                totalPoints,
                totalSetsWon,

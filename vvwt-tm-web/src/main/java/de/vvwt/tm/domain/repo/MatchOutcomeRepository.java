...
public class MatchOutcomeRepository extends TenantScopedRepository<MatchOutcome, UUID> {
    ...
    public MatchOutcomeRepository(
            MatchOutcomeCrudRepository delegate,
    ...
    @Override
    protected CrudRepository<MatchOutcome, UUID> delegate() {
        return delegate;
    }
    ...
    @Override
    protected UUID extractTenantId(MatchOutcome e) {
        return e.getTenantId();
    }

    @Override
    protected void setTenantId(MatchOutcome e, UUID id) {
        e.setTenantId(id);
    }

    @Override
    protected UUID extractId(MatchOutcome e) {
        return e.getMatchId();
    }
     ...
     * cascade). In PENDING state, no outcomes should exist (no results have been recorded), so this
     * is a no-op in the normal case — but protects structural correctness.
     *

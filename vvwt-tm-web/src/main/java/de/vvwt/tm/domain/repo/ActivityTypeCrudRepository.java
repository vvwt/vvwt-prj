...
/**
 * Spring Data JDBC delegate for {@link ActivityType} persistence.
 * Wired into {@link ActivityTypeRepository} as the low-level CRUD provider.
 */
interface ActivityTypeCrudRepository extends CrudRepository<ActivityType, UUID> {

    /**
     * Returns all activity types belonging to the given tournament (unscoped — tenant filtering
     * is applied in {@link ActivityTypeRepository#findByTournamentId(UUID)}).
     *

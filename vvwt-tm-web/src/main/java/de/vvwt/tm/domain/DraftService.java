 ...
 *
 * <ol>
 *   <li>AC2: {@link #saveDraft} — serialize sections to JSON, store on tournament
 *   <li>AC3: {@link #getDraft} — read JSON, return {@link DraftConfig} (empty if no draft)
 *   <li>AC4: {@link #previewDraft} — calculate phase/group/match counts without persisting
 *   <li>AC5–AC7: {@link #applyDraft} — create Phase entities, distribute Phase 1 TeamAvatars,
 *       transition tournament to PLANNED
 * </ol>
 *
 * <h2>Business rules enforced</h2>
 *
 * <ul>
 *   <li>AC2: only DRAFT-status tournaments may save/modify a draft
 *   <li>AC5: apply requires DRAFT status, ≥ 1 section, ≥ 2 participating teams
 *   <li>AC7: apply transitions tournament DRAFT → PLANNED (DEC-5 safe: only ACTIVE is constrained)
 *   <li>AC11: apply validates setQuantity against tournament matchFormat
 *   <li>AC12: apply is rejected if phases already exist (tournament already applied)
 *   <li>AC14: all operations are tenant-scoped via the TenantContext (via repositories)
 * </ul>
 *
...
public class DraftService {
     ...
     * @param phaseBreakRepository phase break persistence (tenant-scoped) — for apply (AC7 —
     *     E08S05)
     * @param teamRepository team persistence (tenant-scoped) — for preview/apply team count
     * @param teamAvatarRepository team avatar persistence (tenant-scoped) — for Phase 1
     *     distribution
     * @param timelineCalculationService stateless service for computing predicted clock times
     *     (E08S03)
     ...
     *
     * <p>Only DRAFT-status tournaments may have their draft modified. If the tournament is in
     * PLANNED or any other non-DRAFT status, a {@link ConflictException} is thrown.
     *
     ...
     * (round-robin), total laps, total matches, and estimated time.
     *
     * <p>Preview requires at least one section in the draft; otherwise throws {@link
    ...
    public DraftPreviewResult previewDraft(UUID tournamentId) {
        ...
        for (DraftSection section : config.getSections()) {
            ...
            List<PhaseBreakConfig> breakConfigs =
        ...
        // (section break is defined per-section but applies between sections — use first section's
        // value
        // or 0 if only one section)
        int sectionBreakMinutes =
                config.getSections().size() > 1
     ...
     *
     * <ul>
     *   <li>Tournament must be in DRAFT status (409 if not)
     *   <li>Draft must have at least 1 section (400 / 409 per AC5)
     *   <li>At least 2 participating teams must exist (400 / 409 per AC5)
     *   <li>No phases may already exist for this tournament (409 per AC12)
     *   <li>setQuantity for each section must be compatible with the tournament's matchFormat (400)
     * </ul>
     *
     ...
     * team[i] → group = (i % groupCount) + 1, position = (i / groupCount) + 1. sortTypes {@code
     * placement_group} and {@code group_placement} fall back to {@code team_number} ordering for
     * Phase 1 (AC6).
     *
     * <h2>Status transition (AC7)</h2>
     *
     * <p>Tournament transitions from DRAFT to PLANNED after successful apply. The draft can no
     * longer be modified after this point.
     *
     * @param tournamentId the tournament UUID
     * @return ordered list of created Phase IDs (one per section), never empty
     * @throws NoSuchElementException if the tournament does not exist for the current tenant
     * @throws ConflictException if tournament is not DRAFT, has existing phases, or preconditions
     *     not met
     * @throws IllegalArgumentException if draft has no sections, <2 participating teams, or invalid
     *     setQuantity
     * @throws IllegalStateException if no tenant context is active
     */
    public List<UUID> applyDraft(UUID tournamentId) {
        ...
        if (config.getSections().isEmpty()) {
            throw new ConflictException(
                    "Cannot apply draft for tournament '"
                            + tournamentId
                            + "': the draft has no sections. Add at least one section before"
                            + " applying.");
        }
            ...
            Phase phase =
                    new Phase(
                            ...
                            java.time.LocalDateTime
                                    .now() // createdAt — DB default not set by Spring Data JDBC
                            );
        ...
        // AC7: transition tournament DRAFT → PLANNED, clear draft_json (phases are now source of
        // truth)
     ...
     *
     * <ul>
     *   <li>groupNumber = (i % groupCount) + 1
     *   <li>groupPosition = (i / groupCount) + 1
     * </ul>
     *
     ...
     */
    private void distributePhase1TeamAvatars(
    ...
    private DraftPreviewSection computePreview(DraftSection section, int participatingTeamCount) {
        ...
        // Estimated time: laps × lapTime + inter-lap breaks + intra-phase break durations + section
        // break
    ...
    /**
     * Persists each {@link DraftBreak} from a section as a {@link PhaseBreak} entity linked to the
     * given phase (AC7 — E08S05).
     *
     * <p>Breaks are not validated here — break validation occurs in {@link #saveDraft} and during
     * apply preconditions. This method performs only the persistence step.
     *
    ...
    /**
     * Validates that the section's setQuantity is compatible with the tournament matchFormat
     * (AC11).
     *
     * <p>setQuantity must be ≤ matchFormat.maxSets. For FIXED_2_SETS, setQuantity must equal 2. For
     * BEST_OF_N formats, setQuantity must equal the format's maxSets (all sets are played in a
     * structured bracket — the match format dictates the set count).
     *
    ...
    /**
     * Loads the tournament by ID for the active tenant, throwing {@link NoSuchElementException} if
     * not found.
     *
     * @param tournamentId the tournament UUID
    ...
    /**
     * Serializes a {@link DraftConfig} to JSON, wrapping {@link JsonProcessingException} as {@link
     * IllegalStateException} (a configuration failure, not a recoverable error).
     *
        ...
        try {
            ...
            throw new IllegalStateException(
                    "Failed to deserialize draft JSON from database. "
                            + "The stored draft may be corrupt.",
                    ex);
        }
    }
}

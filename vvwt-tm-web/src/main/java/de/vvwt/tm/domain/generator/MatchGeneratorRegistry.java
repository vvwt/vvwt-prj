...
/**
 * Spring bean registry that auto-injects all {@link MatchGenerator} beans and exposes a
...
public class MatchGeneratorRegistry {
     ...
     * @throws IllegalArgumentException if no generator is registered under {@code beanId}, with the
    ...
    public MatchGenerator get(String beanId) {
        ...
        if (generator == null) {
            throw new IllegalArgumentException(
                    "No MatchGenerator bean with id '"
                            + beanId
                            + "' — known ids: "
                            + String.join(", ", generatorsByBeanId.keySet()));
        }
        return generator;
    }
    ...
    /**
     * Returns an unmodifiable view of the full registry map. Useful for diagnostics.
     *

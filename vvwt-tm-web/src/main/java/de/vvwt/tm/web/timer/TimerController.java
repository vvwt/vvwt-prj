package de.vvwt.tm.web.timer;

import de.vvwt.tm.timer.TimerDataResponse;
import de.vvwt.tm.timer.TimerDataService;
import java.util.UUID;
import org.springframework.http.ResponseEntity;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

/**
 * REST controller for the timer data endpoint (E26S03 Q-1a TDD reconstruction).
 *
 * <p>Provides a single public endpoint for timer devices to fetch the full tournament schedule,
 * current position, and audio configuration. No authentication is required
 * (AC-AUTHENTICATION-FLOW-PRESERVED — the timer endpoint is {@code permitAll()}).
 *
 * <h2>Endpoint (Wave-2-aligned URL per Brief v1.1 D-10)</h2>
 *
 * <pre>GET /api/timer/tournaments/{tournamentId}</pre>
 *
 * <h2>Pattern A (DEC-40 §2026-04-27 Clarification)</h2>
 *
 * <p>The return type {@link TimerDataResponse} is a bounded-context-owned query-shape record at
 * {@code de.vvwt.tm.timer.*} (per E26S01 Pattern A placement). No web-tier DTO mapping is needed
 * (projection == wire shape; Clause B (a)/(c)/(d) conditions do NOT fire). The controller is a thin
 * Jackson-serialization adapter: delegates to service, wraps in {@link ResponseEntity}.
 *
 * <h2>Security (AC-AUTHENTICATION-FLOW-PRESERVED)</h2>
 *
 * <p>{@code /api/timer/tournaments/**} is listed in {@link de.vvwt.tm.auth.internal.SecurityConfig}
 * as {@code permitAll()}. The {@code /api/**} catch-all rule that requires authentication is
 * declared AFTER this, so the timer endpoint remains public.
 *
 * <h2>Error handling</h2>
 *
 * <p>Exceptions are translated by {@link de.vvwt.tm.web.GlobalExceptionHandler}:
 *
 * <ul>
 *   <li>{@link de.vvwt.tm.timer.InvalidTimerUrlException} → HTTP 404 with {@code errorCode:
 *       "INVALID_TIMER_URL"}
 *   <li>{@link de.vvwt.tm.timer.NoActiveTournamentException} → HTTP 404 with {@code errorCode:
 *       "NO_ACTIVE_TOURNAMENT"}
 * </ul>
 *
 * @see TimerDataService
 * @see de.vvwt.tm.timer.TimerDataResponse
 * @since E26S03
 */
@RestController
@RequestMapping("/api/timer/tournaments")
public class TimerController {

    private final TimerDataService timerDataService;

    public TimerController(TimerDataService timerDataService) {
        this.timerDataService = timerDataService;
    }

    /**
     * Returns the full timer data for the given tournament.
     *
     * <p>The response covers:
     *
     * <ul>
     *   <li>Tournament metadata and status
     *   <li>Ordered schedule of rounds and breaks with optional wall-clock times
     *   <li>Phase structure summary with per-phase lap counts
     *   <li>Audio file URLs per category (Wave-2-aligned URL shape)
     * </ul>
     *
     * <p>Returns HTTP 200 in all success cases, including when no phases have been configured yet
     * ({@code emptySchedule: true}).
     *
     * @param tournamentId the UUID of the tournament from the URL path
     * @return 200 with {@link TimerDataResponse}, or 404 if the tournament does not exist or is in
     *     DRAFT / CANCELLED status
     */
    @GetMapping("/{tournamentId}")
    public ResponseEntity<TimerDataResponse> getTimerData(
            @PathVariable("tournamentId") UUID tournamentId) {

        TimerDataResponse response = timerDataService.buildTimerData(tournamentId);
        return ResponseEntity.ok(response);
    }
}

...
/**
 * TM-side publisher integration service for the Public Participant Info Portal (AC1, AC2, AC3,
 ...
 *   <li>Snapshot recovery on 409 FULL_RESYNC — posts snapshot to /api/v1/publish/{t}/{l}/{id}/snapshot.
 ...
 *
 * @see <a href="../../../../../../../../.gaai/project/contexts/artefacts/stories/E38S09.story.md">E38S09</a>
 */
public class InfoPortalPublisherService {
    ...
    public void registerTenant() {
        ...
        try {
            String publicKeyBase64 = Base64.getEncoder()
                    .encodeToString(keypairManager.getPublicKey().getEncoded());
            RegistrationRequest req = new RegistrationRequest(
                    "ed25519", publicKeyBase64, null, null); // UNSIGNED (signature=null)
            Envelope<RegistrationRequest> envelope =
            ...
            if (resp.getStatusCode().is2xxSuccessful() && resp.getBody() instanceof RegistrationResponse regResp) {
            ...
            log.error("[InfoPortal] Unexpected error during tenant registration: {}", e.getMessage());
    ...
    /**
     * Registers a tournament via the dedicated endpoint (D-X2 c). Returns the
     * {@code tournament_token} + {@code per_tournament_secret} and persists them in
     * {@code info_portal_state}.
     *
    ...
    public void registerTournament(String tournamentId, List<UUID> teamUuids) {
        ...
        try {
            ...
            String signatureBase64 = Base64.getEncoder()
                    .encodeToString(keypairManager.sign(canonicalBytes));
            ...
            ResponseEntity<Object> resp = restTemplate.postForEntity(
                    properties.getUrl() + "/api/v1/tournaments/"
            ...
            log.error("[InfoPortal] Error registering tournament '{}': {}", tournamentId, e.getMessage());
        }
    }
     ...
     *
     * <p>Seq is assigned atomically before posting. On 409 FULL_RESYNC, the current
     * {@code last_published_seq} is retained (NOT reset to 1) and a snapshot is posted (AC3, AC12).
     *
    ...
    public void publishDelta(String tournamentId, String eventJson) {
        if (!properties.isEnabled() || status.isAlgorithmDeprecatedHardStop()) {
            log.info("[InfoPortal] Publisher not active — skipping delta publish for '{}'", tournamentId);
            return;
        }

        Optional<InfoPortalStateRecord> stateOpt =
                stateDao.findByTournament(properties.getTenantId(), properties.getLocationId(), tournamentId);
        if (stateOpt.isEmpty()) {
            log.warn("[InfoPortal] No registration found for tournament '{}' — cannot publish delta", tournamentId);
            return;
        }

        try {
            // Atomic seq increment BEFORE posting (AC12)
            long seq = stateDao.incrementAndGetSeq(
            ...
            restTemplate.postForEntity(
                    properties.getUrl() + "/api/v1/publish/"
                            + properties.getTenantId() + "/"
                            + properties.getLocationId() + "/"
                            + tournamentId + "?seq=" + seq,
            ...
            log.error("[InfoPortal] 5xx from info-server for tournament '{}': {}",
                    tournamentId, e.getStatusCode());
            status.recordServerError();
        } catch (Exception e) {
            log.error("[InfoPortal] Unexpected error publishing delta for '{}': {}",
     ...
     * The info-server overwrites its {@code last_applied_seq} to that value; subsequent deltas
     * continue at {@code seq = current + 1}.
     *
     * @param tournamentId tournament identifier
    ...
    public void postSnapshot(String tournamentId, String snapshotJson) {
        try {
            ...
            long currentSeq = stateDao.findCurrentSeq(
                    properties.getTenantId(), properties.getLocationId(), tournamentId);

            String snapshotPayload = "{\"sequenceNumber\":" + currentSeq
                    + ",\"state\":" + snapshotJson + "}";
            ...
            log.info("[InfoPortal] Snapshot posted for tournament '{}' at seq={}", tournamentId, currentSeq);
        } catch (Exception e) {
            log.error("[InfoPortal] Error posting snapshot for '{}': {}", tournamentId, e.getMessage());
            status.recordServerError();
        }
    }
     ...
     * Severity is HIGH if {@code days_remaining ≤ deprecationWarningThresholdDays}, else LOW.
     * The warning is always surfaced (DEC-43 D3 mandates it regardless of threshold).
     */
    public void handleAlgorithmWarning(AlgorithmWarning warning) {
        if (warning == null) {
            return;
        }
        int daysRemaining = warning.days_remaining() != null ? warning.days_remaining() : Integer.MAX_VALUE;
        boolean withinThreshold = daysRemaining <= properties.getDeprecationWarningThresholdDays();
        PublisherStatus.DeprecationSeverity severity = withinThreshold
        ...
        log.warn("[InfoPortal] Algorithm '{}' deprecated on {} ({} days remaining) — severity={}",
                warning.algorithm_id(), warning.deprecation_date(), daysRemaining, severity);
    }
    ...
    private void handleRegistrationError(HttpClientErrorException e) {
        ...
        if (status4xx == null) {
            log.error("[InfoPortal] Registration error (unknown status {}): {}", e.getStatusCode(), e.getMessage());
        ...
        switch (status4xx) {
            ...
            case CONFLICT -> {
                log.warn("[InfoPortal] Registration key mismatch (409 KEY_MISMATCH) — re-register required");
                this.status.setRegistrationError(true);
            }
            case GONE -> {
    ...
    private void handlePublishError(HttpClientErrorException e, String tournamentId, String eventJson) {
        HttpStatus status4xx = HttpStatus.resolve(e.getStatusCode().value());
        if (status4xx == null) {
            log.error("[InfoPortal] Publish error (unknown status {}): {}", e.getStatusCode(), e.getMessage());
            return;
        }
        switch (status4xx) {
            case UNAUTHORIZED -> {
                log.error("[InfoPortal] Signature mismatch (401) for tournament '{}'"
                        + " — info-server signature mismatch; keypair may have changed; re-register manually",
                        tournamentId);
                this.status.setSignatureMismatch(true);
            }
            case CONFLICT -> {
                // 409 FULL_RESYNC — snapshot recovery (AC3, D-X8 c — no queue)
                log.info("[InfoPortal] 409 FULL_RESYNC for tournament '{}' — posting snapshot", tournamentId);
                postSnapshot(tournamentId, eventJson != null ? eventJson : "{}");
            }
            case FORBIDDEN -> {
                log.error("[InfoPortal] 403 RegistrationRejected for tournament '{}': {}",
                        tournamentId, e.getMessage());
            }
            case GONE -> {
                log.error("[InfoPortal] 410 ALGORITHM_DEPRECATED — publisher stopped; operator must rotate algorithm");
                this.status.setAlgorithmDeprecatedHardStop(true);
            }
            case TOO_MANY_REQUESTS -> {
                log.warn("[InfoPortal] 429 RateLimited for tournament '{}' — exponential backoff", tournamentId);
                this.status.recordRateLimited();
            }
            default -> log.warn("[InfoPortal] Unexpected 4xx ({}) for tournament '{}'",
                    status4xx, tournamentId);
        }
    }
    ...
    /**
     * Mutable status holder for the publisher (admin UI data source via
     * {@link InfoPortalStatusController}).
     */
    public static class PublisherStatus {

        public enum DeprecationSeverity { HIGH, LOW }
        ...
        public boolean isRegistrationError() { return registrationError; }
        public void setRegistrationError(boolean v) { this.registrationError = v; }

        public boolean isSignatureMismatch() { return signatureMismatch; }
        public void setSignatureMismatch(boolean v) { this.signatureMismatch = v; }

        public boolean isAlgorithmDeprecatedHardStop() { return algorithmDeprecatedHardStop; }
        public void setAlgorithmDeprecatedHardStop(boolean v) { this.algorithmDeprecatedHardStop = v; }

        public boolean hasDeprecationWarning() { return deprecationAlgorithmId != null; }
        public DeprecationSeverity getDeprecationSeverity() { return deprecationSeverity; }
        public String getDeprecationAlgorithmId() { return deprecationAlgorithmId; }
        public LocalDate getDeprecationDate() { return deprecationDate; }

        public boolean isPublisherUnhealthy() { return publisherUnhealthy; }

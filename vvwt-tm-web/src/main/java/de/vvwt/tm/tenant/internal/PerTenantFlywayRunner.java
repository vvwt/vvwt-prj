 ...
 * <ol>
 *   <li>Resolves the tenant's {@link DataSource} via {@link TenantDataSourceResolver}
 *       (which enforces fail-fast for unknown tenants — AC6).</li>
 *   <li>Derives the per-module migration locations from {@link ApplicationModules#of(Class)}
 *       in module-dependency order (using the module comparator — AC3).</li>
 *   <li>Configures and executes Flyway programmatically against the tenant-specific DataSource
 *       using only {@code classpath:db/migration/{moduleName}} locations (AC3, AC8).</li>
 * </ol>
 *
 ...
 *
 * <p>This runner preserves the DEC-21 module-dependency-ordered semantics by consulting
 * {@code ApplicationModules.of(applicationClass).getComparator()} — the same ordering logic
 * that Modulith uses internally — but invoked at tenant-creation time, not boot time.
 *
 ...
 * skipped (Flyway would throw a {@code FlywayException} for non-existent locations if not filtered).
 * The legacy root {@code classpath:db/migration} is NEVER included (AC8).
 *
 * <h2>Idempotency (AC5)</h2>
 * <p>Flyway's default behaviour is to skip already-applied migrations based on
 * {@code flyway_schema_history} checksums. No custom idempotency logic is needed.
 *
 * <h2>Failure behaviour (AC4)</h2>
 * <p>Any {@code FlywayException} thrown by Flyway propagates unchanged. The runner does NOT
 * catch, wrap, or suppress Flyway exceptions. Tenant cleanup on migration failure is a lifecycle
 * concern (caller responsibility), not a runner concern.
 *
...
public class PerTenantFlywayRunner {
     ...
     * @param applicationClass the Spring Boot main class, used to bootstrap
     *                         {@link ApplicationModules#of(Class)} for module-ordered migration
     *                         locations; must not be {@code null}
     ...
     *
     * <p>Resolves the tenant's DataSource, derives module-ordered migration locations, and
     * invokes Flyway. Subsequent calls with the same {@code tenantId} are idempotent — Flyway
     * skips already-applied migrations based on {@code flyway_schema_history} (AC5).
     *
     ...
     * @throws org.flywaydb.core.api.FlywayException           if a migration fails (AC4); propagated unchanged
     */
    ...
    /**
     * Derives the ordered list of Flyway migration locations from the project's
     * {@link ApplicationModules}, filtered to only locations that exist on the classpath.
     *
     * <p>Location format: {@code classpath:db/migration/{moduleName}}.
     * The legacy root {@code classpath:db/migration} is NEVER returned (AC8).
     * Ordering follows {@link ApplicationModules#getComparator()} — dependencies before dependents.
     *
     ...
     *
     * @return an ordered, non-null list of classpath migration locations; may be empty if no
     *         module has a migration directory on the classpath
     */
    public List<String> buildLocations() {
        ...
        java.util.Comparator<ApplicationModule> comparator =
                (java.util.Comparator<ApplicationModule>) (java.util.Comparator<?>) modules.getComparator();

        List<ApplicationModule> orderedModules = modules.stream()
                .sorted(comparator)
                .toList();
    ...
    /**
     * Returns {@code true} if the given classpath location has at least one resource accessible
     * on the current classpath.
     *
     * <p>Uses {@link ClassLoader#getResource(String)} to probe the location directory.
     * If the resource is found (non-null), the location is considered valid.
     *
     * @param classpathLocation a Flyway location string of the form {@code classpath:db/migration/{name}}
    ...
    private static boolean migrationLocationExistsOnClasspath(String classpathLocation) {
        ...
        String resourcePath = classpathLocation.startsWith("classpath:")

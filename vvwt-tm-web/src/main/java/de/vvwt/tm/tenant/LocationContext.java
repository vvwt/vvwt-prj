 ...
 * <p>Any bounded context that needs to scope an operation to the current venue location
 * (e.g., a WebSocket session handler that must distinguish court-specific vs. overview-mode
 * display behaviour). Consumers declare {@code @ApplicationModule(allowedDependencies = "tenant")}
 * and inject {@code LocationContext} via constructor injection.
 *
 * <h2>Relationship to {@link TenantContext}</h2>
 * <p>{@code LocationContext} mirrors the shape of {@link TenantContext}: it offers
 * {@link #current()}, {@link #bind(UUID)}, and nested-bind semantics via the
 * {@link Scope} inner interface. The binding is per-thread (backed by a {@code ThreadLocal}).
 *
 * <h2>Wave-1 usage contract (DEC-24 D2)</h2>
 * <p>At WebSocket handshake time, the {@code DeviceTokenHandshakeInterceptor} binds this context
 * to the device's {@code location_id} when the device has an assigned location, OR sets a
 * session-level "overview mode" marker and leaves this context UNBOUND when the device has
 * {@code location_id = NULL}. Handlers that need location-specific data MUST check whether
 * a location context is bound before calling {@link #current()}.
 *
 ...
 * <p>DEC-24 acknowledges that a Wave-2 Discovery session may move {@code LocationContext}
 * to its own bounded context (e.g., a {@code location} module). For Wave 1, it lives beside
 * {@link TenantContext} in the {@code tenant} root package for locality.
 *
 * @see TenantContext
...
public interface LocationContext {
     ...
     *         Callers MUST always be inside a {@link Scope} before invoking {@code current()}.
     *         This method NEVER returns {@code null} and NEVER silently falls back to a default.
     */
    UUID current();
     ...
     * closing the inner scope restores the outer (AC3 nested-bind contract, mirrors
     * {@link TenantContext} AC-NESTED-BIND from E14S01).
     *
     * @param locationId the location to bind; must not be {@code null}
     * @return a {@link Scope} that, when closed, unbinds this location (or restores the outer location)

 ...
 * Consumers declare {@code @ApplicationModule(allowedDependencies = "tenant")} and inject {@code
 * TenantContext} via constructor injection. Usage example:
 *
 * <pre>{@code
 ...
 * <h2>Thread-safety</h2>
 *
 * <p>Implementations MUST be thread-safe. The binding is per-thread (typically backed by a {@code
 * ThreadLocal}). Each thread maintains its own binding stack; bindings in one thread are never
 * visible to another thread.
 *
 * <h2>Transactional semantics</h2>
 *
 * <p>The tenant binding MUST be established BEFORE a {@code @Transactional} boundary is entered.
 * DataSource routing (via {@code AbstractRoutingDataSource}) reads the binding at
 * connection-acquisition time, which is the moment a transaction begins. Binding after transaction
 * start has no effect on the active connection.
 *
 * <h2>Nested-bind contract (AC-NESTED-BIND)</h2>
 *
 * <p>{@link #bind(UUID)} MUST support re-entrant / nested binding. An inner {@code bind} overrides
 * the outer for the inner's scope only; closing the inner scope restores the outer. Downstream
 * stories (E14S03+) consume this contract and MUST NOT re-open it.
 *
...
public interface TenantContext {
     ...
     *     programming error — callers MUST always be inside a {@link Scope} before invoking {@code
     *     current()}. This method NEVER returns {@code null} and NEVER silently returns a shared or
     *     default DataSource (AC5).
     */
    UUID current();

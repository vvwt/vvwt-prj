 ...
 *
 * <p>Runs as an {@link ApplicationRunner} with {@code @Order(2)} so it executes after {@code
 * DefaultTenantBootstrap} ({@code @Order(1)}) and after Flyway has applied all migrations
 * (including V6 which creates the {@code admin_credentials} table).
 *
 * <h2>Startup flow</h2>
 *
 * <ol>
 *   <li>Query {@code SELECT id, password_hash FROM admin_credentials} (at most one row due to the
 *       singleton_guard unique index from V6 migration).
 *   <li>If one row exists: load the hash into memory. Log the startup notice (AC3). The plaintext
 *       password cannot be recovered from the hash — only the INFO log line from first boot
 *       matters. The log line on subsequent boots reads: {@code [tm-auth] Admin password: (set at
 *       first start — check startup log)}
 *   <li>If no row exists: generate a 16-character alphanumeric password via {@link SecureRandom}
 *       (AC1, AC11). Hash it with bcrypt. Persist both ID and hash. Log the plaintext password at
 *       INFO (AC3). Store the hash in memory.
 *   <li>If more than one row exists: abort with {@link IllegalStateException} — schema invariant
 *       violated (should be impossible due to singleton_guard unique index).
 * </ol>
 *
 * <h2>Concurrent-start race (AC1)</h2>
 *
 * <p>If two processes start simultaneously and both see zero rows, one INSERT will fail with a
 * unique-constraint violation on {@code idx_admin_credentials_singleton}. The losing process
 * catches {@link DataIntegrityViolationException}, re-queries to load the winner's hash, and logs a
 * notice that it could not display the plaintext. The winner already logged the plaintext (AC3
 * satisfied for the first boot's primary process).
 *
 * <h2>DB unavailable (AC9)</h2>
 *
 * <p>Any DB exception other than the concurrent-race constraint violation is rethrown as {@link
 * IllegalStateException} with a clear actionable message, aborting startup.
 *
 * <h2>Password strength (AC11)</h2>
 *
 * <p>The alphabet has 62 characters [A-Za-z0-9]. A 16-character password drawn uniformly from this
 * alphabet has log2(62^16) ≈ 95.3 bits of entropy — well above the 72-bit minimum required by AC11.
 * {@link SecureRandom} is cryptographically strong per the Java SE specification.
 *
 * <h2>Password not logged at DEBUG/TRACE (AC11)</h2>
 *
 * <p>The plaintext variable is used only in the INFO log line. No DEBUG or TRACE log statement in
 * this class references the plaintext or hash value.
 *
 * @see SecurityConfig
 * @see <a
...
public class AdminCredentialsBootstrap implements ApplicationRunner, AdminCredentialsProvider {
    ...
    /**
     * Password alphabet: uppercase, lowercase, digits — 62 characters. No special characters to
     * ensure log-friendliness and AC11 compliance (alphanumeric).
     */
    ...
    /**
     * Holds the bcrypt hash of the admin password after bootstrap completes. {@code null} before
     * bootstrap — callers must not access before context is ready.
     */
        ...
        if (existing.size() > 1) {
            throw new IllegalStateException(
                    "Multiple admin_credentials rows detected — schema invariant violated "
                            ...
                            + "Investigate database integrity. Found "
                            + existing.size()
                            + " rows.");
        }
    ...
    private List<CredentialRow> queryCredentials() {
        return jdbcTemplate.query(
                SELECT_CREDENTIALS,
                (rs, rowNum) -> new CredentialRow(rs.getObject(1, UUID.class), rs.getString(2)));
    }

    /**
     * Generates a random alphanumeric password, hashes it with bcrypt, persists the hash, and logs
     * the plaintext at INFO level.
     *
     * <p>On concurrent-start race: catches {@link DataIntegrityViolationException}, re-queries, and
     * loads the winning process's hash. Does NOT log the plaintext (cannot recover it).
     */
    private void generateAndPersist() {
        ...
        try {
            transactionTemplate.executeWithoutResult(
                    status -> jdbcTemplate.update(INSERT_CREDENTIALS, id, hash));
            ...
            throw new IllegalStateException(
                    "FATAL: Admin credentials bootstrap failed: could not insert admin_credentials"
                            + " row. Check database connectivity, disk space, and schema integrity."
                            + " Underlying cause: "
     ...
     *
     * <p>Entropy: log2(62^16) ≈ 95.3 bits (AC11 minimum: 72 bits). The value is returned as a
     * {@code String} and used only in the INFO log line (AC11 — not logged at DEBUG or TRACE).
     */

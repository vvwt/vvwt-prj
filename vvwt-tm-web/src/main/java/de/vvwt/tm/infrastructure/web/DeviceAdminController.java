 ...
 * <h2>Endpoints</h2>
 * <ul>
 *   <li>POST /api/admin/devices/{deviceId}/location/{locationId} — assign device to a location (AC4, AC5)</li>
 *   <li>DELETE /api/admin/devices/{deviceId}/location — remove device location assignment (AC5)</li>
 * </ul>
 *
 * <h2>Authentication</h2>
 * <p>All endpoints require admin HTTP Basic auth. The {@code /api/admin/**} path is covered by
 * the existing {@link de.vvwt.tm.auth.SecurityConfig} rule that requires authentication
 * for all {@code /api/**} paths.
 *
 * <h2>Tenant scope (DEC-5, DEC-24)</h2>
 * <p>Location assignment is validated against the active tenant (AC8 — cross-tenant guard):
 * {@code locationId} must belong to the device's tenant. Validation is delegated to
 * {@link DeviceService#assignDeviceLocation(UUID, UUID)}.
 *
...
public class DeviceAdminController {
     ...
     * <p>Returns:
     * <ul>
     *   <li>200 OK with updated device summary on success</li>
     *   <li>400 Bad Request if {@code locationId} does not belong to the active tenant (AC8)</li>
     *   <li>404 Not Found if {@code deviceId} does not exist for the active tenant</li>
     * </ul>
     *

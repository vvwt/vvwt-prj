 ...
 * <ul>
 *   <li>{@code /admin} or {@code /admin/} — the SPA entry point (AC3)</li>
 *   <li>{@code /admin/**} — deep-link URL typed into the address bar (AC4, AC8)</li>
 * </ul>
 *
 ...
 * {@code @RequestMapping} fall through to Spring Boot's default {@code BasicErrorController},
 * which returns HTTP 404 (AC8). This controller does NOT install a global catch-all.
 *
...
public class AdminSpaController {
     ...
     *
     * <p>Maps both {@code /admin} (without trailing slash) and {@code /admin/} (with trailing slash)
     * to forward to the SPA {@code index.html} in {@code classpath:/static/admin/}.
     */
     ...
     *
     * <p>Paths such as {@code /admin/tournaments} typed directly into the address bar are
     * forwarded to the SPA {@code index.html}. The Svelte router then handles the hash-based
     * sub-route client-side.
     *
     * <p>This mapping does NOT match static asset paths ({@code /admin/assets/**}) because
     * Spring Boot resolves classpath static resources before dispatching to controllers.
     */
    @GetMapping("/admin/**")

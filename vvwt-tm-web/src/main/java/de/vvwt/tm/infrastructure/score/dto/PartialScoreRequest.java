 ...
 *
 * <p>Sent by the scoring tablet on every button press or direct input change.
 * The server validates the device token, verifies field ownership (AC12), and broadcasts
 * the partial update via WebSocket to admin UI clients.
 *
 * <p>No set result is persisted — this is a live score broadcast only. The final set result
 * is submitted via {@link SetSubmitRequest} when the scorekeeper confirms the set.
 *

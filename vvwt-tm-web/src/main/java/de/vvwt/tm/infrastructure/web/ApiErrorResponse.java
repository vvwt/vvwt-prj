 ...
 * <h2>Fields</h2>
 * <ul>
 *   <li>{@code status} — HTTP status code (e.g., 400, 404, 409, 500)</li>
 *   <li>{@code error} — HTTP status reason phrase (e.g., "Bad Request")</li>
 *   <li>{@code message} — Human-readable error description</li>
 *   <li>{@code messageKey} — i18n message key for the SPA translation layer (AC9).
 *       The SPA uses this key to look up a locale-specific message. If no specific key
 *       applies, the value is {@code "error.internal"} for 500s or a specific key
 *       like {@code "error.validation"}, {@code "error.notFound"}, {@code "error.conflict"}.</li>
 *   <li>{@code path} — The request path that triggered the error (aids debugging)</li>
 *   <li>{@code timestamp} — ISO-8601 instant when the error occurred</li>
 *   <li>{@code fieldErrors} — Optional list of per-field validation errors (AC1, 400 responses only).
 *       Null/absent for non-validation errors. {@link JsonInclude#NON_NULL} ensures null fields
 *       are omitted from the serialized JSON (AC2).</li>
 * </ul>
 *
 * <h2>Security (AC1)</h2>
 * <p>The {@code message} field contains a generic message for 500 errors — no stack traces,
 * no internal class names, no database details are included in the response body.
 *
...
public final class ApiErrorResponse {
    ...
    /**
     * @return i18n message key for the SPA translation layer (AC9).
     *         E.g., {@code "error.validation"}, {@code "error.notFound"},
     *         {@code "error.conflict"}, {@code "error.internal"}.
     */
    ...
    /**
     * @return Per-field validation errors; {@code null} for non-validation errors
     *         (omitted from JSON by {@link JsonInclude#NON_NULL} — AC2).
     */
    public java.util.List<FieldError> getFieldErrors() { return fieldErrors; }

 ...
 *   <li>POST   /api/tournaments/{tournamentId}/teams/bulk       — bulk create teams (AC5)</li>
 * </ul>
 *
 * <h2>Error handling</h2>
 * <p>All exceptions are mapped to structured JSON responses by {@link GlobalExceptionHandler}:
 * <ul>
 *   <li>{@link java.util.NoSuchElementException} → 404 (tournament or team not found)</li>
 *   <li>{@link ConflictException} → 409 (status constraint or duplicate team_number)</li>
...
public class TeamController {
    ...
    public ResponseEntity<TeamResponse> updateTeam(
        ...
        var team = teamService.updateTeam(
                tournamentId,
                id,
                request.description(),
                request.resolvedTeamNumber(),
     ...
     *
     * <p>Returns HTTP 200 with per-item results. Individual failures (e.g., duplicate
     * team_number) do not prevent other items from being created.
     *
     * @param tournamentId the tournament UUID (path variable)
    ...
    public ResponseEntity<TeamBulkCreateResponse> bulkCreateTeams(
        ...
        List<TeamService.BulkCreateRequest> domainRequests = request.teams().stream()
                .map(t -> new TeamService.BulkCreateRequest(
                        t.description(),

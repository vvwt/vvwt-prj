 ...
 * field". The service applies only non-null (and valid) values. The appointment field may be set to
 * {@code null} to clear the tournament date.
 *
 * <p>Bean validation is applied only to fields that are present (non-null). Null values are skipped
 * by the service layer rather than rejected — this is a partial-update pattern.
 *
 * <p>E08S05: {@code plannedStartTime} is added to allow the organizer to set the tournament start
 * time for print timeline calculation (AC1). Setting to {@code null} clears the field.
 *
 * @see <a
...
public record TournamentUpdateRequest(
        ...
        /**
         * Optional planned start time (HH:mm) for timeline calculation (AC1 — E08S05). Applied
         * unconditionally — {@code null} clears the existing value. Jackson deserializes from
         * ISO-8601 time string (e.g., {@code "10:00"}).
         */

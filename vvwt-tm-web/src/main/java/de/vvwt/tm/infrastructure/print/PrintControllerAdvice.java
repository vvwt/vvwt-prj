 ...
 * package and handles {@link TournamentNotFoundException} before the global handler can. Spring's
 * {@code @ExceptionHandler} resolution prefers the most specific applicable handler, and a handler
 * in the same package as the throwing controller takes precedence over a global catch-all for the
 * same exception type.
 *
 * <p>Returns HTTP 404 directly via {@link HttpServletResponse#sendError} to avoid any response-body
 * serialization issues (the global handler's JSON serializer would conflict with the print
 * controller's text/html content-type expectation).
 *
 * <h2>AC7</h2>
 *
 * <p>Non-existent tournament → HTTP 404. This handler enforces that contract for the print route
 * without returning JSON or raw stack traces.
 *
 * @see PrintController

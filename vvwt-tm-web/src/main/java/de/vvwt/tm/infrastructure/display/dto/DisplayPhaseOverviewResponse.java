 ...
 * <p>The {@code tenantId} field was added so the display SPA can build the tenant-scoped
 * WebSocket topic {@code /topic/display/{tenantId}/events} (E07S06 AC1, AC11).
 * The token already determines the tenant on the server; this field just sends it back so the
 * client can subscribe to the correct topic without additional round-trips.
 *

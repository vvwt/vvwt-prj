 ...
 * <h2>Routes</h2>
 * <ul>
 *   <li>{@code GET /display/overview} — serves the Gesamtübersicht (E07S05 AC8)</li>
 *   <li>{@code GET /display/register} — serves the device registration page (E07S07 AC1, AC8)</li>
 * </ul>
 *
 * <p>Both routes forward to the same Vite-built {@code index.html}. The Svelte app uses
 * {@code window.location.pathname} to decide which "page" to render client-side (E07S07 plan §2).
 * Spring Security permits the entire {@code /display/**} path space without authentication
 * (see {@link de.vvwt.tm.auth.SecurityConfig}).
 *
 * <h2>SPA serving strategy</h2>
 * <p>The display SPA does not use a router library. {@code App.svelte} inspects
 * {@code window.location.pathname} on mount to dispatch to {@code DisplayRegisterPage} or the
 * existing overview flow. Both paths share the same Vite entry point and compiled bundle —
 ...
 *
 * <h2>Security (AC11)</h2>
 * <p>The display routes serve a read-only Svelte bundle. No admin actions are possible from
 * this surface. Device tokens are never embedded in the HTML shell — tokens are written to and
 * read from {@code localStorage} exclusively by the Svelte app at runtime.
 *
 ...
 * @see <a href="../../../../../../../../.gaai/project/contexts/artefacts/stories/E07S07.story.md">Story E07S07</a>
 */
@Controller
public class DisplayViewController {
     ...
     *
     * <p>Forwards to the same Vite-built {@code index.html} as {@code /display/overview}.
     * The Svelte app reads {@code window.location.pathname} on mount and renders
     * {@code DisplayRegisterPage} when the path starts with {@code /display/register}.
     * No authentication is required — display devices register themselves without admin credentials.
     *

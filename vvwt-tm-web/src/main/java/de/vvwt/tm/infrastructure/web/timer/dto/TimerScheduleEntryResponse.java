 ...
 * <ul>
 *   <li>{@code "ROUND"} — a match round. {@code phaseNumber} and {@code lapNumber} are set;
 *       {@code breakType} and {@code label} are {@code null}.</li>
 *   <li>{@code "BREAK"} — a break between rounds. {@code breakType} is set ({@code "REGULAR"}
 *       or {@code "ADDITIONAL"}); {@code phaseNumber}, {@code lapNumber} are {@code null}.</li>
 * </ul>
 *
 * <p>{@code startTime} and {@code endTime} are in {@code "HH:mm"} format when
 * {@code tournament.plannedStartTime} is set; {@code null} otherwise.
 *
 * <h2>Break type semantics (AC3)</h2>
 * <ul>
 *   <li>{@code "REGULAR"} — mapped from {@link de.vvwt.tm.domain.timeline.TimelineEntryType#LAP_BREAK};
 *       triggers pause music on the timer device (D-8)</li>
 *   <li>{@code "ADDITIONAL"} — mapped from {@code INTRA_PHASE_BREAK} or {@code SECTION_BREAK};
 *       does NOT trigger pause music</li>
 * </ul>
 *
...
public class TimerScheduleEntryResponse {
    ...
    /**
     * Optional display label for the break (e.g., "Mittagspause").
     * Present only for {@code INTRA_PHASE_BREAK} entries that carry an organiser label.
     */
    ...
    /**
     * Wall-clock start time in {@code "HH:mm"} format.
     * {@code null} if the tournament has no {@code plannedStartTime}.
     */
    private String startTime;

    /**
     * Wall-clock end time in {@code "HH:mm"} format.
     * {@code null} if the tournament has no {@code plannedStartTime}.
     */
     ...
     */
    public static TimerScheduleEntryResponse round(int phaseNumber, int lapNumber,
     ...
     */
    public static TimerScheduleEntryResponse breakEntry(String breakType, String label,
                                                         String startTime, String endTime) {
        TimerScheduleEntryResponse entry = new TimerScheduleEntryResponse();
        entry.type = "BREAK";

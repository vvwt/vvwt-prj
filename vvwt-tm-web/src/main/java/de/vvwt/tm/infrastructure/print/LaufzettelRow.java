 ...
 *
 * <p>This POJO is passed to jmustache as a list element. jmustache accesses each field by name
 * (getter-based), so standard JavaBean getters are required.
 *
 * <p>Each row represents one of the following:
 *
 * <ul>
 *   <li>A phase header ({@link #isPhaseHeader} = true) — separates phases in multi-phase schedules
 *   <li>A break separator ({@link #isBreak} = true) — intra-phase break or section break
 *   <li>A match round row — exactly one of {@link #isPlaying}, {@link #isRefereeing}, {@link
 *       #isActivity}, or {@link #isFree} is true
 * </ul>
 *
 * <h2>Round state priority (AC6 — E08S08)</h2>
 *
 * <p>The assembler determines each team's state per round following: PLAYING &gt; REFEREEING &gt;
 * ACTIVITY &gt; FREE. By E03S10 / E08S04 design, PLAYING and REFEREEING are mutually exclusive, and
 * ACTIVITY is only assigned to free rounds, so this priority is a safety-net, not a common
 * disambiguation.
 *
 ...
 * prevents {@code MustacheException} caused by absent keys in strict mode (documented in E08S07 QA
 * report D2).
 *
...
public class LaufzettelRow {
    ...
    /**
     * Formatted time window for this round (e.g., "10:00–10:15"). Empty string if the tournament
     * has no {@code plannedStartTime} (AC10).
     */
    ...
    /**
     * Field number as a display string. Non-empty only when {@code isPlaying} or {@code
     * isRefereeing} (AC4, AC5).
     */
    ...
    public boolean isPhaseHeader() {
        return isPhaseHeader;
    }

    public String getPhaseHeaderName() {
        return phaseHeaderName;
    }

 ...
 *
 * <p>E08S05: the {@code timeline} field is added. When the tournament has a
 * {@code plannedStartTime} set, this list contains predicted clock-time entries for every
 * match round, lap break, intra-phase break, and section break. When {@code plannedStartTime}
 * is {@code null}, the list is empty — backward compatible with E05S06 clients.
 *
...
public record DraftPreviewResponse(
        List<DraftPreviewSectionResponse> sections,
        List<DraftTimelineEntryResponse> timeline
) {

    /**
     * Maps a list of {@link DraftPreviewSection} domain objects (plus optional timeline entries)
     * to this response DTO.
     *
    ...
    public static DraftPreviewResponse from(List<DraftPreviewSection> previews,
        ...
        return new DraftPreviewResponse(
                previews.stream()
                        ...
                        .toList(),
                timeline.stream()
                        .map(DraftTimelineEntryResponse::from)
                        .toList()
        );
    }
}

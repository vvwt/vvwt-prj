...
/**
 * Unit tests for {@link DeviceLimitErrorResponse} JSON shape (E21S06, AC-TDD-DeviceLimitErrorResponse).
 *
 * <h2>RED state</h2>
 *
 * <p>This test was committed RED: {@link DeviceLimitErrorResponse} at {@code
 * de.vvwt.tm.tournament.internal.DeviceLimitErrorResponse} did not exist at commit time — satisfying
 * the DEC-22 Iron Law.
 *
 * <h2>Coverage</h2>
 *
...
class DeviceLimitErrorResponseTest {
    ...
    void serializes_errorCode_configuredLimit_currentCount() throws Exception {
        DeviceLimitErrorResponse response = new DeviceLimitErrorResponse("DEVICE_LIMIT_EXCEEDED", 10, 10L);

        String json = mapper.writeValueAsString(response);
        JsonNode node = mapper.readTree(json);

        assertThat(node.has("errorCode"))
                .as("JSON must contain errorCode field")
                .isTrue();
        ...
        assertThat(node.has("currentCount"))
                .as("JSON must contain currentCount field")
                .isTrue();
    ...
    void gettersReturnCorrectValues() {
        DeviceLimitErrorResponse response = new DeviceLimitErrorResponse("DEVICE_LIMIT_EXCEEDED", 5, 6L);

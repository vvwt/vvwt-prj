 ...
 * assertions, setup, and fixture data. Scenarios mined from deleted legacy
 * {@code LaufzettelAssemblerTest} as BLACK-BOX reference only — no copy-paste per DEC-41 §3
 * strict.
 *
 * <p>Per DEC-36 same-package rule: this test lives at {@code de.vvwt.tm.print.internal}
 * (same package as {@link DefaultLaufzettelAssembler}) and MAY type-reference the concrete
 * class directly (white-box carve-out). The subject field is declared as {@link LaufzettelAssembler}
 * interface for DEC-36 cross-package compliance at the test declaration level; the
 * concrete {@code DefaultLaufzettelAssembler} is instantiated directly (same-package).
 *
 * <p>Per DEC-41 §3: legacy {@code LaufzettelAssemblerTest} classified as Snapshot-Driven
 * (0/4 observable criteria). Not reused as Contract Test. All scenarios re-authored fresh.
 *
 * <p>Test authored RED-first per DEC-22 Iron Law: committed before
 * {@link DefaultLaufzettelAssembler} exists.
 *
 ...
 * AC10-WITH-TIME, AC11-MULTI-PHASE, AC6-PRIORITY, EMPTY-TEAMS-EDGE, DISPLAY-NAME-FALLBACK.
 * This test covers all 12 + additional edge cases per AC-ERROR-HANDLING-COVERED-IN-FRESH-TESTS.
 */
...
class DefaultLaufzettelAssemblerTest {
    ...
    private static final UUID TOURNAMENT_ID = UUID.fromString("00000000-0000-0000-0001-000000000002");
    ...
    /** Subject declared as interface (DEC-36 test-declaration level); instantiated as concrete (white-box). */
    private LaufzettelAssembler assembler;

    @BeforeEach
    void setUp() {
        team1 = new Team(TEAM1_ID, TENANT_ID, TOURNAMENT_ID, 1, "Rote Wölfe", true, false, false, null);
        team2 = new Team(TEAM2_ID, TENANT_ID, TOURNAMENT_ID, 2, "Blaue Haie", true, false, false, null);
        team3 = new Team(TEAM3_ID, TENANT_ID, TOURNAMENT_ID, 3, null /* null description */, true, false, false, null);

        phase1 = new Phase(PHASE_ID, TENANT_ID, TOURNAMENT_ID, 1, "Vorrunde", "ACTIVE", 0, null);

        avatar1 = new TeamAvatar(AVATAR1_ID, TENANT_ID, TOURNAMENT_ID, PHASE_ID, 1, 1, TEAM1_ID, null, null);
        avatar2 = new TeamAvatar(AVATAR2_ID, TENANT_ID, TOURNAMENT_ID, PHASE_ID, 1, 2, TEAM2_ID, null, null);
        avatar3 = new TeamAvatar(AVATAR3_ID, TENANT_ID, TOURNAMENT_ID, PHASE_ID, 1, 3, TEAM3_ID, null, null);

        // lap=1, field=2, no referee, team1 vs team2
        matchLap1 = new Match(MATCH_ID, TENANT_ID, TOURNAMENT_ID, PHASE_ID,
                AVATAR1_ID, AVATAR2_ID, 0, 1, 1 /* lapNumber */, 2 /* fieldNumber */, null, null, null, null);
        ...
        when(activityService.assignActivities(any(), any(), any(), anyInt(), any()))
                .thenReturn(new ActivityAssignmentResult(Collections.emptyMap(), Collections.emptyMap()));
    ...
    @Test
    @DisplayName("S1-AC4: team playing in a round produces a PLAYING row with opponent name and field number")
    void playing_rowHasOpponentAndField() {
        Tournament tournament = noTimeT();
        var result = assembler.assemble(tournament, phases(phase1), teams(team1, team2),
                avatars(PHASE_ID, avatar1, avatar2), matches(PHASE_ID, matchLap1),
    ...
    void refereeing_rowHasFieldNumber() {
        Tournament tournament = noTimeT();
        Match matchWithRef = new Match(MATCH_ID, TENANT_ID, TOURNAMENT_ID, PHASE_ID,
                AVATAR1_ID, AVATAR2_ID, 0, 1, 1, 3 /* field 3 */,
                TEAM3_ID /* referee */, null, null, null);

        var result = assembler.assemble(tournament, phases(phase1), teams(team1, team2, team3),
    ...
    void activity_rowHasActivityName() {
        ...
        when(activityService.assignActivities(any(), any(), any(), anyInt(), any()))
                .thenReturn(new ActivityAssignmentResult(
                        Map.of(actType, List.of(new ActivityAssignment(TEAM3_ID, 1, "Mannschaftsfoto"))),
                        Collections.emptyMap()));

        var result = assembler.assemble(tournament, phases(phase1), teams(team1, team2, team3),
                avatars(PHASE_ID, avatar1, avatar2, avatar3),
                matches(PHASE_ID, matchLap1),
                Collections.emptyMap(), List.of(actType), 0);
    ...
    @Test
    @DisplayName("S5-AC8: team not playing, refereeing, or assigned an activity produces a FREE row")
    void free_rowWhenNothingAssigned() {
        ...
        List<LaufzettelRow> rows = result.get(TEAM3_ID);
        assertThat(rows).hasSize(1);
        assertThat(rows.get(0).isFree()).as("team3 has no match/ref/activity — must be FREE").isTrue();
    ...
    void break_separatorRowAppeared() {
        ...
        Match matchLap2 = new Match(UUID.randomUUID(), TENANT_ID, TOURNAMENT_ID, PHASE_ID,
                AVATAR1_ID, AVATAR2_ID, 0, 1, 2 /* lap 2 */, 1, null, null, null, null);

        // Timeline: MATCH_ROUND(phase=1, lap=1, 10:00–10:15), INTRA_PHASE_BREAK(label=Mittagspause, 10:15–10:30), MATCH_ROUND(phase=1, lap=2, 10:30–10:45)
        when(timelineService.calculate(any(), any(), anyInt())).thenReturn(List.of(
                new TimelineEntry(1, 1, TimelineEntryType.MATCH_ROUND,
                        LocalTime.of(10, 0), LocalTime.of(10, 15), null),
                new TimelineEntry(1, 0, TimelineEntryType.INTRA_PHASE_BREAK,
                        LocalTime.of(10, 15), LocalTime.of(10, 30), "Mittagspause"),
                new TimelineEntry(1, 2, TimelineEntryType.MATCH_ROUND,
                        LocalTime.of(10, 30), LocalTime.of(10, 45), null)
        ));
        ...
        assertThat(rows.get(1).breakTimeWindow()).as("break time window non-empty with start time").isNotEmpty();
    ...
    @Test
    @DisplayName("S7-AC10: tournament without plannedStartTime produces rows with empty time windows")
    ...
    @Test
    @DisplayName("S8-AC10: tournament with plannedStartTime produces rows with non-empty time windows")
    ...
    void multiPhase_hasPhaseHeaderRows() {
        ...
        Phase phase2 = new Phase(phase2Id, TENANT_ID, TOURNAMENT_ID, 2, "Finale", "PENDING", 0, null);
        ...
        TeamAvatar avP2T1 = new TeamAvatar(av1p2, TENANT_ID, TOURNAMENT_ID, phase2Id, 1, 1, TEAM1_ID, null, null);
        TeamAvatar avP2T2 = new TeamAvatar(av2p2, TENANT_ID, TOURNAMENT_ID, phase2Id, 1, 2, TEAM2_ID, null, null);

        Match matchP2 = new Match(UUID.randomUUID(), TENANT_ID, TOURNAMENT_ID, phase2Id,
                av1p2, av2p2, 0, 1, 1, 1, null, null, null, null);

        var result = assembler.assemble(tournament, phases(phase1, phase2), teams(team1, team2),
                Map.of(PHASE_ID, List.of(avatar1, avatar2), phase2Id, List.of(avP2T1, avP2T2)),
    ...
    void priority_playingOverRefereeing() {
        ...
        Match matchSelfRef = new Match(MATCH_ID, TENANT_ID, TOURNAMENT_ID, PHASE_ID,
                AVATAR1_ID, AVATAR2_ID, 0, 1, 1, 2,
                TEAM1_ID /* team1 self-referee */, null, null, null);
    ...
    void emptyTeams_returnsEmptyResult() {
        Tournament tournament = noTimeT();
        var result = assembler.assemble(tournament, phases(phase1), Collections.emptyList(),
    ...
    @Test
    @DisplayName("S12-EDGE: null description falls back to 'Team N'; named description used verbatim")
    void teamDisplayName_nullFallsBackToTeamNumber() {
        ...
        TeamAvatar avT3 = new TeamAvatar(avA, TENANT_ID, TOURNAMENT_ID, PHASE_ID, 1, 1, TEAM3_ID, null, null);
        TeamAvatar avT1 = new TeamAvatar(avB, TENANT_ID, TOURNAMENT_ID, PHASE_ID, 1, 2, TEAM1_ID, null, null);
        Match m = new Match(UUID.randomUUID(), TENANT_ID, TOURNAMENT_ID, PHASE_ID,
                avA, avB, 0, 1, 1, 1, null, null, null, null);

        var result = assembler.assemble(tournament, phases(phase1), teams(team3, team1),
                avatars(PHASE_ID, avT3, avT1), matches(PHASE_ID, m),
                Collections.emptyMap(), Collections.emptyList(), 0);
    ...
    void emptyPhases_returnsEmptyResult() {
        Tournament tournament = noTimeT();
        var result = assembler.assemble(tournament, Collections.emptyList(), teams(team1, team2),
                Collections.emptyMap(), Collections.emptyMap(),
                Collections.emptyMap(), Collections.emptyList(), 0);
        assertThat(result).isEmpty();
    }
    ...
    void lapBreak_skipped() {
        ...
        when(timelineService.calculate(any(), any(), anyInt())).thenReturn(List.of(
                new TimelineEntry(1, 1, TimelineEntryType.MATCH_ROUND,
                        LocalTime.of(9, 0), LocalTime.of(9, 15), null),
                new TimelineEntry(1, 0, TimelineEntryType.LAP_BREAK,
        ...
        Match matchLap2 = new Match(UUID.randomUUID(), TENANT_ID, TOURNAMENT_ID, PHASE_ID,
                AVATAR1_ID, AVATAR2_ID, 0, 1, 2, 1, null, null, null, null);
    ...
    void assembleWithPhaseConfig_lapTimeOverrides() {
        ...
        var result = assembler.assembleWithPhaseConfig(
                tournament, phases(phase1), teams(team1, team2),
                avatars(PHASE_ID, avatar1, avatar2), matches(PHASE_ID, matchLap1),
                Collections.emptyMap(), Collections.emptyList(), 0,
    ...
    @Test
    @DisplayName("EDGE: single-phase tournament produces no phase header rows (header only in multi-phase)")
    void singlePhase_noPhaseHeaderRow() {
        ...
        List<LaufzettelRow> rows = result.get(TEAM1_ID);
        assertThat(rows.stream().noneMatch(LaufzettelRow::isPhaseHeader)).as("no phase headers in single-phase").isTrue();
    }
    ...
    void sectionBreak_producesBreakRow() {
        ...
        when(timelineService.calculate(any(), any(), anyInt())).thenReturn(List.of(
                new TimelineEntry(1, 1, TimelineEntryType.MATCH_ROUND,
                        LocalTime.of(9, 0), LocalTime.of(9, 15), null),
                new TimelineEntry(1, 0, TimelineEntryType.SECTION_BREAK,
                        LocalTime.of(9, 15), LocalTime.of(9, 25), null),
                new TimelineEntry(2, 1, TimelineEntryType.MATCH_ROUND,
                        LocalTime.of(9, 25), LocalTime.of(9, 40), null)
        ));
        ...
        List<LaufzettelRow> rows = result.get(TEAM1_ID);
        // Expected: phase header (multi-phase), PLAYING(p1-lap1), SECTION_BREAK, phase header, PLAYING(p2-lap1)
    ...
    private Tournament noTimeT() {
        return new Tournament(TOURNAMENT_ID, TENANT_ID, "Test", "BEST_OF_3",
                "setPoints", "standardVolleyball", "roundRobin", "ACTIVE",
                null, null, 4, 8, null);
    }

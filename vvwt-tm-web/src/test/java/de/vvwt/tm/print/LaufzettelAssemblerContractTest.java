 ...
 * {@code de.vvwt.tm.print} (different Java package from the implementation at {@code
 * de.vvwt.tm.print.internal}) and therefore MUST reference {@link LaufzettelAssembler} interface
 * type only — NEVER the concrete {@code DefaultLaufzettelAssembler} class.
 *
...
class LaufzettelAssemblerContractTest {
     ...
     *
     * <p>This test does not execute any logic — it is a compile-time signature assertion. A {@code
     * null} reference is used to avoid instantiating the implementation class (cross-package rule:
     * interface-typed reference only per DEC-36).
     */
    @Test
    ...
    void hasTimeMethodSignatureIsCorrect() {
        LaufzettelAssembler assembler = null;
        Assertions.assertThrows(
                NullPointerException.class, () -> assembler.hasTime((Tournament) null));
    }
}

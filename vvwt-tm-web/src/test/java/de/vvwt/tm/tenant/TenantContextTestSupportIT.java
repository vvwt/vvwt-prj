 ...
 *
 * <p>This test class was committed BEFORE {@link TenantContextTestSupport} was implemented. In the
 * RED state the class reference does not compile (compilation failure = valid RED indicator per
 * DEC-22 Iron Law — the test cannot pass before the implementation exists).
 *
 * <h2>Test coverage</h2>
 *
 * <ul>
 *   <li>AC1 — test-first: this file is committed before the implementation (RED)
 *   <li>AC6 — {@code TenantContext.current()} returns a non-null UUID equal to the default-tenant
 *       UUID
 *   <li>AC9 — back-to-back cleanup: second test asserts clean re-bind, no leaked state from first
 *   <li>AC3 — nested bind: inner {@code bind()} overrides outer; closing inner scope restores outer
 * </ul>
 *
...
class TenantContextTestSupportIT {
    ...
    @Autowired private TenantContextTestSupport.Binder binder;
    ...
    /**
     * AC6 — Within a {@code @SpringBootTest} method body, {@code TenantContext.current()} returns a
     * non-null UUID matching the default-tenant UUID resolved from the E14S05 bootstrap.
     */
    @Test
    void currentReturnsDefaultTenantUuidAfterAutoBind() {
        ...
        try {
            assertThat(tenantContext.current())
                    .as(
                            "TenantContext.current() must return the default-tenant UUID after"
                                    + " auto-bind")
                    .isNotNull()
                    .isEqualTo(boundId);
        } finally {
    ...
    /**
     * AC9 (second method of back-to-back pair) — after the first method released the binding, a
     * fresh bind succeeds and returns the same default-tenant UUID (no leaked state).
     */
    @Test
     ...
     * for its scope only; closing the inner scope restores the outer tenant (AC-NESTED-BIND from
     * E14S01).
     */
    @Test
    @SuppressWarnings(

 ...
 * UnsupportedOperationException} — the cancel path wires to {@link CancelFlagRegistry} which fails
 * fast.
 *
 ...
 * </ul>
 *
 * <p>Uses {@code tm.slotopt.exhaustive-max-n=2} so a 4-team roundRobin phase (3 laps) routes to Leg
 * 3 (cancelable in-process). The cancel signal is issued via {@link CancelFlagRegistry} while
 * drainNext() blocks in L3.
 *
 * <p>The drain runs in a background thread that explicitly binds the tenant context (same pattern
 * as {@link OrchestratorDec37LockIT}) — required because the routing DataSource uses a thread-local
 * tenant context, and spawned threads do not inherit the caller's thread-local.
 *
...
class CancelBsfInvariantIT {
    ...
    @Autowired private SlotOptimizationJobRegistry slotOptJobRegistry;
    ...
    void setUp() {
        ...
        jdbcTemplate.update(
                ...
                "DRAFT",
                LocalDateTime.now(),
                4, // 4 fields (8 teams / 2 per match = 4 matches per lap)
                8, // 8 teams → 7 laps → 7!=5040 perms > exhaustive-max-n=2 → routes to Leg 3
                true);
     ...
     *
     * <p>Strategy: start drainNext() in a background thread that explicitly binds the tenant
     * context (DEC-20 routing DataSource requires a thread-local tenant entry — spawned threads do
     * not inherit the caller's thread-local, same pattern as {@link OrchestratorDec37LockIT}).
     * After a short delay (giving L3 time to start), signal cancel via {@link
     * CancelFlagRegistry#requestCancel(UUID)} and also set {@code cancelled=TRUE} on the job row.
     * The L3 loop observes the cancellation token and applies BSF. The orchestrator step-B
     * completes normally.
     */
    @Test
    @DisplayName(
    ...
    void cancelMidL3_bsfApplied_phaseOptimized_jobCompleted() throws Exception {
        ...
        // Start drain in background — binds tenant context for the DataSource routing (DEC-20)
        Future<?> drainFuture =
                executor.submit(
                        () -> {
        ...
        // Wait for drain to start, then wait until L3 registers a JobHandle (proving L3 has begun)
        assertThat(drainStarted.await(5, TimeUnit.SECONDS))
                .as("drain must start within 5s")
                .isTrue();

        // Poll until the SlotOptimizationJobRegistry has a handle for this tournament, which means
        // RoutingSlotOptimizationClient.executeLeg3() has started and the CancellationToken exists.
        long deadline = System.currentTimeMillis() + 10_000;
        while (slotOptJobRegistry.getHandle(tournamentId).isEmpty()
                && System.currentTimeMillis() < deadline) {
            Thread.sleep(5);
        }
        assertThat(slotOptJobRegistry.getHandle(tournamentId))
                .as("Leg 3 JobHandle must be registered within 10s")
                .isPresent();

        // Signal cancel — mirrors what SlotOptimizationCancelController does (DEC-64 D-10):
        // 1. DB persistence: set cancelled=TRUE on the RUNNING row
        jobRepository
                .findRunningJobIdForTournament(tournamentId)
                .ifPresent(jobId -> jobRepository.markCancelled(jobId));
        // 2. In-memory flag: signal orchestrator step-B clear-on-completion path
        cancelFlagRegistry.requestCancel(tournamentId);
        // 3. CancellationToken: signal L3 permutation loop to stop and apply BSF
        slotOptJobRegistry
                .getHandle(tournamentId)
                .ifPresent(handle -> handle.getCancellationToken().cancel());
        ...
        Integer nullLapCount =
                jdbcTemplate.queryForObject(
                        ...
                        Integer.class,
                        phaseId);
        assertThat(nullLapCount).as("all matches must have lap_number (BSF applied)").isEqualTo(0);

 ...
 *
 * <ul>
 *   <li>The {@code devices} table exists after migration
 *   <li>All expected columns are present with correct types and constraints
 *   <li>Default value for {@code device_type} is {@code SCORING_TABLET}
 *   <li>Default value for {@code status} is {@code REGISTERED}
 *   <li>Unique constraint on {@code device_token} is present
 *   <li>Unique constraint on {@code (tenant_id, pin)} is present
 * </ul>
 *
 * @see <a href="../../../../../.gaai/project/contexts/artefacts/stories/E06S03.story.md">Story
 *     E06S03</a>
 */
@SpringBootTest(
...
class E06S03MigrationIT {
        ...
        Long count =
                jdbcTemplate.queryForObject(
                        "SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME ="
                                + " 'DEVICES'",
                        Long.class);
        assertThat(count).as("AC1 — devices table must exist after V8 migration").isEqualTo(1L);
    }
    ...
    void devicesTableHasAllExpectedColumns() {
        ...
        assertThat(columnNames)
                .as(
                        "AC1 — devices table column set (includes E07S01 additions: device_name,"
                                + " configuration)")
                .containsExactlyInAnyOrder(
                        ...
                        "last_seen_at",
                        "device_name",
                        "configuration"); // E07S01 AC1 additions
    }
    ...
    void deviceTypeDefaultIsScoringTablet() {
        ...
        jdbcTemplate.update(
                "INSERT INTO tenants (id, display_name, tenant_location_count, is_default,"
                        + " created_at) VALUES (?, 'Test', 1, FALSE, CURRENT_TIMESTAMP)",
                tenantId);
        jdbcTemplate.update(
                "INSERT INTO locations (id, tenant_id, display_name, created_at)"
                        + " VALUES (?, ?, 'Loc', CURRENT_TIMESTAMP)",
                locationId,
                tenantId);
    ...
    void deviceTokenUniqueConstraintEnforced() {
        ...
        jdbcTemplate.update(
                "INSERT INTO tenants (id, display_name, tenant_location_count, is_default,"
                        + " created_at) VALUES (?, 'Test2', 1, FALSE, CURRENT_TIMESTAMP)",
                tenantId);
        jdbcTemplate.update(
                "INSERT INTO locations (id, tenant_id, display_name, created_at)"
                        + " VALUES (?, ?, 'Loc2', CURRENT_TIMESTAMP)",
                locationId,
                tenantId);
    ...
    void tenantPinUniqueConstraintEnforced() {
        ...
        jdbcTemplate.update(
                "INSERT INTO tenants (id, display_name, tenant_location_count, is_default,"
                        + " created_at) VALUES (?, 'Test3', 1, FALSE, CURRENT_TIMESTAMP)",
                tenantId);
        jdbcTemplate.update(
                "INSERT INTO locations (id, tenant_id, display_name, created_at)"
                        + " VALUES (?, ?, 'Loc3', CURRENT_TIMESTAMP)",
                locationId,
                tenantId);

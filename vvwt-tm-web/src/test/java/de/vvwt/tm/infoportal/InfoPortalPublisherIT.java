 ...
 *
 * <p><b>PRECONDITION:</b> Requires Docker to be available (for Testcontainers) AND the
 * {@code vvwt-info-server} fat-JAR to be built (via
 * {@code mvn package -pl vvwt-info-server -DskipTests}).
 *
 ...
 * </ol>
 *
 * <p>Enable by removing {@code @Disabled} and running in a Docker-enabled environment:
 * {@code mvn test -pl vvwt-tm-web -Dgroups=testcontainers}
 *
 ...
 *
 * @see <a href="../../../../../../../../.gaai/project/contexts/artefacts/stories/E38S09.story.md">E38S09 AC2, AC3, AC13</a>
 */
@Tag("testcontainers")
...
class InfoPortalPublisherIT {
    ...
    /**
     * AC2 happy-path:
     * (a) Ed25519 keypair already generated in setUp
     * (b) POST /api/v1/register UNSIGNED → 200 + algorithm_warning (or null)
     * (c) POST /api/v1/tournaments/test-tenant/test-venue/t1/register SIGNED → 200 +
     *     tournament_token + per_tournament_secret
     * (d) POST /api/v1/publish/test-tenant/test-venue/t1?seq=1 SIGNED × 3 → all 200
     * Verified: stateDao.findCurrentSeq(...) = 3; publisher status is healthy
     */
    @Test
    ...
    /**
     * AC3 recovery path:
     * (a) 3 deltas published successfully
     * (b) info-server container stopped and restarted (simulates restart/loss)
     * (c) 4th delta → 409 FULL_RESYNC
     * (d) publisher posts snapshot with seq=3 (NOT reset to 1)
     * (e) 5th delta with seq=4 → 200
     * Verified: no queue involved; snapshot carries seq=3
     */
    @Test
    ...
    /**
     * AC13 idempotency:
     * (a) Publish delta with seq=1 → 200
     * (b) Retry same publish (seq=1 again) → info-server responds 409 (seq already applied)
     * (c) Publisher snapshot-recovers without advancing last_published_seq further
     * Verified: last_published_seq unchanged after idempotent retry
     */
    @Test

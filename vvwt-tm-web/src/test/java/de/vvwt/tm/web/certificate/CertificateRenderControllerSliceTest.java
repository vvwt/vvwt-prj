 ...
 *   <li>No-template single → 200 system-default HTML (AC-SLICE-NO-TEMPLATE-200-SINGLE, E67S01 AC1)
 *   <li>No-template batch → 200 system-default HTML (AC-SLICE-NO-TEMPLATE-200-BATCH, E67S01 AC2)
 *   <li>HTML-template → 200 system-default HTML (AC-SLICE-HTML-TEMPLATE-200, E67S01 AC4)
 *   <li>No game results → 400 (AC-SLICE-NO-RESULTS-400, E67S01 AC5/AC8)
...
@DisplayName("CertificateRenderController @WebMvcTest slice — E24S05 / E67S01")
class CertificateRenderControllerSliceTest {
    ...
    /** Standard tom_label_* stub map — reused by every HTML-path test. */
    private Map<String, Object> tomLabelStubMap;
    ...
    void setUp() {
        ...
        // D-17 hard-cut). The convenience boolean tom_has_photo is the 14th entry.
        // Map.ofEntries used because 13 entries exceed the Map.of() 10-entry limit.
        tomLabelStubMap =
                Map.ofEntries(
                        Map.entry("tom_placement", "1"),
                        Map.entry("tom_team_name", "Test Team"),
                        Map.entry("tom_team_photo", "/api/photo/teams/test"),
                        Map.entry("tom_tournament_name", "Test Tournament"),
                        Map.entry("tom_date", "2026-04-26"),
                        Map.entry("tom_location", "Test Location"),
                        Map.entry("tom_organizer", "Test Organizer"),
                        ...
                        Map.entry("tom_label_generated_by", "generated by"),
    ...
    // =========================================================================
    // AC-SLICE-SVG-BRANCH — SVG single certificate (AC3: SVG path unchanged)
    // =========================================================================
    ...
    // =========================================================================
    // AC-SLICE-ZIP-BRANCH — ZIP batch certificates (SVG format, AC3: SVG path unchanged)
    // =========================================================================
    ...
    // =========================================================================
    // AC-SLICE-NO-TEMPLATE-200-SINGLE — E67S01 AC1/AC7
    // No template uploaded → HTTP 200, system-default certificate/print view.
    // RED-first: this test FAILed against pre-story code (returned 400 for no-template).
    // =========================================================================

    @Test
    @WithMockUser
    @DisplayName(
            "AC-SLICE-NO-TEMPLATE-200-SINGLE (E67S01 AC1): no template uploaded → 200"
                    + " certificate/print view with singleCertificate model attribute")
    void singleCertificate_noTemplate_returns200SystemDefault() throws Exception {
        when(tournamentRepository.findById(TOURNAMENT_ID)).thenReturn(Optional.of(tournament));
    ...
    // =========================================================================
    // AC-SLICE-NO-TEMPLATE-200-BATCH — E67S01 AC2/AC7
    // No template uploaded, batch → HTTP 200, system-default certificate/print-all view.
    // RED-first: this test FAILed against pre-story code (returned 400 for no-template).
    // =========================================================================
    ...
    @WithMockUser
    @DisplayName(
            "AC-SLICE-NO-TEMPLATE-200-BATCH (E67S01 AC2): no template uploaded, batch → 200"
                    + " certificate/print-all view with certificates model attribute")
    void allCertificates_noTemplate_returns200SystemDefault() throws Exception {
        when(tournamentRepository.findById(TOURNAMENT_ID)).thenReturn(Optional.of(tournament));
        when(certificateTemplateService.retrieveFile(TOURNAMENT_ID)).thenReturn(Optional.empty());
        when(certificateAssembler.getFinalPhase(TOURNAMENT_ID)).thenReturn(Optional.of(phase));
        when(certificateAssembler.computePlacementOrder(eq(TOURNAMENT_ID), any()))
    ...
    // =========================================================================
    // AC-SLICE-HTML-TEMPLATE-200 — E67S01 AC4
    // HTML template uploaded → same as no-template path (200 system-default).
    // =========================================================================

    @Test
    @WithMockUser
    @DisplayName(
            "AC-SLICE-HTML-TEMPLATE-200 (E67S01 AC4): HTML template uploaded → 200 system-default"
                    + " certificate/print view (HTML template content ignored)")
    void singleCertificate_htmlTemplateUploaded_returns200SystemDefault() throws Exception {
        byte[] htmlContent = "<html>custom</html>".getBytes(StandardCharsets.UTF_8);
        CertificateTemplateService.TemplateFile templateFile =
                new CertificateTemplateService.TemplateFile(
                        new ByteArrayInputStream(htmlContent), "text/html", htmlMetadata);
    ...
    // =========================================================================
    // AC-SLICE-NO-RESULTS-400 — E67S01 AC5/AC8
    // No game results (getFinalPhase returns empty) → 400 text/plain.
    // The no-results 400 branch is PRESERVED (AC5). This test covers the retained
    // 400 error path so AC8's "a slice case for a still-400 branch" requirement is met.
    // =========================================================================

    @Test
    @WithMockUser
    @DisplayName(
            "AC-SLICE-NO-RESULTS-400 (E67S01 AC5/AC8): no final phase → 400 text/plain"
                    + " (no-results 400 branch preserved unchanged)")
    void singleCertificate_noFinalPhase_returns400Plaintext() throws Exception {
        when(tournamentRepository.findById(TOURNAMENT_ID)).thenReturn(Optional.of(tournament));
        when(certificateTemplateService.retrieveFile(TOURNAMENT_ID)).thenReturn(Optional.empty());
        when(certificateAssembler.getFinalPhase(TOURNAMENT_ID)).thenReturn(Optional.empty());
        when(tenantContext.current()).thenReturn(UUID.randomUUID());
    ...
    // =========================================================================
    // AC-SLICE-MUSTACHE-500-BRANCH — 500 plaintext on MustacheException (SVG path)
    // =========================================================================

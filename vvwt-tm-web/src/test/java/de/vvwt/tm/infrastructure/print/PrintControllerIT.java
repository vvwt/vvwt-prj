...
/**
 * Integration tests for {@link PrintController} — E08S07: Print route skeleton + Mustache layout + CSS.
 *
 * <p>Tests the full HTTP stack to verify:
 * <ul>
 *   <li>AC1: {@code /print/**} routes are served by this controller alongside other routes</li>
 *   <li>AC2: {@code /print/**} routes require basic auth (401 without credentials)</li>
 *   <li>AC6: Print static assets ({@code /print/assets/print.css}) are served correctly</li>
 *   <li>AC7: Non-existent tournament → 404; tournament with no phases → human-readable HTML</li>
 *   <li>AC9: Tournament resolved via tenant-scoped repository (cross-tenant 404)</li>
 * </ul>
 *
 * <h2>Test strategy</h2>
 * <p>The test context uses an isolated in-memory H2 database. Tournament records are created
 * via the {@code POST /api/tournaments} endpoint using authenticated requests. This ensures
 * the full tenant-scoped data path is exercised (including {@code TenantContext} population by
 * the default-tenant resolver).
 *
 * @see PrintController
 * @see de.vvwt.tm.auth.SecurityConfig
 * @see <a href="../../../../../../../../.gaai/project/contexts/artefacts/stories/E08S07.story.md">Story E08S07</a>
 */
@SpringBootTest(
...
class PrintControllerIT {
    ...
    void printIndexRequiresAuthentication() throws Exception {
        ...
        ResponseEntity<String> response = restTemplate.getForEntity(
                new URI(baseUrl + "/print/" + randomId),
                String.class);

        assertThat(response.getStatusCode())
                .as("AC2: /print/** must require authentication — unauthenticated request returns 401")
    ...
    void printIndexReturns404ForUnknownTournament() throws Exception {
        ...
        ResponseEntity<String> response = authed.getForEntity(
                new URI(baseUrl + "/print/" + unknownId),
                String.class);
    ...
    @Test
    @DisplayName("AC7: GET /print/{id} for draft tournament returns human-readable HTML error, not JSON")
    void printIndexReturnsMustacheErrorForDraftTournament() throws Exception {
        ...
        assertThat(response.getStatusCode())
                .as("AC7: Draft tournament (no phases) must return 200 with human-readable error page")
    ...
    void printIndexRouteIsServedByController() throws Exception {
        // Create a tournament — controller must respond (404 from controller, not from 'route not found')
        ...
        assertThat(response.getStatusCode())
                .as("AC1: /print/** route must be handled by PrintController (returns 200 with Mustache page)")
                .isEqualTo(HttpStatus.OK);
    ...
    void printCssIsPubliclyAccessible() throws Exception {
        ResponseEntity<String> response = restTemplate.getForEntity(
                new URI(baseUrl + "/print/assets/print.css"),
                String.class);
    ...
    void printIndexReturns404ForRandomUuid() throws Exception {
        ...
        ResponseEntity<String> response = authed.getForEntity(
                new URI(baseUrl + "/print/" + neverCreated),
                String.class);
     ...
     *
     * <p>The created tournament has status {@code DRAFT} and no phases. Used to test
     * the draft/no-phases error path (AC7) and the general route existence check (AC1).
     *

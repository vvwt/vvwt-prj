 ...
 *
 * <p>This class is TEST-SCOPE ONLY ({@code src/test/java/}). It is not a production DAO. Its sole
 * purpose is to serve as the subject under test in {@link MarkerDaoTest}, showing that {@link
 * TenantDaoTestSupport} makes all three DEC-26 rules the natural default for DAO integration tests.
 *
 * <p>Implementation is intentionally minimal: a single {@code insert} and a single {@code findAll}.
 * This is enough to demonstrate the three-rule pattern end-to-end.
 *
...
class MarkerDao {
    ...
    void insert(String id, String note) {
        try (var conn = dataSource.getConnection();
                var ps = conn.prepareStatement("INSERT INTO marker (id, note) VALUES (?, ?)")) {
            ps.setString(1, id);
            ps.setString(2, note);
            ps.executeUpdate();
        } catch (Exception e) {

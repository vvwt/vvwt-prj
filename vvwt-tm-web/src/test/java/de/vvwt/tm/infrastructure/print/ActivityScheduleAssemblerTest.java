 ...
 * <p>Covers:
 * <ul>
 *   <li>AC3: Happy-path row assembly — rounds ordered by lap number, empty rounds omitted</li>
 *   <li>AC4: Break separator rows interleaved from timeline</li>
 *   <li>AC5: Summary counts (totalAssignedTeams, roundCount)</li>
 *   <li>AC6: Unassigned team names collected</li>
 *   <li>AC7: No-start-time path — time windows empty, no breaks emitted</li>
 * </ul>
 *
 * <p>The {@link TimelineCalculationService} is used as a real instance (pure function).
 * The {@link ActivityAssignmentService} is mocked to control assignment outcomes.
 * The integration-level route and auth checks are in {@link PrintActivityScheduleIT}.
 *
...
class ActivityScheduleAssemblerTest {
    ...
    void setUp() {
        ...
        assembler = new ActivityScheduleAssembler(new TimelineCalculationService(), activityAssignmentService);
    }
    ...
    @Test
    @DisplayName("AC3: single assigned round produces one data row with correct lap number and team names")
    void happyPath_singleAssignedRound() {
        ...
        ActivityAssignmentResult assignResult = new ActivityAssignmentResult(
                Map.of(activityType, List.of(assignment)),
                Map.of(activityType, Set.of()));
        when(activityAssignmentService.assignActivities(any(), any(), any(), anyInt(), any()))
        ...
        ActivityScheduleModel model = assembler.assemble(
                tournament, List.of(phase1), List.of(team1, team2, team3),
                Map.of(PHASE_ID, List.of(avatar1, avatar2, avatar3)),
                Map.of(PHASE_ID, List.of(match, match2)),
                Collections.emptyMap(),
                List.of(activityType), activityType);
    ...
    void happyPath_multipleRoundsOrdered() {
        ...
        ActivityAssignmentResult assignResult = new ActivityAssignmentResult(
                Map.of(activityType, List.of(a1, a2)),
                Map.of(activityType, Set.of()));
        when(activityAssignmentService.assignActivities(any(), any(), any(), anyInt(), any()))
                .thenReturn(assignResult);

        // Matches in laps 1, 2, 3 so maxLap = 3
        Match m2 = new Match(UUID.randomUUID(), TENANT_ID, TOURNAMENT_ID, PHASE_ID,
                AVATAR1_ID, AVATAR2_ID, 0, 1, 2, 1, null, null, null, null);
        Match m3 = new Match(UUID.randomUUID(), TENANT_ID, TOURNAMENT_ID, PHASE_ID,
                AVATAR1_ID, AVATAR2_ID, 0, 1, 3, 1, null, null, null, null);
        ...
        List<ActivityScheduleRow> dataRows = model.rows().stream()
                .filter(ActivityScheduleRow::isDataRow)
                .toList();
    ...
    void emptyRoundsOmitted() {
        ...
        ActivityAssignmentResult emptyResult = new ActivityAssignmentResult(
        ...
        Tournament tournament = tournamentWithoutStartTime();
        ActivityScheduleModel model = assembler.assemble(
                tournament, List.of(phase1), List.of(team1, team2),
    ...
    void summaryCountsCorrect() {
        ...
        ActivityAssignmentResult result = new ActivityAssignmentResult(
                Map.of(activityType, List.of(a1, a2, a3)),
                Map.of(activityType, Set.of()));
        when(activityAssignmentService.assignActivities(any(), any(), any(), anyInt(), any()))
                .thenReturn(result);
    ...
    @Test
    @DisplayName("AC6: teams with no free round appear in unassignedTeamNames; hasUnassigned is true")
    ...
    void allTeamsAssigned_noWarning() {
        ActivityAssignment a1 = new ActivityAssignment(TEAM1_ID, 2, "Mannschaftsfoto");
        ActivityAssignmentResult result = new ActivityAssignmentResult(
                Map.of(activityType, List.of(a1)),
                Map.of(activityType, Set.of()));
        when(activityAssignmentService.assignActivities(any(), any(), any(), anyInt(), any()))
                .thenReturn(result);
    ...
    @Test
    @DisplayName("AC7: tournament without plannedStartTime — time windows are empty, hasTime is false")
    void noStartTime_timeWindowsEmpty() {
        ...
        List<ActivityScheduleRow> dataRows = model.rows().stream()
                .filter(ActivityScheduleRow::isDataRow).toList();
        assertThat(dataRows).hasSize(1);
        assertThat(dataRows.get(0).getTimeWindow())
                .as("AC7: time window must be empty without start time").isEmpty();
        // No break rows without timeline
        List<ActivityScheduleRow> breakRows = model.rows().stream()
                .filter(ActivityScheduleRow::isBreak).toList();
        assertThat(breakRows).as("AC7: no break rows without timeline").isEmpty();
    }

    @Test
    @DisplayName("AC7: tournament with plannedStartTime — time windows are formatted, hasTime is true")
    ...
    void emptyPhases_returnsEmptyModel() {
        Tournament tournament = tournamentWithoutStartTime();
        ActivityScheduleModel model = assembler.assemble(
                tournament, Collections.emptyList(), List.of(team1),
                Collections.emptyMap(), Collections.emptyMap(),
                Collections.emptyMap(),
                List.of(activityType), activityType);
    ...
    void emptyTeams_returnsEmptyModel() {
        Tournament tournament = tournamentWithoutStartTime();
        ActivityScheduleModel model = assembler.assemble(
                tournament, List.of(phase1), Collections.emptyList(),

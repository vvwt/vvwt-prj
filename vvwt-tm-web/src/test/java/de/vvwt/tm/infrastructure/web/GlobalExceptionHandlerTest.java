 ...
 *
 * <p>Uses a full {@link SpringBootTest} context (MOCK web env) with an extra {@link TestController}
 * to trigger each exception type. The test controller is registered via {@link
 * TestRestControllerConfig}.
 *
 * <p>Tests cover:
 *
 * <ul>
 *   <li>AC1: {@link org.springframework.web.bind.MethodArgumentNotValidException} → 400
 *   <li>AC1: {@link NoSuchElementException} → 404
 *   <li>AC1: {@link ConflictException} → 409
 *   <li>AC1: {@link Exception} → 500 without stack trace in body
 *   <li>AC9: all responses include {@code messageKey} field
 * </ul>
 *
...
class GlobalExceptionHandlerTest {
    ...
    void unexpectedException_returns500WithGenericMessage() throws Exception {
        mockMvc.perform(
                ...
                .andExpect(
                        jsonPath("$.message")
                                .value(
                                        "An unexpected error occurred. Please contact the"
                                                + " administrator."))
    ...
    /**
     * Minimal REST controller registered only during testing that deliberately throws each
     * exception type handled by {@link GlobalExceptionHandler}.
     */
    @RestController
    static class TestController {
        ...
        void unexpected() {
            throw new RuntimeException(
                    "Internal implementation detail — must not appear in response");
        }

 ...
 * <ol>
 *   <li>HTTP 200 at {@code GET /timer/{uuid}} without authentication (AC1)</li>
 *   <li>Content-Type text/html (AC1)</li>
 *   <li>Response body contains the Svelte mount point {@code <div id="app">} (AC1)</li>
 *   <li>No external CDN links in the response — DEC-15 local assets check</li>
 * </ol>
 *
 * <h2>Timer SPA assets</h2>
 * <p>The timer SPA assets are built by {@code npm run build} in the
 * {@code generate-resources} Maven phase and copied to
 * {@code target/classes/static/timer/} by {@code maven-resources-plugin}. These
...
class TimerViewControllerIT {
     ...
     * password at startup. In the test context we override with a known BCrypt hash so the
     * Spring context can start without I/O. (Timer route tests do not require authentication,
     * but the context needs a valid {@link AdminCredentialsProvider} to boot.)
     */
    @TestConfiguration
     ...
     *
     * <p>The timer route is permit-all — no authentication is required. Venue staff open
     * this URL on a device without admin credentials.
     */
    @Test
    void timerPageReturns200WithoutAuthentication() {
        ResponseEntity<String> response = restTemplate
                .getForEntity("http://localhost:" + port + "/timer/" + VALID_UUID, String.class);
    ...
    /**
     * DEC-15: The HTML shell must NOT contain external {@code https://} URLs in
     * {@code <script>} or {@code <link>} tags.
     *
     * <p>All Vite-compiled assets are served from {@code /timer/assets/} (local, bundled
     * in the JAR). Any external CDN reference would prevent offline use (DEC-15 self-host).
     */
    @Test

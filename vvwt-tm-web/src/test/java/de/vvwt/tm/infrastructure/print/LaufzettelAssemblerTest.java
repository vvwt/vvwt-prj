 ...
 *
 * <p>Covers the assembly logic for all team row states (AC4–AC8, AC10, AC11)
 * using a minimal test dataset with one tournament, one phase, two teams, and one match.
 * The {@link TimelineCalculationService} is injected as a real instance (it is a pure function
 * with no dependencies). The {@link ActivityAssignmentService} is mocked.
 *
...
class LaufzettelAssemblerTest {
    ...
    void setUp() {
        ...
        match = new Match(MATCH_ID, TENANT_ID, TOURNAMENT_ID, PHASE_ID,
                AVATAR1_ID, AVATAR2_ID,
                0 /* OPEN */, 1 /* setLimit */,
                1 /* lapNumber */, 2 /* fieldNumber */,
                null, null, null, null);
        ...
        assembler = new LaufzettelAssembler(new TimelineCalculationService(), activityAssignmentService);
    }
    ...
    @Test
    @DisplayName("AC4: team playing in a round produces a PLAYING row with opponent name and field number")
    void assemblesSingleTeamWithMatchRow() {
        ...
        Map<UUID, List<LaufzettelRow>> result = assembler.assemble(
                tournament, phases, teams,
                avatarsByPhase, matchesByPhase,
        ...
        assertThat(row.getOpponentName()).as("AC4: opponent name must be team2's description").isEqualTo("Blaue Haie");
    ...
    void assemblesSingleTeamWithRefereeRow() {
        ...
        Match matchWithReferee = new Match(MATCH_ID, TENANT_ID, TOURNAMENT_ID, PHASE_ID,
                AVATAR1_ID, AVATAR2_ID,
                0, 1, 1, 3 /* field 3 */,
                TEAM3_ID /* refereeTeamId */, null, null, null);

        List<Team> teams = List.of(team1, team2, team3);
        List<Phase> phases = List.of(phase1);
        Map<UUID, List<TeamAvatar>> avatarsByPhase = Map.of(PHASE_ID, List.of(avatar1, avatar2, avatar3));
    ...
    void assemblesSingleTeamWithActivityRow() {
        ...
        de.vvwt.tm.domain.ActivityType activityType = buildActivityType(activityTypeId, "Mannschaftsfoto");

        // Configure mock to assign "Mannschaftsfoto" to team3 in lap 1
        ActivityAssignmentResult assignResult = new ActivityAssignmentResult(
                Map.of(activityType, List.of(
                        new de.vvwt.tm.domain.activity.ActivityAssignment(TEAM3_ID, 1, "Mannschaftsfoto"))),
        ...
        Map<UUID, List<LaufzettelRow>> result = assembler.assemble(
                tournament, phases, teams,
                avatarsByPhase, matchesByPhase,
                Collections.emptyMap(), List.of(activityType), 0);
    ...
    void assemblesSingleTeamWithFreeRow() {
        ...
        assertThat(team3Rows).hasSize(1);
        LaufzettelRow row = team3Rows.get(0);
        assertThat(row.isFree()).as("AC8: team3 must be FREE (no play, referee, or activity)").isTrue();
        assertThat(row.getRoundNumber()).isEqualTo(1);
        assertThat(row.isPlaying()).isFalse();
        assertThat(row.isRefereeing()).isFalse();
        assertThat(row.isActivity()).isFalse();
    ...
    void breakSeparatorRowAppended() {
        ...
        Match match2 = new Match(UUID.randomUUID(), TENANT_ID, TOURNAMENT_ID, PHASE_ID,
                AVATAR1_ID, AVATAR2_ID,
                0, 1, 2 /* lap 2 */, 1,
                null, null, null, null);
        ...
        Map<UUID, List<de.vvwt.tm.domain.PhaseBreak>> breaksByPhase = Map.of(PHASE_ID, List.of(phaseBreak));

        Map<UUID, List<LaufzettelRow>> result = assembler.assemble(
                tournament, phases, teams,
                avatarsByPhase, matchesByPhase,
                breaksByPhase, Collections.emptyList(), 0);
        ...
        assertThat(breakRow.getBreakTimeWindow()).as("AC9: break time window must be non-empty with start time set")
                .isNotEmpty();
    ...
    void noStartTimeProducesEmptyTimeWindows() {
        Tournament tournament = tournamentWithoutStartTime();
        assertThat(assembler.hasTime(tournament)).as("AC10: hasTime must be false when plannedStartTime is null").isFalse();
    ...
    void withStartTimeProducesFilledTimeWindows() {
        ...
        assertThat(assembler.hasTime(tournament)).as("AC10: hasTime must be true when plannedStartTime is set").isTrue();
    ...
    void multiPhaseScheduleHasPhaseHeaders() {
        ...
        TeamAvatar avP2T1 = new TeamAvatar(avatar1p2, TENANT_ID, TOURNAMENT_ID, phase2Id, 1, 1, TEAM1_ID, null, null);
        TeamAvatar avP2T2 = new TeamAvatar(avatar2p2, TENANT_ID, TOURNAMENT_ID, phase2Id, 1, 2, TEAM2_ID, null, null);
        ...
        // Expected for team1: PHASE_HEADER(Vorrunde), PLAYING(lap1), PHASE_HEADER(Finale), PLAYING(lap1)
        List<LaufzettelRow> rows = result.get(TEAM1_ID);
        assertThat(rows).as("AC11: two phases must produce 4 rows (header + match × 2)").hasSize(4);

        assertThat(rows.get(0).isPhaseHeader()).as("AC11: first row must be a phase header").isTrue();
        assertThat(rows.get(0).getPhaseHeaderName()).isEqualTo("Vorrunde");

        assertThat(rows.get(1).isPlaying()).isTrue();

        assertThat(rows.get(2).isPhaseHeader()).as("AC11: third row must be a phase header for phase 2").isTrue();
    ...
    void playingTakesPriorityOverRefereeing() {
        ...
        Match matchWithSelfReferee = new Match(MATCH_ID, TENANT_ID, TOURNAMENT_ID, PHASE_ID,
                AVATAR1_ID, AVATAR2_ID,
                0, 1, 1, 2,
                TEAM1_ID /* team1 as referee — intentionally contrived */, null, null, null);
        ...
        assertThat(row.isPlaying())
                .as("AC6: PLAYING must take priority over REFEREEING")
                .isTrue();
        assertThat(row.isRefereeing()).isFalse();
    }
    ...
    void emptyTeamsReturnsEmptyResult() {
        ...
        Map<UUID, List<LaufzettelRow>> result = assembler.assemble(
                tournament, phases, Collections.emptyList(),
    ...
    @Test
    @DisplayName("AC4: team description used as display name; fallback to 'Team N' when description is null")
    void teamDisplayNameFallsBackToTeamNumber() {
        ...
        TeamAvatar av3 = new TeamAvatar(avatar3InPhase, TENANT_ID, TOURNAMENT_ID, PHASE_ID, 1, 1, TEAM3_ID, null, null);
        TeamAvatar av1 = new TeamAvatar(avatar1InPhase, TENANT_ID, TOURNAMENT_ID, PHASE_ID, 1, 2, TEAM1_ID, null, null);
        Match m = new Match(UUID.randomUUID(), TENANT_ID, TOURNAMENT_ID, PHASE_ID,
                avatar3InPhase, avatar1InPhase,
                0, 1, 1, 1, null, null, null, null);
    ...
    private Tournament tournamentWithoutStartTime() {
        return new Tournament(TOURNAMENT_ID, TENANT_ID, "Test Turnier", "BEST_OF_3",
                "setPoints", "standardVolleyball", "roundRobin",
                "ACTIVE", null, null, 4, 8, null /* no start time */);
    }

    private Tournament tournamentWithStartTime(LocalTime startTime) {
        return new Tournament(TOURNAMENT_ID, TENANT_ID, "Test Turnier", "BEST_OF_3",
                "setPoints", "standardVolleyball", "roundRobin",
                "ACTIVE", null, null, 4, 8, startTime);
    }

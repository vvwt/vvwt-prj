...
/**
 * Verifies that the V10__e07s01_device_model_extension.sql Flyway migration is applied correctly
 * (AC1–AC4, E07S01).
 *
 * <p>Checks:
 *
 * <ul>
 *   <li>AC1 — {@code pin} column is now nullable
 *   <li>AC1 — {@code device_name} column is present and nullable
 *   <li>AC1 — {@code configuration} column is present and nullable
 *   <li>AC3 — Two devices with {@code pin = NULL} in the same tenant do NOT violate any constraint
 *       (unique constraint on (tenant_id, pin) was dropped in V10)
 *   <li>AC4 — Existing scoring tablet inserts (with pin) still work; values retained
 *   <li>AC5 — JSON configuration string can be stored and retrieved from the TEXT column
 * </ul>
 *
 * @see <a href="../../../../../.gaai/project/contexts/artefacts/stories/E07S01.story.md">Story
 *     E07S01</a>
 */
@SpringBootTest(
...
class E07S01MigrationIT {
    ...
    void devicesTableHasConfigurationColumn() {
        ...
        String nullable = (String) columns.get(0).get("IS_NULLABLE");
        assertThat(nullable)
                .as("AC1 — configuration must be nullable")
    ...
    void pinColumnIsNullable() {
        ...
        String nullable = (String) columns.get(0).get("IS_NULLABLE");
        assertThat(nullable)
                .as("AC1 — pin must be nullable after V10 migration")
                .isEqualToIgnoringCase("YES");
    }
    ...
    void twoNullPinDevicesInSameTenantDoNotViolateConstraint() {
        ...
        jdbcTemplate.update(
                "INSERT INTO tenants (id, display_name, tenant_location_count, is_default,"
                        + " created_at) VALUES (?, 'AC3Tenant', 1, FALSE, CURRENT_TIMESTAMP)",
                tenantId);
        jdbcTemplate.update(
                "INSERT INTO locations (id, tenant_id, display_name, created_at)"
                        + " VALUES (?, ?, 'AC3Loc', CURRENT_TIMESTAMP)",
                locationId,
                tenantId);
                ...
                () ->
                        jdbcTemplate.update(
                                "INSERT INTO devices (id, tenant_id, location_id, device_token,"
                                    ...
                                    + " ?, NULL, 'DISPLAY', 'REGISTERED', CURRENT_TIMESTAMP)",
    ...
    void scoringTabletWithPinStillInsertsAndRetainsPin() {
        ...
        jdbcTemplate.update(
                "INSERT INTO tenants (id, display_name, tenant_location_count, is_default,"
                        + " created_at) VALUES (?, 'AC4Tenant', 1, FALSE, CURRENT_TIMESTAMP)",
                tenantId);
        jdbcTemplate.update(
                "INSERT INTO locations (id, tenant_id, display_name, created_at)"
                        + " VALUES (?, ?, 'AC4Loc', CURRENT_TIMESTAMP)",
                locationId,
                tenantId);
        ...
        jdbcTemplate.update(
                "INSERT INTO devices (id, tenant_id, location_id, device_token, pin, device_type,"
                        + " status, registered_at) VALUES (?, ?, ?, ?, ?, 'SCORING_TABLET',"
    ...
    void configurationJsonCanBeStoredAndRetrieved() {
        ...
        jdbcTemplate.update(
                "INSERT INTO tenants (id, display_name, tenant_location_count, is_default,"
                        + " created_at) VALUES (?, 'AC5Tenant', 1, FALSE, CURRENT_TIMESTAMP)",
                tenantId);
        jdbcTemplate.update(
                "INSERT INTO locations (id, tenant_id, display_name, created_at)"
                        + " VALUES (?, ?, 'AC5Loc', CURRENT_TIMESTAMP)",
                locationId,
                tenantId);

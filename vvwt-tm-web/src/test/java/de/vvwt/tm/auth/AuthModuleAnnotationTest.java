 ...
 * <p>The deliberate-violation spike was run during E15S06 delivery (documented in
 * {@code E15S06.execution-plan.md}): a {@code Class<?>} field reference to
 * {@code de.vvwt.tm.tenant.internal.TenantFileRegistry} in
 * {@code de.vvwt.tm.auth.internal.PasswordGenerator} caused
 * {@code ApplicationModulesTest.verifiesModuleStructure} to FAIL with:
 * <pre>
 ...
 * <p>Confirms that:
 * <ol>
 *   <li>The {@code auth} module CAN reference types from the {@code tenant} public API
 *       (e.g., {@link TenantContext}, {@link TenantDataSourceResolver}) — these imports
 *       in this test class itself are visible to Spring Modulith's scanner.</li>
 *   <li>{@code ApplicationModules.verify()} does NOT object to this consumption.</li>
 * </ol>
 *
 ...
 * @see <a href="../../../../../../../../.gaai/project/contexts/artefacts/stories/E15S06.story.md">Story E15S06</a>
 * @since E15S06
 */
class AuthModuleAnnotationTest {
    ...
    void authPackageDeclaresApplicationModuleAnnotation() throws Exception {
        ...
        assertThat(authPackage)
                .as("Package de.vvwt.tm.auth must be present on the classpath (package-info.java compiled)")
                .isNotNull();
     ...
     * {@code @ApplicationModule} annotation present, including the positive fact that
     * the {@code auth} module's imports of {@code TenantContext} and
     * {@code TenantDataSourceResolver} (tenant public API) do NOT trigger a violation.
     *
     * <p>The static imports of {@link TenantContext} and {@link TenantDataSourceResolver}
     * at the top of this test class ensure the compiled bytecode of this test file references
     * those types. Spring Modulith scans test classpath too; if the annotation were wrong
     * (e.g., allowedDependencies empty), this file's references would cause verify() to fail.
     */
    @Test
    ...
    /**
     * AC6 — Verifies that the {@code auth} module is detected by Spring Modulith and
     * declares {@code tenant} as its sole allowed dependency.
     */
    @Test
    void authModuleAllowedDependenciesIncludeTenant() {
        ...
        assertDoesNotThrow(
                (Executable) modules::verify,
                "verify() must pass with auth module having allowedDependencies = {\"tenant\"} (AC6)");
    }
}

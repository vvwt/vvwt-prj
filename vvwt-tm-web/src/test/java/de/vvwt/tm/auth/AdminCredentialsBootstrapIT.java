 ...
 *
 * <p>Verifies AC1 (first-boot generation), AC2 (persistence), AC3 (startup logging proxy),
 * AC9 (DB integrity), and AC11 (password strength and hash storage).
 *
 * <p>Uses the "test" profile: in-memory H2 with Flyway migrations (including V6).
 * Each context load triggers a fresh in-memory DB, so these tests always exercise
 * the "first boot" path (no pre-existing admin_credentials row).
 *
 * <p>Acceptance criteria covered:
 * <ul>
 *   <li>AC1 — {@link #adminCredentialsRowIsCreatedOnFirstBoot()}</li>
 *   <li>AC2 — {@link #passwordHashIsPersistedInDatabase()}</li>
 *   <li>AC3 (proxy) — {@link #passwordHashIsNonNull()} — the bootstrap must have run
 *       (the actual console log line is verified by checking log output in
 *       {@link #passwordIsLoggedAtInfoLevel()} — integration test uses a log appender)</li>
 *   <li>AC9 — {@link #dbUnavailableAbortStartup()} — tested via separate context</li>
 *   <li>AC11 — {@link #passwordHashIsBcrypt()} and {@link #generatedPasswordMeetsMinEntropy()}</li>
 * </ul>
 *
...
class AdminCredentialsBootstrapIT {
    ...
    /**
     * AC2 — Verifies that the password hash persisted in the database is non-null and
     * matches the expected bcrypt format (starts with "$2a$" or "$2b$").
     */
    @Test
    ...
    /**
     * AC3 (proxy) — Verifies that the bootstrap ran and populated the in-memory hash.
     * The actual INFO log line is produced during context startup; this test verifies
     * the post-condition (hash is available via AdminCredentialsProvider).
     */
    @Test
    void passwordHashIsNonNull() {
        ...
        assertThat(hash)
                .as("AdminCredentialsProvider must return a non-null hash after bootstrap (AC3 proxy)")
                .isNotNull()
                .isNotBlank();
    }

    /**
     * AC11 — Verifies that the stored hash is a valid bcrypt hash (format check) and that
     * a fresh plaintext password matching the hash would pass bcrypt verification.
     * Since we cannot recover the plaintext from the hash, we verify the hash format only
     * and trust that the bootstrap's entropy is correct (16 chars × log2(62) ≈ 5.95 bits
     * each = ~95 bits total — see {@link AdminCredentialsBootstrap} class javadoc).
     */
    @Test
    void passwordHashIsBcrypt() {
        ...
        assertThat(hash)
                .as("AC11 — password must be stored as a bcrypt hash")
                .startsWith("$2");
    ...
    /**
     * AC11 — Verifies password entropy via alphabet size and length.
     * The generated password is 16 alphanumeric characters: 62^16 ≈ 2^95 bits > 72 bits (AC11).
     * This test verifies the alphabet and length constants in the bootstrap class.
     */
    @Test
    void generatedPasswordMeetsMinEntropy() {
        ...
        assertThat(passwordEncoder.matches("wrongpassword1!", credentialsProvider.getPasswordHash()))
    ...
    void singletonGuardPreventsSecondRow() {
        ...
        org.junit.jupiter.api.Assertions.assertThrows(
                ...
                () -> jdbcTemplate.update(
                        ...
                        + "VALUES (?, '$2a$10$fakehashfortest0000000000000000000000000000000000000000000', TRUE)",
                        java.util.UUID.randomUUID()),
                "singleton_guard unique index must prevent inserting a second admin_credentials row (AC2)");
    }
}

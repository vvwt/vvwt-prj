 ...
 * </ul>
 *
 * <p>Decision table for POST /api/v1/register:
 *
 * <ol>
 *   <li>Algorithm unknown → 400 {@link AlgorithmUnknownException}
 *   <li>Algorithm deprecated per DEC-48 boundary → 410 {@link AlgorithmDeprecatedException}
 *   <li>Primary profile (INVITATION_ONLY) — token missing/invalid → 403 {@link
 *       InvitationInvalidException}
 *   <li>Second key for existing tenant (different public key) → 409 {@link KeyMismatchException}
 *   <li>Self-host max-tenants exceeded (new tenant, no room) → 403 {@link
 *       TenantLimitExceededException}
 *   <li>All guards pass → insert/confirm tenant row, return {@link RegistrationResponse}
 * </ol>
 *
 * <p>Every request (accepted or rejected) is audit-logged per DEC-6 + AC13 (split signature_outcome
 * + rejection_reason per D-X5). First-registration is unsigned per D-X4(b) → signature_outcome =
 * NA.
 *
...
public class RegistrationService {
    ...
    // -------------------------------------------------------------------------
    // Typed exception hierarchy for registration rejections
    // -------------------------------------------------------------------------

    /**
     * Algorithm not found in registry → 400 Bad Request (AC3 ALGORITHM_UNKNOWN, RFC 7231 §6.5.1)
     */
    public static class AlgorithmUnknownException extends RuntimeException {
    ...
    public static class AlgorithmDeprecatedException extends RuntimeException {
    ...
    public static class KeyMismatchException extends RuntimeException {
     ...
     *
     * <p>Returns either a {@link RegistrationResponse} (success) or one of the typed exception
     * subtypes (rejection). The caller (controller) maps exceptions to HTTP status codes.
     *

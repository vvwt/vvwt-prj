 ...
 * </ul>
 *
 * <p><b>Per-request signature verification (AC10, AC11):</b> every signed request is verified via
 * {@link SignatureVerifierRegistry} before any business logic executes. Invalid signature → 401
 * {@link ErrorResponse.SignatureInvalid}; audit row written with {@code signature_outcome=INVALID}.
 *
 * <p><b>On-demand location creation (AC12):</b> for the default tenant in self-host mode, the first
 * publish/registration for an unseen {@code location_id} implicitly succeeds — {@code location_id}
 * is stored directly on the {@code tournament} row; no separate location entity is maintained.
 *
 * <p><b>Atomic supersede (AC6):</b> re-registration of an existing {@code (tenant, location,
 * tournament_id)} triple executes in a single {@code @Transactional} method: (1) prior tournament
 * row gets {@code superseded_at = now()}; (2) prior deltas are deleted; (3) new tournament row is
 * inserted; (4) audit row is written.
 *
 * <p><b>5xx-durability for audit-log (AC13):</b> audit writes use {@code Propagation.REQUIRES_NEW}
 * so they commit even when the outer transaction rolls back.
 *
...
public class PublishService {
    ...
    public static class SignatureInvalidException extends RuntimeException {
    ...
    public static class SeqMismatchException extends RuntimeException {

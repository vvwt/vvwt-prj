 ...
 *
 * <p>AC9: {@code VarietyScorer} is thread-safe by virtue of statelessness. 10 threads × 10000 calls
 * each must produce consistent, race-free results.
 */
class VarietyScorerConcurrencyTest {
    ...
    void concurrentScoringProducesConsistentResults() throws Exception {
                    ...
                    executor.submit(
                            () -> {
                                ...
                                for (int i = 0; i < CALLS_PER_THREAD; i++) {
                                    double result =
                                            scorer.scoreWithMatrix(perm, 4, 4, activeMatrix);
        ...
        assertThat(terminated).as("Executor must terminate within 30 seconds").isTrue();
        ...
        assertThat(inconsistencyCount.get())
                .as(
                        "No race-induced inconsistencies expected across %d threads × %d calls",

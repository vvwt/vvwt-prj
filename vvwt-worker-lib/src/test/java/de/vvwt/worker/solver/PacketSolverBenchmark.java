 ...
 * approximately 15 million permutations per benchmark call (derived from the H-2 gate
 * reference rate of ≥ 3 million perms/sec/core × 5 second target duration in Brief D-10).
 *
 ...
 * (Brief D-10). This benchmark measures ops/sec; multiply by
 * {@link #PACKET_SIZE} to derive perms/sec. The E01S12 ship-gate story will enforce this threshold
 * in a dedicated benchmark submodule.
 *
 * <h2>Advisory status</h2>
 * <p>This benchmark is in {@code test} scope and does NOT gate the Maven Surefire build.
 * To run it, use the JMH runner directly:
 * <pre>
...
public class PacketSolverBenchmark {
     ...
     *
     * <p>Rationale: at the H-2 gate threshold of 3M perms/sec, this packet takes ~5 seconds
     * to solve (Brief D-10 reference rate). A single benchmark call measures the throughput
     * of solving this packet, providing a meaningful sustained-load data point.
     *
     * <p>Note: 14! = 87_178_291_200. A packet of 15M perms is a small fraction (< 0.02%)
     * of the full 14! space, which is the realistic production packet size.
     */
    ...
    /**
     * Builds the benchmark state: a representative N=14 phase definition with a realistic
     * avatar distribution (7 avatars across 14 rows — alternating active/idle pattern
     * to exercise the variety scorer non-trivially).
     */
    @Setup
    public void setUp() {
     ...
     *
     * <p>Throughput in ops/sec × {@link #PACKET_SIZE} = perms/sec. The H-2 gate threshold
     * (≥ 3M perms/sec/core) corresponds to ≥ 0.2 ops/sec for this packet size.
     */
    @Benchmark

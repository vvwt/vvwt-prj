 ...
 * <ul>
 *   <li>Row-order permutations</li>
 *   <li>Within-row position-tuple reorderings</li>
 *   <li>{@code phaseId} substitutions</li>
 * </ul>
 *
...
class StructuralFingerprintPropertyTest {
    ...
    /**
     * Given a random valid RawPhaseDef and a permutation of its rows,
    ...
    void ac4_rowOrderPermutation_sameFingerprint(
        ...
        RawPhaseDef permuted = new RawPhaseDef(original.phaseId(),
                original.rowCount(), shuffledRows);
    ...
    /**
     * Given a random valid RawPhaseDef, shuffling positions WITHIN each row must
     * not change the fingerprint (row semantics are set semantics).
     */
    @Property(tries = 1000)
    void ac4_withinRowPositionReordering_sameFingerprint(
        ...
        RawPhaseDef reordered = new RawPhaseDef(original.phaseId(),
                original.rowCount(), reorderedRows);
    ...
    /**
     * Substituting phaseId with any other value must not change the fingerprint.
     * phaseId is audit-only metadata per DEC-9 and AC1 of E01S09.
     */
    @Property(tries = 1000)
    void ac4_phaseIdSubstitution_sameFingerprint(
        ...
        RawPhaseDef withDifferentPhaseId = new RawPhaseDef(alternatePhaseId,
                original.rowCount(), original.rows());
    ...
    /**
     * Two structurally DISTINCT RawPhaseDefs (different rowCount, or different
     * avatarCount, or different multiset of row position-sets) must produce
     * different fingerprints.
     *
     * <p>We generate guaranteed-distinct pairs by adding an extra distinct row to
     * the second definition. This ensures structural non-isomorphism.
     */
    @Property(tries = 1000)
    void ac5_structurallyDistinct_differentFingerprint(
        ...
        RawRow extraRow = new RawRow(
                List.of(new PositionTuple(sentinelGroup, 0)));

        List<RawRow> extendedRows = new ArrayList<>(base.rows());
        extendedRows.add(extraRow);

        RawPhaseDef extended = new RawPhaseDef(base.phaseId(),
                extendedRows.size(), extendedRows);
        ...
        assertThat(fpBase)
                .as("Structurally distinct inputs (different rowCount) must produce different fingerprints")
    ...
    /**
     * Generates valid {@link RawPhaseDef} instances with rowCount ∈ [3, 15]
     * and a small pool of shared avatars (to ensure some cross-row sharing,
     * which tests the multiset semantics properly).
     */
    @Provide
    Arbitrary<RawPhaseDef> validRawPhaseDefs() {
        ...
        return rowCounts.flatMap(rowCount ->
                net.jqwik.api.Arbitraries.integers().between(0, 999).flatMap(phaseId ->
                        generateRows(rowCount, avatarArb).map(rows ->
                                new RawPhaseDef(phaseId, rowCount, rows))));
    }

    /**
     * Generates {@code rowCount} rows, each with 2–4 distinct position tuples
     * drawn from the given avatar arbitrary.
     */
    private Arbitrary<List<RawRow>> generateRows(int rowCount,
                                                  Arbitrary<PositionTuple> avatarArb) {
        Arbitrary<RawRow> rowArb = avatarArb.list().ofMinSize(2).ofMaxSize(4)
                .map(tuples -> {
                    // Deduplicate (list-based set)
                    List<PositionTuple> distinct = tuples.stream().distinct()

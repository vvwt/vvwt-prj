...
/**
 * The canonical form of a phase definition, passed to workers and to the scorer.
 * Contains dense integer avatar indices in canonical order.
 *
 * <p>NO {@code phaseId}, NO {@code rowIndex}. The scorer (E01S02) accesses rows by
 * their list position only.
 *
 ...
 * {@link StructuralFingerprint#canonicalize(RawPhaseDef)}.
 *
 ...
 * @param rows        canonically ordered list of rows; each row is a sorted-ascending
 *                    list of dense integer avatar IDs
 */
    ...
    public CanonicalPhaseDef {
        if (rowCount < 0) {
            throw new IllegalArgumentException(
                    "rowCount must be >= 0 but was: " + rowCount);
        }
        if (avatarCount < 0) {
            throw new IllegalArgumentException(

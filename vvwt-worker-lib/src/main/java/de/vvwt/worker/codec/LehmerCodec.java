 ...
 *
 * <p>Provides deterministic, random-access conversion between rank (in lexicographic order)
 * and permutations of N elements, using the factorial number system (Lehmer code).
 *
 ...
 *   <li>{@link #permutationToRank(int[])} is the inverse: given a valid permutation of
 *       {@code [0, n-1]} it returns its 0-based lexicographic rank.
 ...
 *
 * <h2>Capacity</h2>
 * <p>Supports {@code n ∈ [1, 17]}. At n=17, the highest rank is {@code 17! - 1 = 355_687_428_095_999},
 * which fits comfortably in a {@code long} (max {@code long} ≈ 9.22 × 10^18). No overflow is
 * possible in intermediate factorial computations when the precomputed table is used.
 *
 * <h2>Thread-safety</h2>
...
public final class LehmerCodec {
    ...
    /**
     * Precomputed factorials 0! through 17!.
     * FACTORIAL[i] = i! as a long. Index 0 = 1 (0! = 1 by convention).
     * Overflow is impossible: FACTORIAL[17] = 355_687_428_096_000 &lt;&lt; Long.MAX_VALUE.
     */
    ...
    /**
     * Converts a 0-based lexicographic rank to the corresponding permutation of
     * {@code [0, n-1]}.
     *
     * <p>Algorithm: factoradic (Lehmer code) decomposition.
     * <ol>
     *   <li>Compute the Lehmer code digits {@code d[0..n-1]}: {@code d[i] = rank / (n-1-i)!},
     *       then {@code rank %= (n-1-i)!}.
    ...
    public static int[] rankToPermutation(long rank, int n) {
        ...
        if (rank < 0) {
            throw new IllegalArgumentException(
                    "rank must be non-negative, got: " + rank);
        }
        long maxRank = FACTORIAL[n] - 1L;
        if (rank > maxRank) {
            throw new IllegalArgumentException(
                    "rank " + rank + " is out of range for n=" + n
                    + "; maximum valid rank is " + maxRank + " (" + n + "! - 1)");
        }
     ...
     *
     * <p>Algorithm: compute the Lehmer code of the permutation, then evaluate it in the
     ...
     * @throws IllegalArgumentException if {@code perm} is null, empty, too long (length > 17),
     *                                  or is not a valid permutation of {@code [0, n-1]}
     */
    ...
    private static void validateN(int n) {
        if (n < 1) {
            throw new IllegalArgumentException(
                    "n must be at least 1, got: " + n);
        }
        if (n > MAX_N) {
            throw new IllegalArgumentException(
                    "n must be at most " + MAX_N + " (17! is the largest factorial that fits in long), got: " + n);
        }
    }
    ...
    private static void validatePermutation(int[] perm, int n) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (elem < 0 || elem >= n) {
                throw new IllegalArgumentException(
                        "perm[" + i + "]=" + elem + " is out of range [0, " + (n - 1) + "]"
                        + " for a permutation of length " + n);
            }
            if (present[elem]) {
                throw new IllegalArgumentException(
                        "perm contains duplicate value " + elem
                        + " — not a valid permutation of [0, " + (n - 1) + "]");
            }
            present[elem] = true;
        }
    }

 ...
 *
 * <p>Composes {@link LehmerCodec} and {@link VarietyScorer} into a single closed-form
 * function: {@link #solvePacket(JobDef, long, long) solvePacket(jobDef, rankFrom, rankTo) → PacketResult}.
 *
 * <h2>Determinism (AC2 — Brief D-3, Q-2)</h2>
 * <p>For identical inputs on any JVM, on any platform supported by Phase 1
 * (Linux/macOS/Windows × x86_64/aarch64), this method returns bit-identical
 * {@code bestRank} and {@code bestScore}. The guarantee holds because:
 * <ul>
 *   <li>Rank-to-permutation conversion is pure integer arithmetic ({@link LehmerCodec}).
 *   <li>Scoring uses only {@code *}, {@code +}, {@code /} on values promoted from {@code int}
 *       to {@code double} — IEEE 754 double-precision arithmetic is deterministic for this
 *       operation set on all supported platforms ({@link VarietyScorer#scoreWithMatrix}).
 ...
 * <p>When two permutations have equal {@code bestScore}, the permutation with the
 * <strong>lowest Lehmer rank</strong> wins. The implementation achieves this by iterating
 * ranks in ascending order and updating the best only on a <em>strict improvement</em>
 * ({@code newScore < bestScore}), never on equality. The first (lowest-rank) permutation
 * with a given score is therefore always retained.
 *
 * <h2>Security (AC6)</h2>
 * <p>No I/O, no logging, and no system-time sampling occur inside the hot loop.
 * {@code System.nanoTime()} is called exactly twice: immediately before and immediately after
 * the hot loop. This class is stateless and therefore unconditionally thread-safe.
 *
 * <h2>Abuse resistance (AC7)</h2>
 * <p>All inputs are validated at the packet boundary before the hot loop is entered.
 * A malformed or untrusted {@link CanonicalPhaseDef} (e.g., negative avatar indices,
 * row-count mismatch) is caught at matrix-build time and throws {@link IllegalArgumentException}
...
public final class PacketSolver {
    ...
    /**
     * Scores every permutation in the rank interval {@code [rankFrom, rankTo)} and returns the
     * best one — the permutation with the lowest variety score, with ties broken by lowest rank.
     *
     * <p>The method is deterministic: identical inputs always produce identical outputs, bit-for-bit,
     * on any JVM or platform (see class-level Javadoc for the proof sketch).
     *
     * @param jobDef   the job descriptor; must not be {@code null}; {@code jobDef.n} must be in
     *                 [{@value de.vvwt.worker.types.JobDef#MIN_N},
     *                  {@value de.vvwt.worker.types.JobDef#MAX_N}]
     * @param rankFrom inclusive lower bound of the rank interval; must be ≥ 0
     * @param rankTo   exclusive upper bound of the rank interval; must be ≤ {@code jobDef.n}!
     *                 and strictly greater than {@code rankFrom}
     ...
     *                                  rankFrom < 0, rankTo > n!, rankFrom >= rankTo, or
     *                                  malformed phaseDef)
     */
    public static PacketResult solvePacket(JobDef jobDef, long rankFrom, long rankTo) {
        ...
        try {
            activeMatrix = scorer.buildActiveMatrix(phaseDef.rows(), phaseDef.rowCount(), phaseDef.avatarCount());
        } catch (IllegalArgumentException e) {
            throw new IllegalArgumentException(
                    "PacketSolver rejected packet for job " + jobDef.jobId()
                    + ": malformed canonicalPhaseDef — " + e.getMessage(), e);
        }
        ...
        for (long rank = rankFrom; rank < rankTo; rank++) {
            ...
            final double score = scorer.scoreWithMatrix(permutation, rowCount, avatarCount, activeMatrix);
    ...
    /**
     * Computes {@code n!} for {@code n} in [1, 17]. All values fit in {@code long}.
     * Computed locally to avoid depending on the package-private {@code LehmerCodec.FACTORIAL} array.
     */
    ...
    private static void validateArguments(JobDef jobDef, long rankFrom, long rankTo) {
        ...
        if (rankFrom < 0) {
            throw new IllegalArgumentException(
                    "rankFrom must be >= 0, got: " + rankFrom);
        }
        ...
        if (rankFrom >= rankTo) {
            throw new IllegalArgumentException(
                    "rankFrom=" + rankFrom + " must be strictly less than rankTo=" + rankTo

...
/**
 * Constructs the normative 72-byte canonical byte layout for a packet result and
 * signs it using the worker's Ed25519 private key.
 *
 ...
 *
 * <p>{@code Double.doubleToLongBits} (not {@code doubleToRawLongBits}) is used to normalize
 * NaN representations, ensuring consistent byte output across platforms.
 *
...
public final class ResultSigner {
    ...
    /**
     * Builds the 72-byte canonical representation.
     * Package-visible for unit testing.
     */
    static byte[] buildCanonicalBytes(UUID packetId, UUID jobId, PacketResult result, UUID workerKeyId) {
    ...
    /**
     * Writes a UUID as 16 big-endian bytes: most-significant 8 bytes, then least-significant 8 bytes.
     * This matches the standard UUID binary encoding (RFC 4122 §4.1.2).
     */
    ...
    public String signPullNonce(UUID workerKeyId, String signedNonce) {
        ...
        ByteBuffer buf = ByteBuffer.allocate(prefix.length + 16 + nonceBytes.length)

...
/**
 * Thrown by {@link DispatcherClient} when a dispatcher endpoint returns an error
 * or when a network-level failure occurs.
 *
 * <p>A {@code statusCode} of {@code -1} indicates a network-level failure (I/O error,
 * connection refused, timeout) rather than an HTTP error response.
 */
public final class DispatcherException extends Exception {
    ...
    public DispatcherException(int statusCode, String responseBody, String endpoint) {
        super(String.format("Dispatcher %s returned HTTP %d: %s", endpoint, statusCode, responseBody));
    ...
    /**
     * Returns {@code true} if this exception represents an HTTP 4xx error response.
     * 4xx responses indicate a permanent client-side error — do not retry (AC6 of E01S05).
     */
    ...
    /**
     * Returns {@code true} if this exception represents a network-level failure
     * (connection refused, timeout, I/O error) — eligible for retry (AC5 of E01S05).
     */
    ...
    /**
     * Returns {@code true} if this exception represents an HTTP 5xx error response
     * — eligible for retry (AC5 of E01S05).
     */

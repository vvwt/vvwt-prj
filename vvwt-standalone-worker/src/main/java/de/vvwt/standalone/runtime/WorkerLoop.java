 ...
 *
 * <p>Network errors and 5xx responses are retried with exponential back-off: 1s, 2s, 4s, 8s, 16s,
 * then 30s steady. After 30 consecutive minutes of failures, logs WARN and exits with code 75
 * (EX_TEMPFAIL).
 *
 * <h2>4xx handling (AC6)</h2>
 *
 * <p>Any 4xx response is logged at ERROR with the full response body and exits with code 78
 * (EX_CONFIG). No retry.
 *
 * <h2>No-packet handling (AC3)</h2>
 *
 * <p>HTTP 204 from the dispatcher causes an idle sleep of {@code idlePollSeconds}, then the loop
 * retries.
 *
...
public final class WorkerLoop {
    ...
    /**
     * Exit code: configuration / permanent error — manual intervention required (sysexits.h
     * EX_CONFIG).
     */
    ...
    /**
     * Back-off schedule in seconds. Index represents consecutive failure count (capped at last
     * entry).
     */
    ...
    public int run() {
        ...
        while (running) {
            try {
                ...
                log.info("packet-pull", "Requesting next packet", "workerKeyId", workerKeyId);
                ...
                PacketResult result =
                        PacketSolver.solvePacket(jobDef, packet.rankFrom(), packet.rankTo());

                // Sign
                String base64Signature =
                        resultSigner.signResult(
                                packet.packetId(), packet.jobId(), result, workerKeyId);

                // Submit
                boolean firstResult =
                        dispatcherClient.submitResult(
                                packet.packetId(),
                                packet.jobId(),
                                result,
                                workerKeyId,
                ...
                if (elapsedFailureNanos >= MAX_FAILURE_WINDOW_NANOS) {
                    log.warn(
                            ...
                            "elapsedMinutes",
                            TimeUnit.NANOSECONDS.toMinutes(elapsedFailureNanos));
                ...
                log.info(
                        "retry",
                        "Dispatcher error, retrying with back-off",
                        "statusCode",
                        ex.getStatusCode(),
                        "backoffSeconds",
                        backoffSeconds,
                ...
                try {
                    ...
                    log.warn(
                            "interrupted",
                            "Worker loop interrupted during back-off, shutting down");
                    return 0;
                }

 ...
 *
 * <p>Note: These tests use mocks for {@link DispatcherClient} and {@link ResultSigner}
 * to avoid network calls. The 30-minute test uses an injected failure-window nanosecond
 * via subclassing to avoid actual waiting.
 */
class WorkerLoopTest {
    ...
    void run_4xxFromPullPacket_returnsExitConfig() throws Exception {
        when(mockResultSigner.signPullNonce(any(), anyString()))
                .thenReturn("dummysig");
        when(mockClient.pullPacket(any(), anyString(), anyString()))
                .thenThrow(new DispatcherException(401, "unauthorized", "pull-packet"));

        WorkerLoop loop = new WorkerLoop(config, mockClient, mockResultSigner, cpuThrottle, log, workerKeyId);
    ...
    void run_4xxFromSubmitResult_returnsExitConfig() throws Exception {
        ...
        CanonicalPhaseDef phaseDef = new CanonicalPhaseDef(3, 2,
                List.of(List.of(0, 1), List.of(0), List.of(1)));
        DispatcherClient.PullResult.PacketAssigned assigned =
                new DispatcherClient.PullResult.PacketAssigned(
                        UUID.randomUUID(), UUID.randomUUID(), phaseDef,
        ...
        when(mockClient.pullPacket(any(), anyString(), anyString()))
                .thenReturn(assigned);
        when(mockResultSigner.signResult(any(), any(), any(), any()))
                .thenReturn("resultsig");

 ...
 *
 * <h2>Purpose</h2>
 * <p>This benchmark converts the "Java is fast enough" conditional in Brief D-10 / H-2
 * into an empirically verified, reproducible, ship-blocking measurement. The conditional
 * (DEC-1 hard requirement for Java workers) is valid only if the worker achieves
 * ≤ 300 ns/perm on reference hardware. This benchmark measures exactly that.
 *
 * <h2>Reference Workload (AC1, AC2)</h2>
 * <p>The benchmark uses the fixed reference workload checked in at
 * {@code src/main/resources/h2-reference-N14.json}. The file contains a deterministic
 * {@link JobDef} with N=14 and a {@link CanonicalPhaseDef} derived from a 14-team
 * round-robin volleyball tournament structure with 4 courts per round (see JSON provenance).
 * The workload is NOT randomly generated at benchmark time — it is a fixed, reproducible input.
 *
 ...
 * <p>Target: Intel Xeon E-2236 (or comparable mid-range server CPU at 3.4 GHz base),
 * 4 active cores, JDK 21+, Linux x86_64.
 *
 * <p>The benchmark reports results in two forms:
 * <ol>
 *   <li>Absolute ns/perm on the actual hardware (computed by the gate wrapper script
 *       {@code gate/run-gate.sh} from the JMH JSON output).
 *   <li>Hardware identification (model, frequency, JVM, OS) logged by the gate script
 *       so deviations from the reference target are visible.
 * </ol>
 *
 ...
 * </pre>
 * With {@code PACKET_SIZE = 17_000_000}, this is a sub-run of the full 14! space
 * (87_178_291_200 ranks), sized to approximately 5 seconds at the H-2 gate threshold
 * (300 ns/perm × 17_000_000 = 5.1 s per JMH iteration), fitting within the 5-second
 * measurement window.
 *
 * <h2>Security (AC10)</h2>
 * <p>This benchmark uses ONLY the production code path of {@link PacketSolver#solvePacket}
 * with the canonical production type model. No test-mode shortcuts, no mocked dependencies,
 * no inlined test data via reflection. The reference workload is loaded from a classpath
 * resource to ensure the same input is used on every run.
 *
 * <h2>Story</h2>
...
public class KernelThroughputBenchmark {
     ...
     *
     * <p>Rationale: at the H-2 gate threshold of 300 ns/perm, a packet of 17 million
     * permutations takes approximately 5.1 seconds to solve, matching the JMH measurement
     * iteration duration. This makes each benchmark call a sustained-load data point
     * representative of real production packet sizes.
     *
     * <p>Note: 14! = 87_178_291_200. A packet of 17M perms is ~0.02% of the full rank space,
     * which is consistent with the real dispatcher packet decomposition strategy.
     */
    public static final long PACKET_SIZE = 17_000_000L;

    /**
     * 14! = 87_178_291_200 — the total number of permutations of 14 elements.
     * Used to validate that PACKET_SIZE does not exceed the rank space.
     */
     ...
     *
     * <p>Invoked once per trial (not per iteration) so that fixture loading time
     * is excluded from the benchmark measurement.
     *
    ...
    /**
     * Measures the average time to solve one packet of {@link #PACKET_SIZE} permutations
     * using the production {@link PacketSolver#solvePacket} code path.
     *
     * <p>JMH measures nanoseconds per call to this method. The gate wrapper computes
     * {@code nsPerPerm = result / PACKET_SIZE} to derive the per-permutation throughput.
     *
    ...
    /**
     * Loads and parses the reference workload from {@code h2-reference-N14.json}.
     * Uses only JDK classes — no external JSON library dependency.
     *
    ...
    private static JobDef loadReferenceWorkload() {
        ...
        String json;
        try (InputStream inputStream = KernelThroughputBenchmark.class.getResourceAsStream(resourcePath)) {
            if (inputStream == null) {
                throw new IllegalStateException(
                        "Reference workload not found on classpath: " + resourcePath
                        + ". Ensure vvwt-benchmark/src/main/resources/h2-reference-N14.json is present.");
            }
            json = new String(inputStream.readAllBytes(), StandardCharsets.UTF_8);
        } catch (java.io.IOException ioException) {
            throw new IllegalStateException(
                    "Failed to read reference workload from classpath: " + resourcePath, ioException);
        }
    ...
    /**
     * Minimal JSON parser for the reference workload — extracts only the fields needed
     * to construct a {@link JobDef}. Not a general-purpose parser.
     */
    private static JobDef parseJobDef(String json) {
        ...
        String topSection = (canonicalBlockStart > 0) ? json.substring(0, canonicalBlockStart) : json;
        ...
        if (rows.size() != rowCount) {
            throw new IllegalStateException(
                    "h2-reference-N14.json: rowCount=" + rowCount
                    + " but parsed " + rows.size() + " rows");
        }
    ...
    /**
     * Extracts the value of a JSON string field by name.
     * Handles the form {@code "fieldName": "value"}.
     *
    ...
    /**
     * Extracts the value of a JSON integer field by name.
     * Handles the form {@code "fieldName": 42}.
     *
    ...
    /**
     * Extracts a JSON object block for a named field.
     * Finds the opening brace after {@code "fieldName":} and returns the balanced block.
     */
    ...
    /**
     * Extracts the {@code rows} field as a list of integer lists from a JSON block.
     * Handles the form {@code "rows": [[0,1,2], [3,4,5], ...]}.
     */

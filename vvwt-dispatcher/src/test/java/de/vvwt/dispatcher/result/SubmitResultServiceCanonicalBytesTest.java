 ...
 * <p>Verifies:
 * <ul>
 *   <li>Total length is exactly 72 bytes</li>
 *   <li>Each field occupies the correct byte range in the correct byte order</li>
 *   <li>{@code bestScoreBits} is the IEEE 754 raw bits of the double (big-endian)</li>
 * </ul>
 */
class SubmitResultServiceCanonicalBytesTest {
    ...
    void canonicalBytes_length_is72() {
        ...
        byte[] bytes = SubmitResultService.buildCanonicalBytes72(
    ...
    void canonicalBytes_packetId_bigEndian() {
        ...
        byte[] bytes = SubmitResultService.buildCanonicalBytes72(
                packetId, jobId, 0L, 0.0, 0L, workerId);
    ...
    void canonicalBytes_differentScore_differentBits() {
        ...
        byte[] bytes1 = SubmitResultService.buildCanonicalBytes72(
                packetId, jobId, 0L, 1.0, 0L, workerId);
        byte[] bytes2 = SubmitResultService.buildCanonicalBytes72(
                packetId, jobId, 0L, 2.0, 0L, workerId);
        ...
        for (int i = 40; i < 48; i++) {
            if (bytes1[i] != bytes2[i]) { anyDiff = true; break; }
        }

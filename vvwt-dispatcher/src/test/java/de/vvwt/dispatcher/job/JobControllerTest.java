 ...
 *
 * <p>Tests AC5 (happy path queued), AC6 (signature failure), AC7 (role check), AC8 (N-cap), AC9
 * (cache hit), AC10 (cache miss), AC11 (error handling), and AC12 (audit logging).
 *
...
class JobControllerTest {
    ...
    void setup() {
        ...
        validSignatureBase64 =
                Base64.getEncoder().encodeToString(new byte[64]); // mock — service is mocked
    }
    ...
    private String buildRequest() {
        return """
                ...
                """
                .formatted(validPhaseDefJson, validSignatureBase64, submitterKeyId)
                .trim();
    }
    ...
    void submitJob_nCapExceeded_returns422() throws Exception {
        when(jobService.submit(any())).thenThrow(new JobService.NCapExceededException(16));
        ...
        mockMvc.perform(
                ...
                .andExpect(
                        jsonPath("$.message")
                                .value(
                                        "Phase 1 supports N <= 15. N = 16 requires Phase 2 (not yet"
                                                + " shipped)."));
    }
    ...
    void submitJob_missingRequiredField_returns400() throws Exception {
        ...
        mockMvc.perform(
                        post("/submit-job")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(
                                        "{\"phaseDef\":{\"phaseId\":1,\"rowCount\":0,\"rows\":[]},\"signature\":\"abc\"}"))
    ...
    void submitJob_transientDatabaseError_returns503WithRetryAfter() throws Exception {
        when(jobService.submit(any()))
                .thenThrow(
                        new JobService.DatabaseException(
                                "connection timeout", new RuntimeException("jdbc error")));
    ...
    void submitJob_unauthorizedPath_writesAuditEntry() throws Exception {
        when(jobService.submit(any())).thenThrow(new JobService.UnauthorizedException("bad sig"));

 ...
 * <p>Tests:
 * <ul>
 *   <li>AC5: 200 response shape (packetId, jobId, jobDef, rankFrom, rankTo, deadline)</li>
 *   <li>AC6: 204 No Content when no pending packets</li>
 *   <li>AC4: 401 on stale nonce, 403 on wrong role</li>
 *   <li>AC9: 400 on malformed payload</li>
 *   <li>AC10: audit service called on every outcome</li>
 * </ul>
 *
...
class PullPacketControllerTest {
    ...
    @MockitoBean
    private PullPacketService pullPacketService;
    ...
    void pullPacket_success_returns200WithExpectedShape() throws Exception {
        ...
        JobRecord job = new JobRecord(jobId, UUID.randomUUID(), 1,
                "{}", canonicalJson, new byte[32], "ready", Instant.now());
        ...
        PullPacketRequest req = new PullPacketRequest(workerKeyId, "sigBase64", Instant.now().toString());
        mockMvc.perform(post("/pull-packet")
    ...
    void pullPacket_staleNonce_returns401WithErrorCode() throws Exception {
        UUID workerKeyId = UUID.randomUUID();
        when(pullPacketService.execute(any(), anyString(), anyString()))
                .thenThrow(new PullPacketService.UnauthorizedException(
    ...
    void pullPacket_malformedPayload_returns400() throws Exception {
        UUID workerKeyId = UUID.randomUUID();
        when(pullPacketService.execute(any(), anyString(), anyString()))
                .thenThrow(new IllegalArgumentException("signedNonce is not a valid ISO-8601 instant"));

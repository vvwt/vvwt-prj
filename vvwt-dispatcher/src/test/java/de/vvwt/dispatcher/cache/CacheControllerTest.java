...
class CacheControllerTest {
    ...
    @MockitoBean private ResultsCacheService cacheService;
    ...
    void get_cacheHit_returns200WithBody() throws Exception {
        ...
        when(cacheService.lookup(any(byte[].class), eq(1), eq(1))).thenReturn(Optional.of(hit));

        mockMvc.perform(
                        get("/cache/" + VALID_FINGERPRINT_HEX)
    ...
    void get_cacheMiss_returns404() throws Exception {
        when(cacheService.lookup(any(byte[].class), eq(1), eq(1))).thenReturn(Optional.empty());

 ...
 *
 * <p>Every call is logged regardless of outcome — success or failure — per AC12 of E01S06. Fields:
 *
 * <ul>
 *   <li>{@code loggedAt} — wall clock at log time (server-side)
 *   <li>{@code sourceIp} — remote address from {@code HttpServletRequest}
 *   <li>{@code endpoint} — {@code /register-key} or {@code /submit-job}
 *   <li>{@code keyId} — the key ID involved in the call; null if resolution failed
 *   <li>{@code signatureOutcome} — {@code VERIFIED}, {@code FAILED}, or {@code N/A}
 *   <li>{@code httpStatus} — the HTTP response status actually returned
 * </ul>
 *
 * <p>This entity is NEVER updated after creation. Rows must not be deleted except by a privileged
 * archival process outside of the dispatcher.
 *

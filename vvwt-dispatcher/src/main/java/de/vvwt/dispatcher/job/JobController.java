 ...
 * <h2>Responses</h2>
 * <ul>
 *   <li>{@code 202 Accepted} — job queued (AC10) or cache hit (AC9)</li>
 *   <li>{@code 400 Bad Request} — missing/invalid fields or rowCount out of range (AC5, AC8, AC11)</li>
 *   <li>{@code 401 Unauthorized} — unknown key, expired key, or signature failure (AC6)</li>
 *   <li>{@code 403 Forbidden} — worker key calling submit-job (AC7)</li>
 *   <li>{@code 422 Unprocessable Entity} — rowCount 16–17 (AC8)</li>
 *   <li>{@code 503 Service Unavailable} — transient database error (AC11)</li>
 *   <li>{@code 500 Internal Server Error} — persistent database error (AC11)</li>
 * </ul>
 *
 * <p>Every call (success or failure) is audit-logged via {@link AuditService} (AC12).
 * The audit log records: source IP, endpoint, keyId-or-null, signatureOutcome, httpStatus.
 *
        ...
        try {
            ...
            return switch (result) {
                case JobService.SubmitResult.Queued queued -> {
                    auditService.log(httpReq, ENDPOINT, request.submitterKeyId(),
                            AuditEntry.OUTCOME_VERIFIED, HttpStatus.ACCEPTED.value());
            ...
            auditService.log(httpReq, ENDPOINT, request.submitterKeyId(),
                    AuditEntry.OUTCOME_NA, HttpStatus.UNPROCESSABLE_ENTITY.value());
            return ResponseEntity.status(HttpStatus.UNPROCESSABLE_ENTITY)
                    .body(Map.of(
                            "error", "N-cap exceeded",
                            "message", "Phase 1 supports N <= 15. N = " + nCapExceeded.getRowCount()
            ...
            log.warn("Unauthorized submit-job attempt from {} for keyId={}: {}",
                    httpReq.getRemoteAddr(), request.submitterKeyId(), unauthorized.getMessage());
            ...
            auditService.log(httpReq, ENDPOINT, request.submitterKeyId(),
                    AuditEntry.OUTCOME_NA, HttpStatus.SERVICE_UNAVAILABLE.value());

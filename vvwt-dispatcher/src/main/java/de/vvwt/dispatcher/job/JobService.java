 ...
 *
 * <ol>
 *   <li>Resolve {@code submitterKeyId} → {@link KeyRegistration} (AC5, AC7)
 *   <li>Verify role = {@code "submitter"} (AC7)
 *   <li>Check key validity (not superseded + grace expired) (AC6)
 *   <li>Validate {@code phaseDef.rowCount}: 1–15 accepted; 16–17 → 422; &lt;1 or &gt;17 → 400 (AC8)
 *   <li>JCS-canonicalize the {@code phaseDef} JSON field (AC6)
 *   <li>Verify Ed25519 signature against canonical bytes (AC6)
 *   <li>Parse {@code phaseDef} into {@link RawPhaseDef} and call {@link
 *       StructuralFingerprint#transform(RawPhaseDef)} (AC9, AC10)
 *   <li>Cache lookup via {@link ResultsCacheService} (AC9)
 *   <li>Cache hit → 202 cached (AC9); cache miss → create job row + 202 queued (AC10)
 * </ol>
 *
 * <p>Database errors are surfaced as {@link DatabaseException} (transient → 503) or {@link
 * RuntimeException} (persistent → 500) per AC11.
 *
...
public class JobService {
    ...
    /**
     * Result type returned by {@link #submit} to allow the controller to choose the appropriate
     * HTTP status and audit outcome.
     */
    public sealed interface SubmitResult permits SubmitResult.Queued, SubmitResult.Cached {
    ...
    public SubmitResult submit(SubmitJobRequest request) {
        ...
        if (keyOpt.isEmpty()) {
            throw new UnauthorizedException("Unknown submitterKeyId: " + request.submitterKeyId());
        }
        ...
        if (rowCount < 1 || rowCount > N_CAP_PHASE2_MAX) {
            throw new IllegalArgumentException(
                    "rowCount must be between 1 and "
                            + N_CAP_PHASE2_MAX
                            + " (inclusive), got: "
                            + rowCount);
        }
        ...
        Optional<CachedResult> cacheHit =
                cacheService.lookup(
                        fingerprint,
        ...
        if (cacheHit.isPresent()) {
            ...
            log.info(
                    "submit-job CACHE HIT for keyId={} phaseId={}",
                    key.getKeyId(),
                    rawPhaseDef.phaseId());
        ...
        log.info(
                "submit-job QUEUED jobId={} for keyId={} phaseId={}",
                jobId,
                key.getKeyId(),
                rawPhaseDef.phaseId());

 ...
 *
 * <p>Created on cache miss — the job is queued for decomposition by E01S07. Cache-hit submissions
 * do NOT create a job row (AC9).
 *
 * <p>Fields:
 *
 * <ul>
 *   <li>{@code jobId} — UUID assigned at intake
 *   <li>{@code submitterKeyId} — the registered key that signed the submission
 *   <li>{@code phaseId} — from {@code phaseDef.phaseId} (audit-only, per DEC-9)
 *   <li>{@code rawPhaseDefJson} — the original {@code phaseDef} JSON text as received
 *   <li>{@code canonicalPhaseDefJson} — JSON representation of the {@link
 *       de.vvwt.worker.types.CanonicalPhaseDef}
 *   <li>{@code fingerprint} — 32-byte SHA-256 of the canonical form
 *   <li>{@code status} — {@code "queued"} on creation; updated by E01S07
 *   <li>{@code submittedAt} — intake timestamp
 *   <li>{@code packetCount} — set when job transitions to {@code "ready"} (E01S07 AC1)
 *   <li>{@code priority} — higher value = higher priority for packet distribution (E01S07 AC5)
 * </ul>
 *
...
public class JobRecord {
    ...
    /**
     * Set when the job transitions to {@code "ready"} after decomposition (E01S07 AC1). Null while
     * the job is still {@code "queued"} or {@code "decomposing"}.
     */
    @Column(name = "packet_count")
    private Integer packetCount;

    /**
     * Job priority for packet distribution order (E01S07 AC5). Higher value = higher priority.
    ...
    public JobRecord(
            ...
            UUID submitterKeyId,
            int phaseId,
            String rawPhaseDefJson,
            String canonicalPhaseDefJson,
            byte[] fingerprint,
            String status,
            Instant submittedAt) {
        this.jobId = jobId;
    ...
    public UUID getSubmitterKeyId() {
        return submitterKeyId;
    }
    ...
    public String getRawPhaseDefJson() {
        return rawPhaseDefJson;
    }

    public String getCanonicalPhaseDefJson() {
        return canonicalPhaseDefJson;
    }

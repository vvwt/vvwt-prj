 ...
 *
 * <p>Ed25519 signs raw message bytes directly per RFC 8032 — NO pre-hashing. The caller is
 * responsible for providing the canonical message bytes (e.g. JCS-canonicalized JSON UTF-8) before
 * calling {@link #verify}.
 *
...
public final class Ed25519Verifier {
    ...
    private static final byte[] ED25519_SPKI_PREFIX = {
        ...
        0x2b,
        0x65,
        0x70, // 1.3.101.112 (id-EdDSA / Ed25519)
        0x03,
     ...
     *
     * <p>The 32 raw bytes are wrapped in a SubjectPublicKeyInfo DER envelope (RFC 8410) so the JDK
     * {@code KeyFactory} can parse them.
     *
    ...
    public static PublicKey decodePublicKey(byte[] rawPublicKeyBytes) {
        ...
        try {
            ...
            throw new InvalidSignatureException(
                    "Cannot parse Ed25519 public key bytes: " + invalidSpec.getMessage(),
                    invalidSpec);
        }
    }
     ...
     *
     * <p>The signature is verified against {@code messageBytes} directly — Ed25519 does NOT
     * pre-hash (RFC 8032 §5.1). Callers MUST provide the canonical message bytes (AC6:
     * JCS-canonicalized UTF-8 bytes of the {@code phaseDef} field).
     *
     ...
     * @throws IllegalStateException if the JVM does not support Ed25519 (should not happen on JDK
     *     21)
     */
    public static void verify(
            byte[] rawPublicKeyBytes, byte[] messageBytes, byte[] signatureBytes) {

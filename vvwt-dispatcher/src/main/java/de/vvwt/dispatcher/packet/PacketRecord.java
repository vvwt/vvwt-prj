 ...
 *
 * <p>Created by {@link PacketDecomposerService} when a job transitions from {@code "queued"} to
 * {@code "ready"} (E01S07 AC1).
 *
 * <p>Status lifecycle:
 *
 * <pre>
 ...
 *
 * <ul>
 *   <li>{@code packetId} — UUID assigned at creation
 *   <li>{@code jobId} — references {@code jobs.job_id}
 *   <li>{@code rankFrom} — inclusive start of the rank interval (0-based Lehmer rank)
 *   <li>{@code rankTo} — exclusive end of the rank interval
 *   <li>{@code status} — {@code "pending"}, {@code "assigned"}, {@code "done"}, or {@code "failed"}
 *   <li>{@code attempts} — total number of times this packet has been assigned (incremented on each
 *       claim)
 *   <li>{@code assignedTo} — workerKeyId currently holding this packet; null when not assigned
 *   <li>{@code assignedAt} — timestamp when last assigned; null when not assigned
 *   <li>{@code reissueHistory} — TEXT column storing a JSON array of reissue events (AC8); each
 *       entry: {@code {"timestamp":"...","workerKeyId":"...","attemptNumber":N}}
 * </ul>
 *
...
public class PacketRecord {
    ...
    /**
     * Deadline by which the assigned worker must submit the result (E01S08 AC7). Set when the
     * packet is assigned (pull-packet); null when not assigned.
     */
    @Column(name = "deadline")
    private Instant deadline;

    /**
     * JSON array of reissue audit entries (TEXT column). Null until the first reissue. Each entry:
    ...
    public String getReissueHistory() {
        return reissueHistory;
    }

    public Long getFirstBestRank() {
        return firstBestRank;
    }
    ...
    public UUID getFirstWorkerKeyId() {
        return firstWorkerKeyId;
    }
     ...
     *
     * <p>Sets {@code status='assigned'}, records the worker, timestamp, and deadline, and
     ...
     * Production code MUST use {@link #assign(UUID, Instant, Instant)} to ensure the deadline is
     * always populated for E01S08 AC7.
     *
     * @param workerKeyId the worker that claimed this packet
     * @param at assignment timestamp
     */
     ...
     *
     * <p>Transitions {@code status} to {@code "done"}, clears the worker assignment fields, and
     * stores the first-result data for finalization and late-result comparison.
     *
     ...
     *
     * <p>Clears {@code assignedTo}, {@code assignedAt}, and {@code deadline} and sets {@code
     * status='pending'}. The {@code attempts} counter is NOT reset. Call {@link
     * #appendReissueHistory(String)} BEFORE calling this method to preserve the outgoing worker
     ...
     *
     * <p>The history is stored as a JSON array in a TEXT column. A null or empty history is
     * initialized to a one-element array.
     *
    ...
    public void appendReissueHistory(String jsonEntry) {
        if (this.reissueHistory == null || this.reissueHistory.isBlank()) {
            ...
            this.reissueHistory =
                    this.reissueHistory.substring(0, this.reissueHistory.lastIndexOf(']'))
                            + ","
                            + jsonEntry
                            + "]";
        }

 ...
 *
 * <ol>
 *   <li>Parse {@code signedNonce} as an ISO-8601 {@link Instant}.
 *   <li>Reject if nonce is older than 60 s or more than 5 s in the future → 401.
 *   <li>Lookup {@code workerKeyId} in the key registry → 401 if not found.
 *   <li>Reject if role is not {@code "worker"} → 403.
 *   <li>Reject if key has expired (grace window passed) → 401.
 *   <li>Build canonical bytes: {@code "pull-packet"} UTF-8 (11 bytes) + {@code workerKeyId} 16-byte
 *       big-endian UUID + {@code signedNonce} UTF-8 bytes of the ISO-8601 string. Ed25519 signs
 *       these bytes directly per RFC 8032 — NO pre-hashing (AC3).
 *   <li>Verify Ed25519 signature → 401 on failure.
 * </ol>
 *
 * <h2>Atomic claim (AC5, AC6, AC7)</h2>
 *
 * <ol>
 *   <li>Call {@link PacketRepository#findAndLockNextPendingPacket()} — {@code SELECT FOR UPDATE
 *       SKIP LOCKED} prevents duplicate assignment.
 *   <li>If no pending packet found → return {@link PullResult.NoWork} (204).
 *   <li>Update packet: {@code status='assigned'}, {@code assignedTo}, {@code assignedAt}, {@code
 *       attempts++}.
 *   <li>Return {@link PullResult.Assigned} with packet + job.
 * </ol>
 *
...
public class PullPacketService {
    ...
    public sealed interface PullResult permits PullResult.Assigned, PullResult.NoWork {

        /** A packet was successfully claimed. */
        record Assigned(PacketRecord packet, JobRecord job, Instant deadline)
        ...
        return packetRepository
                ...
                .map(
                        packet -> {
                            ...
                            JobRecord job =
                                    jobRepository
                                            .findById(packet.getJobId())
                                            .orElseThrow(
                                                    () ->
                                                            new IllegalStateException(
                                                                    "Job not found for packet "
                                                                            + packet.getPacketId()
                                                                            + ": jobId="
                                                                            + packet.getJobId()));

                            log.info(
                                    "pull-packet: assigned packetId={} jobId={} workerKeyId={}"
                                            + " deadline={}",
                                    packet.getPacketId(),
                                    packet.getJobId(),
                                    workerKeyId,
    ...
    /**
     * Validates the nonce window, looks up the key, checks the role, and verifies the Ed25519
     * signature (AC3, AC4).
     *
    ...
    KeyRegistration authenticate(UUID workerKeyId, String base64Sig, String signedNonce) {
        ...
        if (!key.isValidForVerificationAt(now)) {
            throw new UnauthorizedException(
                    "unauthorized", "Key " + workerKeyId + " has expired (grace window passed)");
        }
        ...
        try {
            ...
            throw new UnauthorizedException(
                    "unauthorized",
                    "Signature verification failed for workerKeyId "
                            + workerKeyId
                            + ": "
    ...
    /**
     * Builds the canonical byte sequence for Ed25519 signature verification (AC3): {@code
     * "pull-packet"} (11 UTF-8 bytes) + {@code workerKeyId} (16 bytes UUID big-endian) + {@code
     * signedNonce} (UTF-8 bytes of the ISO-8601 string).
     *
    ...
    static byte[] buildCanonicalBytes(UUID workerKeyId, String signedNonce) {
        ...
        System.arraycopy(
                ...
                0,
                result,
                PULL_PACKET_PREFIX.length + uuidBytes.length,
                nonceBytes.length);
        return result;
    }

 ...
 *
 * <ul>
 *   <li>{@link #findAndLockNextPendingPacket()} — atomic claim via {@code SELECT FOR UPDATE SKIP
 *       LOCKED} (AC7)
 *   <li>{@link #findTimedOutPackets(Instant)} — reissue sweeper input (AC8)
 *   <li>{@link #countByJobIdAndStatus(UUID, String)} — used after decomposition to verify (AC1)
 * </ul>
 *
...
public interface PacketRepository extends JpaRepository<PacketRecord, UUID> {
     ...
     *
     * <p>Two concurrent callers will never receive the same packet: SKIP LOCKED causes the second
     * caller to skip any row already locked by the first (AC7).
     *
     * <p>Ordering: highest job priority first, then oldest job first (FIFO within priority), then
     * lowest {@code rank_from} first (deterministic packet ordering within a job).
     *
     ...
     *
     * <p>Used by the timeout sweeper to reissue packets that have been held by a worker longer than
     * the configured packet timeout (AC8).
     *
     ...
     *
     * <p>Used after decomposition to verify the packet count and after sweeping to assess job-level
     * failure state.
     *
     ...
     *
     * <p>Used by the job finalizer (E01S08 AC9) to collect all first-results and compute the global
     * best across the job.
     *
     * @param jobId the job UUID

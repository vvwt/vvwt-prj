 ...
 *
 * <ul>
 *   <li>{@code 200 OK} — packet assigned; body is {@link PullPacketResponse}
 *   <li>{@code 204 No Content} — no pending packets available; worker should back off (AC6)
 *   <li>{@code 400 Bad Request} — malformed request body (AC9)
 *   <li>{@code 401 Unauthorized} — authentication failure (unknown key, bad sig, stale nonce) (AC4,
 *       AC9)
 *   <li>{@code 403 Forbidden} — non-worker key (submitter calling pull-packet) (AC4)
 *   <li>{@code 500 Internal Server Error} — unexpected error
 * </ul>
 *
...
public class PullPacketController {
    ...
    public PullPacketController(
            PullPacketService pullPacketService,
    ...
    public ResponseEntity<?> pullPacket(
            @RequestBody PullPacketRequest request, HttpServletRequest httpReq) {

        try {
            PullPacketService.PullResult result =
                    pullPacketService.execute(
                            request.workerKeyId(), request.signature(), request.signedNonce());

            return switch (result) {
                case PullPacketService.PullResult.Assigned assigned -> {
                    PacketRecord packet = assigned.packet();
                    CanonicalPhaseDef jobDef =
                            parseCanonicalPhaseDef(assigned.job().getCanonicalPhaseDefJson());
                    PullPacketResponse body =
                    ...
                    auditService.log(
                            ...
                            HttpStatus.OK.value());
                    log.info(
                            "pull-packet success: workerKeyId={} packetId={} jobId={}",
                            request.workerKeyId(),
                            packet.getPacketId(),
                            packet.getJobId());
            ...
            log.warn(
                    "pull-packet unauthorized: workerKeyId={} error={} reason={}",
                    request.workerKeyId(),
            ...
            log.error(
                    "INCIDENT: Unexpected error on pull-packet for workerKeyId={}",
                    request.workerKeyId(),
                    unexpected);
            auditService.log(
                    httpReq,

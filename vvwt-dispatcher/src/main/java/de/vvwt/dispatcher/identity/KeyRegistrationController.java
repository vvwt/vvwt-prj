 ...
 *
 * <ul>
 *   <li>{@code 200 OK} — key registered (or idempotent match)
 *   <li>{@code 400 Bad Request} — missing/invalid fields or invalid public key
 *   <li>{@code 409 Conflict} — same key bytes registered with a different role
 * </ul>
 *
...
public class KeyRegistrationController {
    ...
    public ResponseEntity<?> registerKey(
            @RequestBody RegisterKeyRequest request, HttpServletRequest httpReq) {
        ...
        try {
            ...
            auditService.log(
                    httpReq, ENDPOINT, null, AuditEntry.OUTCOME_NA, HttpStatus.BAD_REQUEST.value());
            return ResponseEntity.badRequest().body(Map.of("error", "invalid public key"));

        } catch (RoleConflictException conflict) {
            auditService.log(
                    httpReq, ENDPOINT, null, AuditEntry.OUTCOME_NA, HttpStatus.CONFLICT.value());
            return ResponseEntity.status(HttpStatus.CONFLICT)
                    .body(Map.of("error", "role conflict", "message", conflict.getMessage()));
        }
    }
}

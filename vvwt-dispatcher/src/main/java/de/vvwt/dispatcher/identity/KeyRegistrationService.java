 ...
 * <h2>Happy path</h2>
 * <ol>
 *   <li>Validate {@code role} ({@code "worker"} or {@code "submitter"})</li>
 *   <li>Decode and validate the Base64 {@code publicKey} field (must be exactly
 *       32 bytes and parseable as Ed25519 — AC2)</li>
 *   <li>Check for existing registration:
 *       <ul>
 *         <li>Same bytes + same role → idempotent, return existing keyId (AC4)</li>
 *         <li>Same bytes + different role → {@link RoleConflictException} → 409 (AC4)</li>
 *         <li>No match → proceed to registration</li>
 *       </ul>
 *   </li>
 *   <li>If {@code supersedes} is present:
 *       <ul>
 *         <li>Decode and find the matching key by raw bytes + same role</li>
 *         <li>Mark it superseded with {@code supersededAt = now} and
 *             {@code graceExpiresAt = now + dispatcher.packet.timeout × 2} (AC3)</li>
 *       </ul>
 *   </li>
 *   <li>Insert new {@link KeyRegistration} row; return keyId + role + registeredAt</li>
 * </ol>
 *
 * <p>The grace window default is {@code dispatcher.packet.timeout × 2} minutes.
 * This is the SHARED timeout config also used by E01S07 — do NOT introduce a
 * separate grace-window knob (AC3 explicitly forbids two independent timeout knobs).
 *
...
public class KeyRegistrationService {
    ...
    public RegisterKeyResponse register(RegisterKeyRequest request) {
        ...
        for (KeyRegistration registration : existing) {
            if (registration.getRole().equals(role)) {
                ...
                return new RegisterKeyResponse(
                        registration.getKeyId(), registration.getRole(),
                        registration.getRegisteredAt());
            } else {
                // Role conflict: same key + different role → 409
                throw new RoleConflictException(
                        "Public key is already registered with role '" + registration.getRole()
                        + "'; cannot register as '" + role + "'");
            }
        }
        ...
        if (request.supersedes() != null && !request.supersedes().isBlank()) {
            ...
            if (supersededOpt.isPresent()) {
                ...
                KeyRegistration newRegistration = new KeyRegistration(
        ...
        KeyRegistration registration = new KeyRegistration(
                keyId, role, publicKeyBytes, registeredAt, request.name());
     ...
     * @throws IllegalArgumentException if the string is null/blank, not valid Base64,
     *                                  or the decoded bytes are not a valid Ed25519 key
     */

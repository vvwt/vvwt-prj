 ...
 *
 * <p>Every submission is logged regardless of outcome — success, rejection, or error — per DEC-6
 * mechanism 10. The row is committed in a separate transaction ({@code REQUIRES_NEW}) so it
 * persists even if the outer transaction is rolled back.
 *
 * <p>Fields:
 *
 * <ul>
 *   <li>{@code loggedAt} — server-side wall clock at log time
 *   <li>{@code sourceIp} — remote address from the HTTP request
 *   <li>{@code workerKeyId} — the worker key ID from the request; may be null if unparseable
 *   <li>{@code packetId} — the packet ID from the request; may be null
 *   <li>{@code jobId} — the job ID from the request; may be null
 *   <li>{@code signatureOutcome} — {@code VERIFIED}, {@code FAILED}, or {@code N/A}
 *   <li>{@code decisionOutcome} — one of the {@code DECISION_*} constants
 *   <li>{@code httpStatus} — the HTTP response status code returned
 * </ul>
 *
...
public class ResultAuditEntry {
    ...
    public String getSignatureOutcome() {

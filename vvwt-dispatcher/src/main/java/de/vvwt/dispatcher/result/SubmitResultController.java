 ...
 * <h2>Responses</h2>
 * <ul>
 *   <li>{@code 200 OK} — result accepted or late-logged; body is {@link SubmitResultResponse}</li>
 *   <li>{@code 400 Bad Request} — missing/invalid fields or malformed JSON (AC1, AC11)</li>
 *   <li>{@code 401 Unauthorized} — signature verification failed (AC2)</li>
 *   <li>{@code 404 Not Found} — unknown packetId (AC11)</li>
 *   <li>{@code 409 Conflict} — not assigned to this worker (AC6)</li>
 *   <li>{@code 410 Gone} — deadline exceeded (AC7)</li>
 *   <li>{@code 503 Service Unavailable} — transient DB error (AC11)</li>
 * </ul>
 *
...
public class SubmitResultController {
    ...
    public ResponseEntity<?> submitResult(
        ...
        try {
            ...
            String decision = response.firstResult()
                    ? ResultAuditEntry.DECISION_ACCEPTED_FIRST
                    : ResultAuditEntry.DECISION_ACCEPTED_LATE;

            auditService.log(httpReq,
                    req.workerKeyId(), req.packetId(), req.jobId(),
                    ResultAuditEntry.SIG_VERIFIED, decision,
                    HttpStatus.OK.value());
            ...
            auditService.log(httpReq,
                    safeWorkerKeyId(req), safePacketId(req), safeJobId(req),
                    ResultAuditEntry.SIG_NA, ResultAuditEntry.DECISION_REJECTED_OTHER,
            ...
            auditService.log(httpReq,
                    safeWorkerKeyId(req), safePacketId(req), safeJobId(req),
                    ResultAuditEntry.SIG_FAILED, ResultAuditEntry.DECISION_REJECTED_SIGNATURE,
                    HttpStatus.UNAUTHORIZED.value());
            log.warn("submit-result: unauthorized from {} workerKeyId={}: {}",
                    httpReq.getRemoteAddr(), safeWorkerKeyId(req), unauthorized.getMessage());
            ...
            auditService.log(httpReq,
                    safeWorkerKeyId(req), safePacketId(req), safeJobId(req),
                    ResultAuditEntry.SIG_VERIFIED, ResultAuditEntry.DECISION_REJECTED_NOT_ASSIGNED,
            ...
            auditService.log(httpReq,
                    safeWorkerKeyId(req), safePacketId(req), safeJobId(req),
                    ResultAuditEntry.SIG_VERIFIED, ResultAuditEntry.DECISION_REJECTED_DEADLINE,
                    HttpStatus.GONE.value());
            return ResponseEntity.status(HttpStatus.GONE)
                    .body(Map.of(
                            "error", "deadline-exceeded",
                            "deadline", deadlineExceeded.getDeadline().toString()));

        } catch (Exception unexpected) {
            log.error("INCIDENT: Unexpected error on submit-result workerKeyId={} packetId={}",
                    safeWorkerKeyId(req), safePacketId(req), unexpected);
            auditService.log(httpReq,
                    safeWorkerKeyId(req), safePacketId(req), safeJobId(req),
                    ResultAuditEntry.SIG_NA, ResultAuditEntry.DECISION_REJECTED_OTHER,

 ...
 *
 * <p>A late result is a valid signed submission for a packet that is already {@code "done"} — i.e.,
 * a first valid result has already been accepted. Per DEC-6 mechanism 11, late results are logged
 * but never override the first.
 *
 * <p>If {@code matchesFirst} is {@code false}, a WARN log is emitted (AC5) indicating a discrepancy
 * between the first and late results — a tamper-detection signal per Brief O-9 / D-11.
 *
...
public class LateResult {
    ...
    /**
     * {@code true} if {@code (bestRank, bestScore)} matches the first accepted result. {@code
     * false} signals a potential tamper/defect event (AC5).
     */

 ...
 * <ol>
 *   <li>Validate request fields (AC1, AC11).</li>
 *   <li>Parse {@code bestScore} from decimal string (AC1).</li>
 *   <li>Look up packet and worker key; enforce assignment check (AC6) and deadline (AC7).</li>
 *   <li>Verify Ed25519 signature over 72-byte canonical bytes (AC2).</li>
 *   <li>If packet is already {@code "done"}: log late result (AC4, AC5), return late response.</li>
 *   <li>If packet is {@code "assigned"} to this worker: accept first result (AC3).</li>
 *   <li>After first result: check whether this was the last packet; if so, finalize the job (AC9, AC10).</li>
 * </ol>
 *
...
public class SubmitResultService {
    ...
    public SubmitResultService(PacketRepository packetRepository,
    ...
    public static class PacketNotFoundException extends RuntimeException {
        public PacketNotFoundException(String message) { super(message); }
    }
    ...
    public SubmitResultResponse process(SubmitResultRequest req) {
        ...
        PacketRecord packet = packetRepository.findById(req.packetId())
                .orElseThrow(() -> new PacketNotFoundException(
                        "Unknown packetId: " + req.packetId()));

        // --- Load worker key (AC2 signature, AC6 assignment check) ---
        KeyRegistration key = keyRepository.findById(req.workerKeyId())
        ...
        if (packet.getDeadline() != null && now.isAfter(packet.getDeadline())) {
            throw new DeadlineExceededException(
                    "Deadline exceeded for packet " + req.packetId(),
                    packet.getDeadline());
        }
        ...
        log.info("submit-result: first result accepted packetId={} jobId={} workerKeyId={} bestRank={} bestScore={}",
                req.packetId(), req.jobId(), req.workerKeyId(), req.bestRank(), bestScore);
     ...
     * <ol>
     *   <li>{@code packetId} — 16 bytes UUID big-endian</li>
     *   <li>{@code jobId} — 16 bytes UUID big-endian</li>
     *   <li>{@code bestRank} — 8 bytes long big-endian</li>
     *   <li>{@code bestScoreBits} — 8 bytes from {@code Double.doubleToLongBits(bestScore)} big-endian</li>
     *   <li>{@code permutationsScored} — 8 bytes long big-endian</li>
     *   <li>{@code workerKeyId} — 16 bytes UUID big-endian</li>
     * </ol>
     *
     * <p>Ed25519 signs these 72 bytes DIRECTLY per RFC 8032 — NO pre-hashing.
     */
    static byte[] buildCanonicalBytes72(UUID packetId, UUID jobId,
                                         long bestRank, double bestScore,
                                         long permutationsScored, UUID workerKeyId) {
    ...
    private void verifySignature(SubmitResultRequest req, double bestScore, KeyRegistration key) {
        byte[] canonical = buildCanonicalBytes72(
        ...
        try {
            ...
            throw new BadRequestException("signature is not valid Base64: " + badBase64.getMessage());
        }

        try {
            Ed25519Verifier.verify(key.getPublicKeyBytes(), canonical, sigBytes);
        } catch (InvalidSignatureException sigFail) {
            throw new UnauthorizedException("Signature verification failed: " + sigFail.getMessage());
        }
    }
     ...
     *
     * <p>Checks whether the late result matches the first result. If not, emits a WARN
     * log (AC5 tamper-detection signal). Always logs to the {@code late_results} table.
     * Returns a duplicate response if the same worker submits the same result again.
     */
    private SubmitResultResponse handleLateResult(PacketRecord packet,
                                                   SubmitResultRequest req,
                                                   double bestScore,
                                                   Instant now) {
        boolean rankMatches  = packet.getFirstBestRank()  != null
                               && packet.getFirstBestRank() == req.bestRank();
        boolean scoreMatches = packet.getFirstBestScore() != null
        ...
        if (workerMatches && matchesFirst) {
            log.info("submit-result: duplicate from first worker packetId={} workerKeyId={}",
                    req.packetId(), req.workerKeyId());
            return SubmitResultResponse.ofDuplicate();
        }

        // Tamper-detection signal (AC5): diverging (bestRank, bestScore)
        if (!matchesFirst) {
            log.warn("submit-result: TAMPER SIGNAL late result diverges from first — " +
                     "packetId={} jobId={} firstWorkerKeyId={} lateWorkerKeyId={} " +
                     "firstResult={{rank={}, score={}}} lateResult={{rank={}, score={}}}",
                    req.packetId(), req.jobId(),
                    packet.getFirstWorkerKeyId(), req.workerKeyId(),
                    packet.getFirstBestRank(), packet.getFirstBestScore(),
                    req.bestRank(), bestScore);
        } else {
            log.info("submit-result: late result matches first packetId={} workerKeyId={}",
                    req.packetId(), req.workerKeyId());
        }

        // Persist late result (AC4)
        LateResult lateResult = new LateResult(
                req.packetId(), req.jobId(), req.workerKeyId(),
                req.bestRank(), bestScore, now, matchesFirst);
     ...
     *
     * <p>Finalization is deterministic (AC10): lowest {@code bestScore} wins;
     * on tie, lowest {@code bestRank} wins (Brief D-3). The result is written to
     * the results cache (E01S09 AC6).
     *
    ...
    private void checkAndFinalizeJob(UUID jobId) {
        ...
        PacketRecord globalBest = donePacketList.stream()
        ...
        log.info("submit-result: job {} finalized — bestRank={} bestScore={}",
                jobId, globalBest.getFirstBestRank(), globalBest.getFirstBestScore());
    }

    /**
     * Extracts the number of avatars ({@code n}) from the job's canonical phase definition.
     *
     * <p>For Phase 1, {@code n} is the team count from the canonical form. This is used
     * as the {@code n} parameter in the cache write (E01S09). A simplified extraction is
     * used here — reads the row count from the job's canonical phaseDef JSON length signature.
     * For correctness in Phase 1, we obtain {@code n} from the packet count total.
     *
     * <p>Note: the canonical phase definition already contains {@code rowCount} / group count
     * which is exactly {@code n}. Since {@link JobRecord} stores the canonical JSON, we use
     * the packet count as a proxy: {@code packetCount} was set at decomposition time. The actual
     * {@code n} (avatar count) from the {@code CanonicalPhaseDef} would require parsing the JSON;
     * to avoid a JSON parse dependency here, we delegate to the stored job row.
     */
    private int extractN(JobRecord job) {
        ...
        try {
            ...
            log.warn("Could not parse rowCount from canonicalPhaseDefJson for job {}: {}",
                    job.getJobId(), parseError.getMessage());
            return 0;
        }
    }
    ...
    private void validateRequest(SubmitResultRequest req) {
        ...
        if (req.permutationsScored() == null) throw new BadRequestException("permutationsScored is required");
        if (req.wallClockNanos() == null)    throw new BadRequestException("wallClockNanos is required");

 ...
 *
 * <p>Returns the current status and packet counts for a job, along with the best-so-far result and
 * the final result (if finalized).
 *
...
public class JobStatusController {
    ...
    public ResponseEntity<?> getJobStatus(@PathVariable("jobId") UUID jobId) {
        ...
        JobStatusResponse.ResultSummary bestSoFar =
                donePackets.stream()
                        ...
                        .map(
                                p ->
                                        new JobStatusResponse.ResultSummary(
        ...
        if ("done".equals(job.getStatus())) {
            ...
            int canonicalizationVersion =
                    de.vvwt.worker.types.StructuralFingerprint.CANONICALIZATION_VERSION;
            cacheService
                    .lookup(job.getFingerprint(), scoreFnVersion, canonicalizationVersion)
                    .ifPresent(
                            cached -> {
                                // captured in a local — finalResult reassignment not possible in
                                // lambda
                            });

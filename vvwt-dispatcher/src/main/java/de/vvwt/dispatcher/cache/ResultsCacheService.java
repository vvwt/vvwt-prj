 ...
 *
 * <h2>Lookup (AC7)</h2>
 * Returns an {@link Optional} of the best known result for a given structural fingerprint
 * and version combination. Returns {@link Optional#empty()} on cache miss. Hit/miss is
 * logged at DEBUG (high volume, AC12).
 *
 * <h2>Write (AC8)</h2>
 * Idempotent on identical {@code (bestRank, bestScore)}; throws
 * {@link CacheCorruptionException} on diverging values for the same composite key.
 * Writes are logged at INFO with the fingerprint hex prefix and
 * {@code (n, bestScore)} per AC12.
 *
 * <h2>Startup (AC12)</h2>
...
public class ResultsCacheService {
    ...
    public Optional<CachedResult> lookup(byte[] fingerprint,
                                          int scoreFnVersion,
                                          int canonicalizationVersion) {
        CachedResultId id = new CachedResultId(fingerprint, scoreFnVersion,
                canonicalizationVersion);
        Optional<CachedResultEntity> entity = repository.findById(id);

        if (entity.isPresent()) {
            log.debug("Cache HIT fingerprint={} scoreFnVersion={} canonicalizationVersion={}",
                    fingerprintPrefix(fingerprint), scoreFnVersion, canonicalizationVersion);
        } else {
            log.debug("Cache MISS fingerprint={} scoreFnVersion={} canonicalizationVersion={}",
                    fingerprintPrefix(fingerprint), scoreFnVersion, canonicalizationVersion);
        }
     ...
     *
     * <p>Idempotent: if an identical entry already exists, this method is a no-op.
     * Throws {@link CacheCorruptionException} if the same composite key already
     * exists with different {@code (bestRank, bestScore)} values.
     *
     * @param fingerprint            32-byte SHA-256 fingerprint
     * @param scoreFnVersion         scorer algorithm version
     * @param canonicalizationVersion canonicalization algorithm version
     ...
     * @throws CacheCorruptionException if an existing entry has different
     *         {@code (bestRank, bestScore)} values for the same composite key
     */
    @Transactional
    public void write(byte[] fingerprint, int scoreFnVersion, int canonicalizationVersion,
                      long bestRank, double bestScore, int n, UUID sourceJobId) {
        ...
        if (existing.isPresent()) {
            ...
            throw new CacheCorruptionException(
                    "Cache corruption detected for fingerprint=" + fingerprintPrefix(fingerprint)
                    + " scoreFnVersion=" + scoreFnVersion
                    + " canonicalizationVersion=" + canonicalizationVersion
                    + ": existing=(rank=" + existingEntity.getBestRank()
                    + ", score=" + existingEntity.getBestScore()
                    + ") new=(rank=" + bestRank + ", score=" + bestScore + ")");
        }

        CachedResultEntity entity = new CachedResultEntity(id, bestRank, bestScore, n,
                Instant.now(), sourceJobId);
        repository.save(entity);

        log.info("Cache WRITE fingerprint={} n={} bestScore={}",
                fingerprintPrefix(fingerprint), n, bestScore);
    }
    ...
    /**
     * Returns the first 16 hex characters (8 bytes) of the fingerprint for
     * log messages (AC12: "fingerprint hex prefix, first 16 chars").
     */
